...
function rentownoscKanalowaPanelHTML(){
  ...
  return `<div class="panel allegro-section-panel profitability-page">
    ...
    <section class="profit-von-halsky-prices"><div class="order-section-head"><div><span class="order-pro-label">Cena trzeciego kanału</span><h3>🐕 Ceny Von Halsky</h3><p class="order-detail-lead">Brak własnej ceny oznacza automatyczne dziedziczenie aktualnej ceny Allegro. Ustaw wartość tylko dla produktów, które mają mieć inną cenę w tym kanale.</p></div></div><div class="profit-von-halsky-grid">${visible.slice(0,100).map(({p})=>{const v=vonHalskyRentownoscProduktu(p);return `<article><div><b>${esc(p.nazwa||"Produkt")}</b><small>Von Halsky: ${zl(v.price)} • ${kwotaNum(p.cenaVonHalsky)>0?"własna cena":"dziedziczy Allegro"}</small></div>${vonHalskyScenariuszeMarzyHTML(p)}</article>`;}).join("")||"<p>Brak produktów w aktywnym filtrze.</p>"}</div>${visible.length>100?`<small>Pokazano pierwsze 100 wyników. Zawęź listę wyszukiwarką, aby ustawić kolejne produkty bez obciążania przeglądarki.</small>`:""}</section>
