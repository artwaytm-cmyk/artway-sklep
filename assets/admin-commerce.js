...
function adminStatusyZamowienHTML(wszystkie){
  const ile=s=>s==="wszystkie"?wszystkie.length:s==="wymaga_decyzji"?wszystkie.filter(z=>z.wymagaPotwierdzeniaDostepnosci).length:wszystkie.filter(z=>z.status===s).length;
  return `<div class="orders-status-strip">
    ...
    <button class="${filtrZamowien==="wymaga_decyzji"?"active":""}" onclick="filtrZamowien='wymaga_decyzji';renderuj()">Wymaga decyzji <b>${ile("wymaga_decyzji")}</b></button>
    ${STATUSY.map(s=>`<button class="${filtrZamowien===s?"active":""}" onclick="filtrZamowien=${jsArg(s)};renderuj()">${esc(s)} <b>${ile(s)}</b></button>`).join("")}
  </div>`;
}
function adminEtapRealizacjiZamowieniaSklepu(z={}){
  const status=String(z.status||"").trim().toLowerCase(),etap=String(etapWysylki(z)||daneWysylki(z).etap||"").toLowerCase(),wysylka=daneWysylki(z);
  if(["anulowane","zwrot","zwrot pieniędzy"].includes(status)||["anulowana","zwrot"].includes(etap))return "anulowane";
  if(["dostarczone","zakończone"].includes(status)||etap==="dostarczona")return "zrealizowane";
  if(z.wymagaPotwierdzeniaDostepnosci)return "do_decyzji";
  if(["w doręczeniu"].includes(status)||["transport","doreczenie"].includes(etap))return "w_transporcie";
  if(wysylka.numer||wysylka.inpostId||status==="nadane"||status==="wysłane")return "nadane";
  if(status==="nowe")return "nowe";
  return "do_nadania";
}
function adminZamowienieSklepuAktywne(z={}){return !["zrealizowane","anulowane"].includes(adminEtapRealizacjiZamowieniaSklepu(z));}
function adminTimestampZamowieniaSklepu(z={}){
  const raw=z.ts||z.createdAt||z.utworzono||z.data||"",numeric=Number(raw);
  if(Number.isFinite(numeric)&&numeric>1e9)return numeric<1e11?numeric*1000:numeric;
  return Date.parse(raw)||0;
}
function adminDostawaZamowieniaSklepu(z={}){
  const text=`${z.dostawa||""} ${z.metodaDostawy||""} ${daneWysylki(z).usluga||""}`.toLowerCase();
  if(czyZamowieniePaczkomat(z)||text.includes("paczkomat"))return "paczkomat";
  if(text.includes("punkt")||text.includes("pop"))return "punkt";
  return "kurier";
}
function adminEtapyZamowienSklepuHTML(wszystkie=[]){
  ...
  wszystkie.forEach(z=>{const etap=adminEtapRealizacjiZamowieniaSklepu(z);counts[etap]=(counts[etap]||0)+1;if(adminZamowienieSklepuAktywne(z)&&etap!=="do_obslugi")counts.do_obslugi++;});
  const stages=[["wszystkie","▦","Wszystkie","pełny rejestr"],["nowe","✦","Nowe","czekają na przyjęcie"],["do_obslugi","!","Do obsługi","wymagają działania"],["do_decyzji","?","Do decyzji","przyjmij lub odrzuć"],["do_nadania","＋","Do nadania","bez przesyłki"],["nadane","🏷","Nadane","etykieta utworzona"],["w_transporcie","→","W transporcie","śledzenie InPost"],["zrealizowane","✓","Zrealizowane","dostarczone"],["anulowane","×","Anulowane","zwroty i odrzucenia"]];
  return adminEtapyRealizacjiZamowienHTML({active:filtrEtapuZamowienSklepu,items:stages.map(([id,icon,label,note])=>({id,icon,label,note,count:counts[id],onclick:`filtrEtapuZamowienSklepu=${jsArg(id)};renderuj()`}))});
}
function adminPasujaceZamowieniaSklepu(){
  let lista=pobierzZamowienia().slice();
  if(filtrZamowien==="wymaga_decyzji")lista=lista.filter(z=>z.wymagaPotwierdzeniaDostepnosci);
  else if(filtrZamowien!=="wszystkie")lista=lista.filter(z=>z.status===filtrZamowien);
  if(filtrEtapuZamowienSklepu==="do_obslugi")lista=lista.filter(adminZamowienieSklepuAktywne);
  else if(filtrEtapuZamowienSklepu!=="wszystkie")lista=lista.filter(z=>adminEtapRealizacjiZamowieniaSklepu(z)===filtrEtapuZamowienSklepu);
  if(filtrOkresuZamowienSklepu!=="wszystkie"){
    const days=filtrOkresuZamowienSklepu==="dzisiaj"?1:Number(filtrOkresuZamowienSklepu)||0,threshold=Date.now()-days*86400000;
    lista=lista.filter(z=>adminTimestampZamowieniaSklepu(z)>=threshold);
  }
  if(filtrDostawyZamowienSklepu!=="wszystkie")lista=lista.filter(z=>adminDostawaZamowieniaSklepu(z)===filtrDostawyZamowienSklepu);
  if(szukajZamowien)lista=lista.filter(z=>adminZamowienieSearchText(z).includes(szukajZamowien));
  lista.sort((a,b)=>sortZamowienSklepu==="najstarsze"?adminTimestampZamowieniaSklepu(a)-adminTimestampZamowieniaSklepu(b):sortZamowienSklepu==="wartosc_desc"?kwotaNum(kosztyZamowienia(b).razem)-kwotaNum(kosztyZamowienia(a).razem):sortZamowienSklepu==="wartosc_asc"?kwotaNum(kosztyZamowienia(a).razem)-kwotaNum(kosztyZamowienia(b).razem):adminTimestampZamowieniaSklepu(b)-adminTimestampZamowieniaSklepu(a));
  return lista;
}
...
function adminZaznaczWidoczneZamowienia(checked=true){
  adminPasujaceZamowieniaSklepu().slice(0,limitWidokuZamowienSklepu).forEach(z=>checked?zaznaczoneZamowieniaSklepu.add(String(z.nr)):zaznaczoneZamowieniaSklepu.delete(String(z.nr)));renderuj();
}
function adminWyczyscFiltryZamowienSklepu(){szukajZamowien="";filtrZamowien="wszystkie";filtrEtapuZamowienSklepu="wszystkie";filtrOkresuZamowienSklepu="wszystkie";filtrDostawyZamowienSklepu="wszystkie";sortZamowienSklepu="najnowsze";renderuj();}
...
async function allegroZapiszAutoUzupelnienia(p,d={}){
  ...
  if(auto.sourceEvidence?.imageSourceType==="product_source_page"&&fields.zdjecie){force.zdjecie=fields.zdjecie;force.zdjecia=extraImages;force.sourceEvidence=auto.sourceEvidence;changed=true;}
...
function allegroPanelOperacyjnyStaty(){
  ...
  const liczbaOfert=allegroDaneZaladowane.offers?oferty.length:Number(allegroPodsumowanie.offers?.count||0),serwerPodpiete=Number(allegroPodsumowanie.offers?.mapped||0),liczbaPodpietych=Math.min(liczbaOfert,Math.max(allegroDaneZaladowane.offers?podpiete:0,serwerPodpiete)),liczbaAktywnych=allegroDaneZaladowane.orders?aktywneZamowienia.length:Number(allegroPodsumowanie.orders?.active||0);
...
function allegroSubnavHTML(aktywny="start",st=allegroPanelOperacyjnyStaty()){
  ...
  return adminSubnavHTML([
    ...
    {id:"oferty",href:"#/admin/allegro/oferty",label:"🏷️ Oferty i publikacja",badge:zadaniaWystawiania||""},
...
function allegroWorkspaceSectionHTML(aktywna,mapped,niepodpiete,st=allegroPanelOperacyjnyStaty()){
  const cfg={
    ...
    oferty:{ico:"🏷️",kicker:"Kompletny proces ofertowy",title:"Oferty i publikacja Allegro",opis:"Dopasowanie produktu, kontrola danych, przygotowanie, publikacja i rejestr istniejących ofert działają w jednym spójnym procesie.",metryki:[["Wszystkie oferty",st.oferty],["Powiązane",mapped],["Do działania",st.zadaniaWystawiania+niepodpiete]]},
  ...
  return `<section class="panel allegro-workspace-section ${aktywna==="zamowienia"?"channel-orders-header channel-orders-header-allegro":""}" data-allegro-channel-header><div class="allegro-workspace-title"><span>${cfg.ico||"🟠"}</span><div><small>${esc(cfg.kicker||"Allegro")}</small><h1>${esc(cfg.title||"Panel Allegro")}</h1><p>${esc(cfg.opis||"")}</p></div></div><div class="allegro-workspace-metrics">${(cfg.metryki||[]).map(([l,v])=>`<div><small>${esc(l)}</small><b>${esc(v)}</b></div>`).join("")}</div></section>`;
}
function allegroStartPanelHTML(st=allegroPanelOperacyjnyStaty()){
  ...
  return `<div class="allegro-command-center">
    <section class="panel allegro-command-hero">
      <div class="allegro-command-hero-copy"><span class="order-pro-label">Stan operacyjny kanału</span><h2>Najważniejsze działania na teraz</h2><p>Zamówienia, oferty, publikacja i obsługa klienta mają osobne odpowiedzialności, ale korzystają z jednego potwierdzonego stanu Allegro.</p><div class="allegro-command-health"><span class="${allegroStan.connected?"ok":"warning"}"><i></i>${allegroStan.connected?"API Allegro połączone":"Połączenie wymaga uwagi"}</span><span>↻ zamówienia i komunikacja co 15 min</span><span>🏷️ ostatni odczyt ofert: ${esc(ostatniaOferta?allegroDataTxt(ostatniaOferta):"oczekuje")}</span></div></div>
      <div class="allegro-command-hero-actions"><a class="btn" href="#/admin/allegro/zamowienia">📦 Obsłuż zamówienia${st.aktywneZamowienia?` (${st.aktywneZamowienia})`:""}</a><a class="btn ghost" href="#/admin/allegro/oferty">🟠 Oferty i publikacja</a><a class="btn ghost" href="#/admin/allegro/ustawienia">⚙️ Ustawienia integracji</a></div>
    </section>
    ...
    <section class="panel allegro-module-directory"><div class="order-section-head"><div><span class="order-pro-label">Nawigacja procesowa</span><h2>Wszystkie obszary pracy</h2></div></div><div>${[["📦","Zamówienia","Zlecenia, stan, braki i realizacja","#/admin/allegro/zamowienia"],["🏷️","Oferty i publikacja","Dopasowanie, przygotowanie, publikacja i rejestr","#/admin/allegro/oferty"],["💬","Wiadomości","Korespondencja z klientami","#/admin/allegro/wiadomosci"],["🛟","Dyskusje","Reklamacje i sprawy formalne","#/admin/allegro/dyskusje"],["🛡️","Zgodność","Kontrola opisów i bezpieczeństwo","#/admin/allegro/zgodnosc"],["⚙️","Ustawienia","OAuth, synchronizacja i automatyka","#/admin/allegro/ustawienia"]].map(([i,t,d,h])=>`<a href="${h}"><span>${i}</span><div><b>${t}</b><small>${d}</small></div><em>→</em></a>`).join("")}</div></section>
  </div>`;
}
...
function widokAdminAllegro(sekcja="start"){
  ...
  const zakres=aktywna==="zamowienia"?"orders":aktywna==="oferty"?"offers":aktywna==="zgodnosc"?"offers":aktywna==="ustawienia"?"config":"summary";
  ...
  ${aktywna==="zamowienia"?"":allegroWorkspaceSectionHTML(aktywna,mapped,niepodpiete,staty)}
  ${aktywna==="zamowienia"?allegroZamowieniaTabelaHTML():aktywna==="oferty"?(typeof allegroOfertyIPublikacjaHTML==="function"?allegroOfertyIPublikacjaHTML():allegroWystawianiePanelHTML()):aktywna==="wiadomosci"?allegroKomunikacjaPanelHTML("thread"):aktywna==="dyskusje"?allegroKomunikacjaPanelHTML("issue"):aktywna==="zgodnosc"?allegroZgodnoscPanelHTML():aktywna==="ustawienia"?allegroUstawieniaPanelHTML():allegroStartPanelHTML(staty)}
...
function widokAdminOplacalnosc(){
  allegroLadujJesliTrzeba("offers");
  const all=produktyDoAdministracji().filter(p=>!czyProduktAdminWKoszu(p));
  return asortymentSzkielet("oplacalnosc",`<div class="assortment-profitability-workspace">
    <section class="panel assortment-profitability-head">
      <div class="assortment-profitability-title"><span>📈</span><div><small>Asortyment • finanse produktów</small><h1>Opłacalność wszystkich kanałów</h1><p>Ceny zakupu, prowizje, koszty, cele marżowe i rekomendacje sklepu, Allegro oraz Von Halsky znajdują się przy wspólnej kartotece produktów.</p></div></div>
      <div class="assortment-profitability-metrics"><div><small>Produkty</small><b>${esc(all.length)}</b></div><div><small>Kanały</small><b>3</b></div><div><small>Zapis</small><b>PostgreSQL</b></div></div>
    </section>
    ${rentownoscKanalowaPanelHTML()}
  </div>`);
}
...
function adminPozycjeZamowieniaHTML(z){
  ...
  if(dane.length){
    return `<div class="order-items-pro">${dane.map(p=>{
      ...
      return `<div class="order-item-pro">
        <div><b>${esc(nazwa)}</b>${p.wariant?`<small>Wariant: ${esc(p.wariant)}</small>`:""}${p.sku?`<small>SKU: ${esc(p.sku)}</small>`:""}${adminLokalizacjaPozycjiZamowieniaHTML(p,z)}</div>
  ...
  return `<div class="order-items-pro">${tekstowe.length?tekstowe.map(p=>`<div class="order-item-pro"><div><b>${esc(p)}</b>${adminLokalizacjaPozycjiZamowieniaHTML({nazwa:p},z)}</div></div>`).join(""):`<div class="order-item-pro"><div><b>Brak pozycji w zamówieniu</b></div></div>`}</div>`;
}
...
function widokAdminZamowienia(){
  ...
  const widoczne=zam.slice(0,limitWidokuZamowienSklepu),aktywneFiltry=[szukajZamowien,filtrZamowien!=="wszystkie",filtrEtapuZamowienSklepu!=="wszystkie",filtrOkresuZamowienSklepu!=="wszystkie",filtrDostawyZamowienSklepu!=="wszystkie",sortZamowienSklepu!=="najnowsze"].filter(Boolean).length;
  const istniejace=new Set(wszystkie.map(z=>String(z.nr))),zaznaczone=[...zaznaczoneZamowieniaSklepu].filter(id=>istniejace.has(id));
  const etapy=wszystkie.map(adminEtapRealizacjiZamowieniaSklepu),doObslugi=wszystkie.filter(adminZamowienieSklepuAktywne).length;
  const doNadania=etapy.filter(etap=>etap==="do_nadania").length,wDrodze=etapy.filter(etap=>etap==="w_transporcie").length,zakonczone=etapy.filter(etap=>etap==="zrealizowane").length;
  return adminSzkielet("/admin/zamowienia", `
  ${adminZamowieniaSubnavHTML("lista")}
  ${adminKanalyZamowienHTML("sklep")}
  <div class="panel orders-page channel-orders-page channel-orders-store">
    ${adminCentrumZamowienHTML({kanal:"sklep",ikona:"🏪",etykieta:"Centrum realizacji • sklep",tytul:"Zamówienia i kontakt z klientem",opis:"Jedna kolejka od płatności i rezerwacji produktu, przez kompletację, po etykietę, wiadomość i doręczenie.",className:"orders-hero",metryki:[{icon:"!",value:doObslugi,label:"Wymaga działania",note:"sprawdź teraz",tone:doObslugi?"danger":"ready",onclick:"filtrEtapuZamowienSklepu='do_obslugi';renderuj()"},{icon:"🏷",value:doNadania,label:"Do nadania",note:"bez etykiety",tone:doNadania?"attention":"ready",onclick:"filtrEtapuZamowienSklepu='do_nadania';renderuj()"},{icon:"🚚",value:wDrodze,label:"W drodze",note:"tracking InPost",tone:"transit",onclick:"filtrEtapuZamowienSklepu='w_transporcie';renderuj()"},{icon:"✓",value:zakonczone,label:"Zakończone",note:"dostarczone",tone:"done",onclick:"filtrEtapuZamowienSklepu='zrealizowane';renderuj()"}],akcje:adminAkcjeCentrumZamowienHTML({source:"store-orders-manual",syncAction:"synchronizujBazeCentralna(true)"})})}
    ${adminEtapyZamowienSklepuHTML(wszystkie)}
    ${adminWyszukiwaniePanelHTML({id:"store-orders",title:"Wyszukiwanie i filtry",description:"ID, klient, dane kontaktowe, produkt, tracking, etap i okres.",results:zam.length,active:!!aktywneFiltry,open:true,fields:`<div class="von-halsky-order-filters channel-orders-filter-fields admin-search-full">
      <label class="von-halsky-order-search"><span>Szukaj zamówienia</span><input placeholder="ID, klient, e-mail, telefon, produkt lub tracking…" value="${esc(szukajZamowien)}" oninput="szukajZamowien=this.value.toLowerCase();zaplanujRenderPoWpisaniu()"></label>
      <label><span>Etap realizacji</span><select onchange="filtrEtapuZamowienSklepu=this.value;renderuj()">${[["wszystkie","Wszystkie etapy"],["nowe","Nowe"],["do_obslugi","Do obsługi"],["do_decyzji","Do decyzji"],["do_nadania","Do nadania"],["nadane","Nadane"],["w_transporcie","W transporcie"],["zrealizowane","Zrealizowane"],["anulowane","Anulowane / zwrócone"]].map(([v,l])=>`<option value="${v}" ${filtrEtapuZamowienSklepu===v?"selected":""}>${l}</option>`).join("")}</select></label>
      <label><span>Status kanału</span><select onchange="filtrZamowien=this.value;renderuj()"><option value="wszystkie" ${filtrZamowien==="wszystkie"?"selected":""}>Wszystkie statusy</option><option value="wymaga_decyzji" ${filtrZamowien==="wymaga_decyzji"?"selected":""}>Wymaga decyzji</option>${STATUSY.map(s=>`<option value="${esc(s)}" ${s===filtrZamowien?"selected":""}>${esc(s)}</option>`).join("")}</select></label>
      <label><span>Okres</span><select onchange="filtrOkresuZamowienSklepu=this.value;renderuj()">${[["wszystkie","Cały okres"],["dzisiaj","Dzisiaj"],["7","Ostatnie 7 dni"],["30","Ostatnie 30 dni"],["90","Ostatnie 90 dni"]].map(([v,l])=>`<option value="${v}" ${filtrOkresuZamowienSklepu===v?"selected":""}>${l}</option>`).join("")}</select></label>
      <label><span>Sposób doręczenia</span><select onchange="filtrDostawyZamowienSklepu=this.value;renderuj()">${[["wszystkie","Każda dostawa"],["paczkomat","Paczkomat"],["kurier","Kurier"],["punkt","PaczkoPunkt"]].map(([v,l])=>`<option value="${v}" ${filtrDostawyZamowienSklepu===v?"selected":""}>${l}</option>`).join("")}</select></label>
      <label><span>Sortowanie</span><select onchange="sortZamowienSklepu=this.value;renderuj()">${[["najnowsze","Najnowsza aktualizacja"],["najstarsze","Najstarsze najpierw"],["wartosc_desc","Najwyższa wartość"],["wartosc_asc","Najniższa wartość"]].map(([v,l])=>`<option value="${v}" ${sortZamowienSklepu===v?"selected":""}>${l}</option>`).join("")}</select></label>
      <button class="btn ghost" type="button" ${aktywneFiltry?"":"disabled"} onclick="adminWyczyscFiltryZamowienSklepu()">Wyczyść filtry (${aktywneFiltry})</button>
    </div>`})}
    ${adminNaglowekListyZamowienHTML({id:"store-orders",title:"zamówień",total:zam.length,from:zam.length?1:0,to:widoczne.length,selected:zaznaczone.length,selectPage:"adminZaznaczWidoczneZamowienia(true)",clear:"adminWyczyscZaznaczenieZamowien()",exportAll:"adminEksportujZamowieniaZakres('filtr')",limit:limitWidokuZamowienSklepu,limits:[10,25,50,100],onLimit:"limitWidokuZamowienSklepu=Number(this.value)||25;renderuj()"})}
    ...
    <div class="orders-list channel-orders-register">
      ${widoczne.length ? widoczne.map(kartaAdminZamowieniaHTML).join("") : `<div class="order-empty"><b>Brak zamówień dla tego widoku.</b><br>Zmień filtr albo wyczyść wyszukiwarkę.</div>`}
    </div>
  </div>`);
}
      ...
      <div class="shipment-manager-box inpost-like-box">
        ...
        <section class="shipment-manager-card">
          ...
          <div class="shipment-manager-grid">
            ...
            <div class="shipment-manager-field span-2"><label>Sposób nadania</label><div class="shipment-sending-methods">
              ${Object.entries(INPOST_SP_NADANIA).map(([id,nazwa])=>`<label class="shipment-sending-method"><input type="radio" name="sposobNadania" value="${esc(id)}" ${sposobNadania===id?"checked":""}><span><b>${esc(nazwa)}</b><small>${id==="parcel_locker"?"Wskaż automat nadawczy":id==="dispatch_order"?"Odbiór z adresu Artway‑TM":"Nadaj w obsługiwanym punkcie"}</small></span></label>`).join("")}
            </div></div>
