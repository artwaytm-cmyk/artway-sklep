...
function allegroPublikacjaMetaProduktu(p={}){
  ...
  return {
    ...
    inactive,needsUpdate,
    actionable:noOffer||unresolved||inactive||needsUpdate,
    ready:!missing.length,
    readyNew:noOffer&&!missing.length,
    missingNew:noOffer&&!!missing.length,
  };
}
...
function allegroPublikacjaWystawGotowe(ids=[]){
  const ready=ids.map(String).filter(id=>{const p=asortymentProduktPoId(id),meta=p?allegroPublikacjaMetaProduktu(p):null;return p&&produktDostepnyWSprzedazy(p)&&meta?.ready&&!meta.active&&!meta.unresolved;});
...
function allegroPublikacjaKartaHTML(p={}){
  const meta=allegroPublikacjaMetaProduktu(p),offer=meta.offer,missing=meta.missing,assessment=meta.unresolved?{code:"verify",label:"Zweryfikuj powiązanie",detail:`zapisane ID ${meta.offerId} nie jest jeszcze w rejestrze ofert`,score:35}:allegroPublikacjaOcena(p,offer,missing),action=allegroPublikacjaTrybProduktu(p,offer),selected=zaznaczoneAllegroProduktyKatalogu.has(String(p.id)),image=p.zdjecie||(p.zdjecia||[])[0]||"",status=meta.status;
  return `<article class="allegro-publication-card ${selected?"selected":""} ${assessment.code}" data-allegro-listing-product="${esc(p.id)}"><label class="allegro-publication-check"><input type="checkbox" ${selected?"checked":""} onchange="allegroPrzelaczProduktKatalogu(${jsArg(p.id)},this.checked)"><span>Zaznacz</span></label><div class="allegro-publication-product">${image?`<img src="${esc(image)}" alt="" loading="lazy">`:`<span class="empty-image">📦</span>`}<div><small>${esc(p.producent||p.marka||"Producent nieuzupełniony")}</small><h3>${esc(p.nazwa||"Produkt bez nazwy")}</h3><p>ID ${esc(p.id)}${p.sku?` • SKU ${esc(p.sku)}`:""}</p><div class="allegro-publication-codes"><code>EAN ${esc(p.gtin||p.ean||"—")}</code><code>MPN ${esc(p.kodProducenta||p.mpn||p.externalId||"—")}</code></div></div></div><div class="allegro-publication-readiness"><div><span class="${assessment.code}">${assessment.code==="missing"||assessment.code==="verify"?"⚠️":assessment.code==="synced"?"✓":"●"} ${esc(assessment.label)}</span><b>${assessment.score}%</b></div><progress max="100" value="${assessment.score}"></progress><small>${esc(assessment.detail)}</small></div><div class="allegro-publication-data"><span><small>Cena sklepu</small><b>${zl(p.cena)}</b></span><span><small>Cena Allegro</small><b>${zl(p.cenaAllegro||p.cena)}</b></span><span><small>Kategoria</small><b>${esc(p.allegroCategoryId||"do dobrania")}</b></span><span><small>Oferta</small><b class="${status==="ACTIVE"?"active":""}">${meta.unresolved?`weryfikacja • ${esc(meta.offerId)}`:offer?`${esc(status||"INACTIVE")} • ${esc(offer.id)}`:"nowa"}</b></span></div><div class="allegro-publication-actions">${meta.unresolved?`<button class="btn product-allegro-publish" onclick="allegroWczytajDane(true).then(()=>renderuj())">↻ Zweryfikuj ofertę</button>`:`<button class="btn product-allegro-publish" onclick="allegroPublikacjaOtworzDecyzje(${jsArg(p.id)},'${action.operation}')">${action.icon} ${esc(action.label)}</button>`}<button class="btn ghost" onclick="allegroPublikacjaPrzygotujWybrane(${jsArg(p.id)})">🤖 Przygotuj dane</button><a class="btn ghost" href="#/admin/produkty/edytuj/${encodeURIComponent(p.id)}">✏️ Edytuj</a>${offer?`<a class="btn ghost" href="https://allegro.pl/oferta/${encodeURIComponent(offer.id)}" target="_blank" rel="noopener">↗ Otwórz</a>`:""}<small>${meta.unresolved?"Najpierw potwierdź zapisane powiązanie; tworzenie duplikatu jest zablokowane":`${esc(action.note)} • przed wysłaniem działa ponowna kontrola`}</small></div></article>`;
}
...
function allegroPublikacjaFiltryAktywne(){
  const f=allegroWystawianieFiltry||{};return [filtrAllegroWystawiania!=="bez_oferty",String(szukajAllegroWystawiania||"").trim(),f.kategoria!=="wszystkie",f.producent!=="wszyscy",f.dane!=="wszystkie",f.magazyn!=="wszystkie",f.zrodlo!=="wszystkie",f.cenaOd,f.cenaDo].filter(Boolean).length;
}
...
function allegroPublikacjaPasujeDoStanu(p={}){
  const meta=allegroPublikacjaMetaProduktu(p),filter=filtrAllegroWystawiania;
  ...
  if(["nieaktywne","szkice"].includes(filter)&&!meta.inactive)return false;
  if(filter==="do_weryfikacji"&&!meta.unresolved)return false;
  if(filter==="do_aktualizacji"&&!meta.needsUpdate)return false;
  return true;
}
...
allegroWystawianiePanelHTML=function(){
  if(!["bez_oferty","gotowe_nowe","braki_nowe","do_aktualizacji","nieaktywne","do_weryfikacji","do_dzialania"].includes(filtrAllegroWystawiania))filtrAllegroWystawiania="bez_oferty";
  const query=String(szukajAllegroWystawiania||"").toLowerCase().trim(),saleProducts=produktyDoAdministracji().filter(p=>!czyProduktAdminWKoszu(p)&&produktDostepnyWSprzedazy(p)),all=saleProducts.filter(p=>allegroPublikacjaMetaProduktu(p).actionable),counts={wszystkie:all.length,bez_oferty:0,gotowe_nowe:0,braki_nowe:0,nieaktywne:0,do_weryfikacji:0,do_aktualizacji:0};
  all.forEach(p=>{const meta=allegroPublikacjaMetaProduktu(p);if(meta.noOffer)counts.bez_oferty++;if(meta.readyNew)counts.gotowe_nowe++;if(meta.missingNew)counts.braki_nowe++;if(meta.inactive)counts.nieaktywne++;if(meta.unresolved)counts.do_weryfikacji++;if(meta.needsUpdate)counts.do_aktualizacji++;});
  counts.brak=counts.bez_oferty;counts.gotowe=counts.gotowe_nowe;counts.braki=counts.braki_nowe;counts.szkice=counts.nieaktywne;
  ...
  const priority=p=>{const meta=allegroPublikacjaMetaProduktu(p);if(meta.readyNew)return 0;if(meta.missingNew)return 1;if(meta.needsUpdate)return 2;if(meta.inactive&&meta.ready)return 3;if(meta.unresolved)return 4;return 5;};
  ...
  return `<div class="allegro-listing-workspace"><section class="panel allegro-listing-hero"><div><span class="order-pro-label">KOLEJKA PUBLIKACJI • API ALLEGRO</span><h2>🟠 Wystawianie i aktualizacje</h2><p>To nie jest pełny katalog ofert. Widzisz tu wyłącznie produkty bez oferty albo wymagające działania. Aktualne, poprawne oferty są automatycznie pomijane.</p></div><div class="allegro-listing-hero-actions"><button class="btn product-allegro-publish" onclick="allegroPublikacjaOtworzDecyzje(null,'activate')" ${selected?"":"disabled"}>🟠 Wystaw lub aktualizuj (${selected})</button><button class="btn ghost" onclick='allegroPublikacjaWystawGotowe(${JSON.stringify(readyVisible.map(p=>String(p.id)))})' ${readyVisible.length?"":"disabled"}>🚀 Publikuj gotowe z widoku (${readyVisible.length})</button><a class="btn ghost" href="#/admin/produkty/dodaj">＋ Dodaj produkt</a><a class="btn ghost" href="#/admin/allegro/ustawienia">⚙️ Ustawienia</a></div></section>${!allegroStan.connected?`<section class="allegro-permission-alert"><div><b>Połączenie Allegro wymaga kontroli</b><p>System sprawdzi dostęp przed operacją. Bez ważnej autoryzacji żadna oferta nie zostanie utworzona.</p></div><button class="btn" onclick="allegroPolacz()">🔐 Połącz Allegro</button></section>`:""}<section class="allegro-listing-metrics">${[["bez_oferty","＋","Bez oferty",counts.bez_oferty],["gotowe_nowe","✓","Gotowe nowe",counts.gotowe_nowe],["braki_nowe","⚠","Nowe z brakami",counts.braki_nowe],["do_aktualizacji","↻","Do aktualizacji",counts.do_aktualizacji],["nieaktywne","○","Nieaktywne",counts.nieaktywne],["do_weryfikacji","⌕","Zweryfikuj ID",counts.do_weryfikacji]].map(([id,icon,label,value])=>`<button class="${filtrAllegroWystawiania===id?"active":""}" onclick="allegroPublikacjaPrzelaczFiltr('${id}')"><span>${icon}</span><b>${value}</b><small>${label}</small></button>`).join("")}</section>${adminWyszukiwaniePanelHTML({id:"allegro-listing",description:"Kolejka pracy obejmuje tylko nowe produkty, braki, aktualizacje, nieaktywne oferty i powiązania wymagające kontroli.",results:filtered.length,active:activeFilters>0,open:true,fields,actions:operations})}<section data-allegro-publication-center>${allegroPublikacjaCentrumOperacjiHTML()}</section><section class="panel allegro-listing-catalog"><div class="allegro-listing-results-head"><div><b>${filtered.length} produktów do działania</b><small>Pokazano ${rows.length} • strona ${allegroWystawianieStrona} z ${pages} • filtr: ${esc(filtrAllegroWystawiania)}</small></div><span>${selected} zaznaczonych</span></div><div class="allegro-publication-list">${cards}</div>${pages>1?`<nav class="allegro-listing-pagination"><button class="btn ghost" onclick="allegroPublikacjaUstawStrone(${allegroWystawianieStrona-1})" ${allegroWystawianieStrona===1?"disabled":""}>← Poprzednia</button><span>Strona <b>${allegroWystawianieStrona}</b> z <b>${pages}</b></span><button class="btn ghost" onclick="allegroPublikacjaUstawStrone(${allegroWystawianieStrona+1})" ${allegroWystawianieStrona===pages?"disabled":""}>Następna →</button></nav>`:""}</section>${allegroOstatniBladWystawienia?`<section class="allegro-permission-alert"><div><b>⚠️ Ostatnia operacja wymaga uwagi</b><p>${esc(allegroOstatniBladWystawienia.message||"Błąd Allegro")}</p></div><button class="btn ghost" onclick="allegroOstatniBladWystawienia=null;renderuj()">Zamknij</button></section>`:""}${allegroWynikOperacjiHTML()}${allegroZadaniaAgentaOfertHTML()}</div>`;
};
