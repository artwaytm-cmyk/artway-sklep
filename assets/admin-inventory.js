...
function asortymentKartaProduktuHTML(p={},ukrytaKopia=false){
  ...
  const braki=asortymentBrakiDanych(p),score=Math.max(0,Math.round((8-Math.min(8,braki.length))/8*100)),sourceUrl=String(p.sourceUrl||p.producentUrl||p.urlProducenta||"").trim(),image=p.zdjecie||(Array.isArray(p.zdjecia)?p.zdjecia[0]:"")||"";
  ...
  const state=ukryty?{code:"deleted",icon:"🗑️",label:"Produkt w koszu"}:braki.length?{code:"missing",icon:"⚠️",label:"Wymaga uzupełnienia"}:{code:"ready",icon:"✓",label:"Dane kompletne"};
  ...
  return `<article class="allegro-publication-card catalog-product-card ${selected?"selected is-selected":""} ${state.code}" data-assortment-product-card data-assortment-product-key="${esc(p.id)}">
    ...
    <div class="allegro-publication-product catalog-product-identity">${image?`<img src="${esc(image)}" alt="" loading="lazy" onerror="this.style.display='none';this.nextElementSibling.style.display='grid'"><span class="empty-image" style="display:none">${esc(p.ikona||"📦")}</span>`:`<span class="empty-image">${esc(p.ikona||"📦")}</span>`}<div><small>${esc(p.producent||p.marka||"Producent nieuzupełniony")}</small><h3>${esc(p.nazwa||"Produkt bez nazwy")}</h3><p>ID ${esc(p.id)}${dodany?" • dodany":""}${edytowany?" • edytowany":""}${ukryty?" • w koszu":""}${promocja?` • promocja −${promocja}%`:""}</p><div class="allegro-publication-codes"><code>EXTERNAL_ID ${esc(p.externalId||"—")}</code><code>SKU ${esc(p.sku||"—")}</code><code>EAN ${esc(p.gtin||p.ean||"—")}</code></div></div></div>
    <div class="allegro-publication-readiness catalog-product-readiness"><div><span class="${state.code}">${state.icon} ${esc(state.label)}</span><b>${score}%</b></div><progress max="100" value="${score}"></progress><small>${braki.length?`Braki: ${esc(braki.slice(0,4).join(", "))}${braki.length>4?` +${braki.length-4}`:""}`:`${esc(p.kategoria||"Bez kategorii")} • kartoteka gotowa`}</small><div class="catalog-product-source"><b>${esc(p.kategoria||"Bez kategorii")}</b>${sourceUrl?`<a href="${esc(sourceUrl)}" target="_blank" rel="noopener">Źródło produktu ↗</a>`:`<span>Brak linku źródłowego</span>`}${ukrytaKopia?`<em>ukryta kopia katalogowa</em>`:""}</div></div>
    <div class="allegro-publication-data catalog-product-operational-data"><span class="catalog-product-edit-value"><small>Cena sklepu</small><label><input value="${String(cena.toFixed(2)).replace(".",",")}" inputmode="decimal" aria-label="Cena sklepu: ${esc(p.nazwa||"produkt")}" onchange="ustawCene(${jsArg(p.id)},this.value)"><em>zł</em></label></span><span><small>Cena Allegro</small><b>${cenaAllegro>0?zl(cenaAllegro):"—"}</b></span><span><small>Zakup — tylko administrator</small><b>${Number(p.cenaZakupu)>0?zl(p.cenaZakupu):"brak"}</b></span><span class="catalog-product-edit-value"><small>Stan magazynowy</small><label><input value="${stan??""}" placeholder="∞" inputmode="numeric" aria-label="Stan magazynowy: ${esc(p.nazwa||"produkt")}" onchange="ustawStan(${jsArg(p.id)},this.value)" class="${Number(stan)===0?"empty":""}"><em>${stan===undefined||stan===""?"bez limitu":"szt."}</em></label></span></div>
    <div class="allegro-publication-actions catalog-product-actions"><div class="catalog-product-channel-summary">${allegroStatusProduktuHTML(p)}</div><div class="catalog-product-sale-status">${dostepnoscBadgeAdmin(p)}</div><button class="btn ghost catalog-product-availability-action" onclick="przelaczDostepnoscProduktu(${jsArg(p.id)})">${availabilityLabel}</button><div class="catalog-product-buttons"><a class="btn" href="#/admin/produkty/edytuj/${encodeURIComponent(p.id)}" title="Edytuj produkt">✏️ Edytuj</a>${asortymentMenuDzialanProduktuHTML(p)}<button class="btn ghost" onclick="duplikujProdukt(${jsArg(p.id)})" title="Duplikuj produkt">📄 Kopia</button>${ukryty?`<button class="btn ghost" onclick="przywrocProdukt(${jsArg(p.id)})" title="Przywróć">↩️ Przywróć</button>`:`<button class="btn danger" onclick="if(confirm('Przenieść produkt do kosza?')) usunProduktAdmin(${jsArg(p.id)})" title="Przenieś do kosza">🗑️ Kosz</button>`}</div><small>Sklep jest źródłem danych • zmiany zapisują się we wspólnej bazie</small></div>
  </article>`;
}
