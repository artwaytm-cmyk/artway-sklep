...
function widokAdminProdukty(){
  ...
  return asortymentSzkielet("produkty", `
    <div class="assortment-catalog-workspace">
      ...
      <section class="panel assortment-catalog-page admin-pattern-surface allegro-listing-catalog catalog-product-table-center">
      ...
      <div class="allegro-publication-list assortment-product-list density-${gestoscAdminProduktow}">
        ${fragment.map(p=>{
          ...
          const gotowosc=Math.max(15,100-Math.min(85,braki.length*15));
          return `<article class="allegro-publication-card assortment-product-card ${braki.length?"missing":"ready"} ${ukryty?"is-deleted":""} ${zaznaczoneProdukty.has(p.id)?"selected":""}" data-assortment-product="${esc(p.id)}">
          <label class="allegro-publication-check"><input class="assortment-check" type="checkbox" aria-label="Zaznacz ${esc(p.nazwa||"produkt")}" data-assortment-product-id="${esc(p.id)}" ${zaznaczoneProdukty.has(p.id)?"checked":""} onchange="przelaczZaznProd(${jsArg(p.id)})"><span>Zaznacz</span></label>
          <div class="allegro-publication-product assortment-product-cell"><span class="admin-product-thumb">${p.zdjecie?`<img src="${esc(p.zdjecie)}" alt="${esc(p.nazwa)}" loading="lazy" onerror="this.style.display='none';this.nextElementSibling.style.display='grid'"><span class="admin-product-thumb-fallback" style="display:none">${esc(p.ikona||"📦")}</span>`:`<span class="admin-product-thumb-fallback">${esc(p.ikona||"📦")}</span>`}</span><div><small>${esc(p.producent||p.marka||"Producent nieuzupełniony")}</small><h3>${esc(p.nazwa||"Produkt bez nazwy")}</h3><p>ID ${esc(p.id)}${dodany?" • dodany":""}${edytowany?" • edytowany":""}${ukryty?" • w koszu":""}</p><div class="allegro-publication-codes"><code>EXTERNAL_ID ${esc(p.externalId||"—")}</code><code>SKU ${esc(p.sku||"—")}</code><code>EAN ${esc(p.gtin||p.ean||"—")}</code></div></div></div>
          <div class="allegro-publication-readiness assortment-card-readiness"><div><span class="${braki.length?"missing":"ready"}">${braki.length?"⚠️ Wymaga uzupełnienia":"✓ Dane kompletne"}</span><b>${gotowosc}%</b></div><progress max="100" value="${gotowosc}"></progress><small>${braki.length?`Braki: ${braki.slice(0,4).map(esc).join(", ")}${braki.length>4?` +${braki.length-4}`:""}`:"Kartoteka gotowa do obsługi obu kanałów."}</small><div class="assortment-card-classification"><b>${esc(p.kategoria||"Bez kategorii")}</b>${sourceUrl?`<a href="${esc(sourceUrl)}" target="_blank" rel="noopener">Źródło produktu ↗</a>`:`<span>Brak linku źródłowego</span>`}${audytSklep.hiddenIds.has(String(p.id))?`<em>Ukryta kopia produktu</em>`:""}</div></div>
          <div class="allegro-publication-data assortment-card-commerce"><span class="assortment-card-price"><small>Cena sklepu</small><label><input value="${String(cena.toFixed(2)).replace(".",",")}" inputmode="decimal" aria-label="Cena sklepu: ${esc(p.nazwa||"produkt")}" onchange="ustawCene(${jsArg(p.id)},this.value)"><i>zł</i></label></span><span><small>Zakup (admin) • prywatne</small><b>${Number(p.cenaZakupu)>0?zl(p.cenaZakupu):"brak"}</b></span><span class="assortment-card-stock"><small>Stan lokalny</small><label><input value="${stanyProduktow[p.id]??""}" placeholder="∞" inputmode="numeric" aria-label="Stan magazynowy: ${esc(p.nazwa||"produkt")}" onchange="ustawStan(${jsArg(p.id)},this.value)" class="${Number(stanyProduktow[p.id])===0?"empty":""}"><i>szt.</i></label></span><span><small>Sprzedaż</small><b>${dostepnoscBadgeAdmin(p)}</b></span><div class="assortment-card-allegro"><small>Allegro</small>${allegroStatusProduktuHTML(p)}</div>${promocja?`<div class="assortment-card-promotion"><span class="lvl lvl-blad">Promocja −${promocja}%</span></div>`:""}</div>
          <div class="allegro-publication-actions assortment-row-actions">
            <a class="btn" href="#/admin/produkty/edytuj/${encodeURIComponent(p.id)}" title="Edytuj produkt">✏️ Edytuj</a>
            ${asortymentMenuDzialanProduktuHTML(p)}
            <button class="btn ghost" onclick="przelaczDostepnoscProduktu(${jsArg(p.id)})">${produktAutomatycznieNiedostepny(p)?"Sprawdź oba kanały":produktOznaczonyNiedostepny(p)?"Wznów sklep + Allegro":"Wstrzymaj sklep + Allegro"}</button>
            ...
            <small>Kartoteka sklepu jest źródłem danych dla magazynu i Allegro.</small>
          </div>
        </article>`;}).join("")}
        ${fragment.length?"":`<div class="allegro-listing-empty assortment-empty"><span>⌕</span><b>Brak produktów pasujących do filtrów</b><small>Usuń wybrane kryteria albo wróć do widoku aktywnych produktów.</small><button class="btn ghost" onclick="asortymentResetujFiltry()">Pokaż aktywne produkty</button></div>`}
      </div>
