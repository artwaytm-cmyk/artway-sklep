...
function daneProduktuZFormularza(f, id, poprzedni={}){
  ...
  const allegroSelected=!!(p.allegroOfferId||p.allegroProductId||p.allegroCategoryId||p.allegroPublicationIntent===true||["queued","preparing","ready","published"].includes(String(p.allegroPreparationStatus||"").toLowerCase())),targetChannels=allegroSelected?"shared_store_and_allegro":"store_only";
  if(p.contentEditorial?.channels&&p.contentEditorial.channels!==targetChannels)p.contentEditorial={...p.contentEditorial,status:"queued",channels:targetChannels,targets:{store:true,allegro:allegroSelected},queuedReason:"channel_selected",queuedAt:new Date().toISOString()};
