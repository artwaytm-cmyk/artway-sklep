...
function widokAdminProdukty(){
  ...
  return asortymentSzkielet("produkty", `
    <div class="assortment-catalog-workspace">
      ...
      <section class="panel assortment-catalog-page admin-pattern-surface">
      ...
      <div class="assortment-table-wrap"><table class="log-table admin-responsive-table assortment-product-table density-${gestoscAdminProduktow}">
        ...
        ${fragment.map(p=>{
          ...
          <td data-label=""><input class="assortment-check" type="checkbox" aria-label="Zaznacz ${esc(p.nazwa||"produkt")}" data-assortment-product-id="${esc(p.id)}" ${zaznaczoneProdukty.has(p.id)?"checked":""} onchange="przelaczZaznProd(${jsArg(p.id)})"></td>
          <td data-label="Produkt"><div class="assortment-product-cell"><span class="admin-product-thumb">${p.zdjecie?`<img src="${esc(p.zdjecie)}" alt="${esc(p.nazwa)}" loading="lazy" onerror="this.style.display='none';this.nextElementSibling.style.display='grid'"><span class="admin-product-thumb-fallback" style="display:none">${esc(p.ikona||"📦")}</span>`:`<span class="admin-product-thumb-fallback">${esc(p.ikona||"📦")}</span>`}</span><div><b>${esc(p.nazwa||"Produkt bez nazwy")}</b><small>ID ${esc(p.id)}${dodany?" • dodany":""}${edytowany?" • edytowany":""}${ukryty?" • w koszu":""}</small><div class="assortment-data-status">${braki.length?`<span class="missing">⚠️ Braki: ${braki.slice(0,3).map(esc).join(", ")}${braki.length>3?` +${braki.length-3}`:""}</span>`:`<span class="complete">✅ Dane kompletne</span>`}${audytSklep.hiddenIds.has(String(p.id))?`<span class="duplicate">ukryta kopia</span>`:""}</div></div></div></td>
          <td data-label="Identyfikatory"><div class="assortment-identifiers"><span><small>EXTERNAL_ID</small><b>${esc(p.externalId||"—")}</b></span><span><small>SKU</small><b>${esc(p.sku||"—")}</b></span><span><small>EAN</small><b>${esc(p.gtin||p.ean||"—")}</b></span></div></td>
          <td data-label="Klasyfikacja i źródło"><div class="assortment-classification"><b>${esc(p.producent||p.marka||"Bez producenta")}</b><span>${esc(p.kategoria||"Bez kategorii")}</span>${sourceUrl?`<a href="${esc(sourceUrl)}" target="_blank" rel="noopener">Źródło produktu ↗</a>`:`<small>Brak linku źródłowego</small>`}</div></td>
          <td data-label="Ceny"><div class="assortment-price-cell"><label><input value="${String(cena.toFixed(2)).replace(".",",")}" inputmode="decimal" aria-label="Cena sklepu: ${esc(p.nazwa||"produkt")}" onchange="ustawCene(${jsArg(p.id)},this.value)"><span>zł</span></label>${promocja?`<span class="lvl lvl-blad">Promocja −${promocja}%</span>`:`<small>Cena regularna</small>`}<small class="private-cost">Zakup (admin): <b>${Number(p.cenaZakupu)>0?zl(p.cenaZakupu):"brak"}</b></small></div></td>
          <td data-label="Magazyn i sprzedaż"><div class="assortment-stock-cell"><label><span>Stan</span><input value="${stanyProduktow[p.id]??""}" placeholder="∞" inputmode="numeric" aria-label="Stan magazynowy: ${esc(p.nazwa||"produkt")}" onchange="ustawStan(${jsArg(p.id)},this.value)" class="${Number(stanyProduktow[p.id])===0?"empty":""}"></label>${dostepnoscBadgeAdmin(p)}<button class="btn ghost" onclick="przelaczDostepnoscProduktu(${jsArg(p.id)})">${produktAutomatycznieNiedostepny(p)?"Sprawdź oba kanały":produktOznaczonyNiedostepny(p)?"Wznów sklep + Allegro":"Wstrzymaj sklep + Allegro"}</button></div></td>
          <td data-label="Allegro">${allegroStatusProduktuHTML(p)}</td>
          <td data-label="Akcje"><div class="assortment-row-actions">
        ...
        ${fragment.length?"":`<tr><td data-label="" colspan="8"><div class="assortment-empty"><b>Brak produktów pasujących do filtrów</b><span>Usuń wybrane kryteria albo wróć do widoku aktywnych produktów.</span><button class="btn ghost" onclick="asortymentResetujFiltry()">Pokaż aktywne produkty</button></div></td></tr>`}
      </tbody></table></div>
