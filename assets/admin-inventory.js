...
function allegroPublikacjaPasujeDoFiltrowZaawansowanych(p={}){
  const f=allegroWystawianieFiltry||{},meta=allegroPublikacjaMetaProduktu(p),missing=meta.missing,source=String(p.sourceUrl||p.producentUrl||p.urlProducenta||"").trim(),stock=stanyProduktow[p.id],price=kwotaNum(p.cenaAllegro||p.cena),from=Number(String(f.cenaOd||"").replace(",",".")),to=Number(String(f.cenaDo||"").replace(",","."));
...
allegroWystawianiePanelHTML=function(){
  ...
  const query=String(szukajAllegroWystawiania||"").toLowerCase().trim(),catalogProducts=produktyDoAdministracji().filter(p=>!czyProduktAdminWKoszu(p)),all=catalogProducts.filter(p=>allegroPublikacjaMetaProduktu(p).actionable),counts={wszystkie:all.length,bez_oferty:0,gotowe_nowe:0,braki_nowe:0,szkice_do_aktywacji:0,zakonczone:0,wycofane_brak_towaru:0,oczekuje_na_wycofanie:0,do_weryfikacji:0,do_aktualizacji:0};
  all.forEach(p=>{const meta=allegroPublikacjaMetaProduktu(p);if(meta.noOffer&&meta.saleAvailable)counts.bez_oferty++;if(meta.readyNew)counts.gotowe_nowe++;if(meta.missingNew)counts.braki_nowe++;if(meta.draft)counts.szkice_do_aktywacji++;if(meta.ended)counts.zakonczone++;if(meta.withdrawnNoStock)counts.wycofane_brak_towaru++;if(meta.pendingStockWithdrawal)counts.oczekuje_na_wycofanie++;if(meta.unresolved)counts.do_weryfikacji++;if(meta.needsUpdate&&meta.saleAvailable&&!meta.withdrawnNoStock)counts.do_aktualizacji++;});
  ...
  const pageSize=Math.max(25,Number(allegroLimitWystawiania)||100),pages=Math.max(1,Math.ceil(filtered.length/pageSize));allegroWystawianieStrona=Math.min(Math.max(1,allegroWystawianieStrona),pages);const start=(allegroWystawianieStrona-1)*pageSize,rows=filtered.slice(start,start+pageSize),readyVisible=rows.filter(p=>{const meta=allegroPublikacjaMetaProduktu(p);return meta.selectable&&meta.ready&&!meta.active;}),readyNewFiltered=filtered.filter(p=>allegroPublikacjaMetaProduktu(p).readyNew),missingFiltered=filtered.filter(p=>{const meta=allegroPublikacjaMetaProduktu(p);return meta.selectable&&!meta.ready;}),updateFiltered=filtered.filter(p=>{const meta=allegroPublikacjaMetaProduktu(p);return meta.selectable&&meta.needsUpdate;}),selected=allegroPublikacjaWybraneIds().length,activeFilters=allegroPublikacjaFiltryAktywne(),f=allegroWystawianieFiltry;
  ...
  const fields=`<div class="allegro-listing-advanced-grid admin-search-full">
    ...
    <label><span>Na stronie</span><select onchange="allegroLimitWystawiania=Number(this.value)||100;allegroWystawianieStrona=1;renderuj()">${[25,50,100,250,500,1000].map(n=>`<option value="${n}" ${pageSize===n?"selected":""}>${n}</option>`).join("")}</select></label>
