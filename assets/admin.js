...
async function agentAITelegramAkcja(typ){
  const map={test:["telegram-test",{},"Wysyłam jedną cichą wiadomość testową…"],webhook:["telegram-register-webhook",{},"Podłączam dwustronną komunikację…"],digest:["telegram-dispatch",{forceDigest:true,source:"admin-manual-digest"},"Przygotowuję raport wyłącznie z nowych spraw…"],report:["telegram-send-agent-report",{source:"admin-telegram-center"},"Wysyłam pełny raport na żądanie…"]},def=map[typ];if(!def)return;
  agentAITelegram={...agentAITelegram,loading:true,error:""};renderuj();toast(def[2]);
  try{const d=await chmura(def[0],{method:"POST",body:def[1],timeout:45000});toast(typ==="webhook"?"✅ Bot odbiera już zwykłe pytania przez stronę":typ==="digest"&&!d.dispatch?.sent?`ℹ️ ${d.dispatch?.reason||"Brak nowych spraw do wysłania"}`:"✅ Telegram: operacja zakończona");await agentAITelegramPobierz(true,true);}
...
function agentAITelegramPanelHTML(){
  const t=agentAITelegram,s=t.settings||{enabled:true,mode:"important",automaticCritical:true,customerMessages:true,supplierAlerts:false,operationalAlerts:true,digestEnabled:true,digestTimes:["08:00","16:00"],quietStart:"21:00",quietEnd:"07:00",criticalDuringQuiet:false,onlyChanges:true,repeatOpenHours:24,cooldownMinutes:720,maxItems:8},status=t.status||{},stats=t.stats||{},events=Array.isArray(t.events)?t.events:[],history=Array.isArray(t.history)?t.history:[],times=s.digestTimes||["08:00","16:00"];
  ...
  return `<section class="telegram-center-page">
    <div class="panel telegram-control-hero"><div><span class="order-pro-label">Centrum komunikacji operacyjnej</span><h2>✈️ Telegram bez chaosu i powtórzeń</h2><p>System nie wysyła zmian priorytetów ani zwykłych informacji technicznych. Każde zdarzenie przechodzi przez ważność, zakres, ciszę nocną, deduplikację i czas ochronny.</p></div><div class="diag-actions"><button class="btn ghost" onclick="agentAITelegramPobierz(true)" ${t.loading?"disabled":""}>${t.loading?"⏳ Sprawdzam…":"↻ Sprawdź połączenie"}</button><button class="btn telegram-btn" onclick="agentAITelegramAkcja('report')">Wyślij raport na żądanie</button></div></div>
    <div class="orders-stat-grid telegram-status-grid"><div class="order-stat-card ${status.configured?"money":"hot"}"><span>${status.configured?"✅":"⚠️"}</span><b>${status.configured?"ONLINE":"BRAK"}</b><small>bot i czat docelowy</small></div><div class="order-stat-card ${status.webhook?.active?"money":""}"><span>↔️</span><b>${status.webhook?.active?"AKTYWNY":"DO PODPIĘCIA"}</b><small>pytania z Telegrama</small></div><div class="order-stat-card"><span>🎛️</span><b>${esc(modeLabel)}</b><small>bieżący tryb wysyłki</small></div><div class="order-stat-card"><span>📨</span><b>${esc(stats.sentToday||0)}</b><small>wysłanych dzisiaj</small></div><div class="order-stat-card ${t.quietNow?"hot":""}"><span>🌙</span><b>${t.quietNow?"CISZA":"AKTYWNY"}</b><small>${esc(s.quietStart)}–${esc(s.quietEnd)}</small></div></div>
    ...
    <div class="telegram-center-layout"><form class="panel telegram-policy-form" onsubmit="return agentAITelegramZapisz(event)"><div class="order-section-head"><div><span class="order-pro-label">Jedna polityka dla całej strony</span><h2>Reguły wysyłania</h2><p class="order-detail-lead">Ustawienia dotyczą Agenta, Allegro, producentów i automatycznych kontroli.</p></div><button class="btn" type="submit" ${t.saving?"disabled":""}>${t.saving?"⏳ Zapisuję…":"💾 Zapisz ustawienia"}</button></div>
      ...
      <fieldset><legend>Zakres automatyczny</legend><label><input type="checkbox" name="customerMessages" ${s.customerMessages?"checked":""}> nowe wiadomości klientów</label><label><input type="checkbox" name="operationalAlerts" ${s.operationalAlerts?"checked":""}> zamówienia i wysyłki</label><label><input type="checkbox" name="supplierAlerts" ${s.supplierAlerts?"checked":""}> zmiany u producentów</label></fieldset><fieldset><legend>Raport zbiorczy</legend><label><input type="checkbox" name="digestEnabled" ${s.digestEnabled?"checked":""}> aktywny</label><div class="telegram-time-row"><input type="time" name="digestTime1" value="${esc(times[0]||"08:00")}"><input type="time" name="digestTime2" value="${esc(times[1]||"16:00")}"></div></fieldset>
      <fieldset><legend>Cisza nocna</legend><div class="telegram-time-row"><input type="time" name="quietStart" value="${esc(s.quietStart)}"><input type="time" name="quietEnd" value="${esc(s.quietEnd)}"></div><label><input type="checkbox" name="criticalDuringQuiet" ${s.criticalDuringQuiet?"checked":""}> przepuszczaj krytyczne także nocą</label></fieldset><fieldset><legend>Ochrona przed natłokiem</legend><label>Czas ochronny (min)<input type="number" name="cooldownMinutes" min="15" max="10080" value="${esc(s.cooldownMinutes)}"></label><label>Przypomnij otwartą sprawę po (h)<input type="number" name="repeatOpenHours" min="1" max="168" value="${esc(s.repeatOpenHours)}"></label><label>Maks. pozycji w raporcie<input type="number" name="maxItems" min="3" max="20" value="${esc(s.maxItems)}"></label></fieldset></div></form>
      <aside class="panel telegram-actions-panel"><div class="order-section-head"><div><span class="order-pro-label">Połączenie i test</span><h2>Bot zespołu</h2></div></div><div class="telegram-connection-list"><div><span>🤖</span><b>${esc(status.bot?.name||"Bot Telegram")}</b><small>${status.bot?.username?`@${esc(status.bot.username)}`:"sprawdź połączenie API"}</small></div><div><span>↔️</span><b>Dwustronna komunikacja</b><small>${status.webhook?.active?"Strona odbiera zwykłe pytania z autoryzowanej grupy.":"Podłącz webhook, aby bot odpowiadał także bez działającego komputera."}</small></div><div><span>🛡️</span><b>Bezpieczne działania</b><small>Bot odczytuje stan. Wysyłka do klientów, producenta, faktura i etykieta wymagają panelu.</small></div></div><div class="telegram-action-stack"><button class="btn" onclick="agentAITelegramAkcja('webhook')">↔️ ${status.webhook?.active?"Odśwież webhook":"Podłącz odbieranie pytań"}</button><button class="btn ghost" onclick="agentAITelegramAkcja('test')">✅ Wyślij cichy test</button><button class="btn ghost" onclick="agentAITelegramAkcja('digest')">📋 Wyślij tylko nowe sprawy</button></div></aside></div>
    <div class="telegram-operations-grid"><section class="panel"><div class="order-section-head"><div><span class="order-pro-label">Podgląd przed wysłaniem</span><h2>Aktywne ważne sprawy (${events.length})</h2></div></div><div class="telegram-event-list">${events.map(x=>`<article class="${esc(x.severity)}"><span>${x.severity==="critical"?"🔴":"🟡"}</span><div><b>${esc(x.title)}</b><small>${esc(x.description||"")}</small></div><strong>${esc(x.count)}</strong></article>`).join("")||`<div class="agent-ops-empty">✅ Brak ważnych spraw do komunikacji.</div>`}</div></section><section class="panel"><div class="order-section-head"><div><span class="order-pro-label">Wiadomość ręczna</span><h2>Notatka do zespołu</h2><p class="order-detail-lead">Wysyłana tylko po kliknięciu. Nie jest komunikatem do klienta.</p></div></div><form class="telegram-note-form" onsubmit="return agentAITelegramWyslijNotatke(event)"><textarea id="agentTelegramNote" maxlength="1500" placeholder="Np. Proszę dziś najpierw przygotować zamówienia ATM-…"></textarea><label><input id="agentTelegramNoteSilent" type="checkbox" checked> bez dźwięku</label><button class="btn telegram-btn" type="submit">✈️ Wyślij do zespołu</button></form></section></div>
