...
function agentAIZleceniaPanelHTML(){
  ...
  <nav class="supplier-plan-statusbar" aria-label="Status planu"><button onclick="agentAIPlanFiltrStatusu='aktywne';odswiezPlanZatowarowaniaWidoku()"><b>${otwarteWszystkie.length}</b><small>aktywne</small></button><button onclick="agentAIPlanFiltrStatusu='zatwierdzone';odswiezPlanZatowarowaniaWidoku()"><b>${approved}</b><small>zatwierdzone</small></button><button onclick="agentAIPlanFiltrStatusu='wyslane';odswiezPlanZatowarowaniaWidoku()"><b>${sent}</b><small>w dostawie</small></button><button onclick="agentAIPlanFiltrStatusu='wyslane';odswiezPlanZatowarowaniaWidoku()"><b>${pozostalo}</b><small>szt. do przyjęcia</small></button><span><b>${new Set(otwarteWszystkie.map(agentAIDostawcaZlecenia)).size}</b><small>producenci</small></span></nav>
  ...
  return `<div class="panel allegro-section-panel allegro-integration-settings">
    ...
    <form class="allegro-settings-layout" onsubmit="event.preventDefault();allegroZapiszUstawieniaOfert(this)">
      <section class="allegro-settings-section primary">
        ...
        <div class="allegro-settings-grid compact"><label>Próg pewności<select name="mappingMinScore">${option(88,settings.mappingMinScore,"88 — pewne identyfikatory")}${option(95,settings.mappingMinScore,"95 — bardzo ścisłe")}${option(100,settings.mappingMinScore,"100 — wyłącznie idealne")}</select><small>System dodatkowo odrzuca niejednoznaczne duplikaty.</small></label><div class="allegro-setting-action"><b>Natychmiastowa kontrola</b><small>Sprawdź istniejące oferty i połącz wszystkie pewne pozycje teraz.</small><button class="btn" type="button" onclick="allegroUruchomAutomatyczneMapowanie()">🤖 Połącz pewne teraz</button></div></div>
      </section>
