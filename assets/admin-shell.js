/* GENERATED ADMIN SHELL — shared navigation and workspace */
...
const MENU_ADMINA = [
  {id:"sprzedaz",ikona:"🛒",nazwa:"Obsługa sprzedaży",opis:"Od zamówienia do doręczenia",elementy:[
    ...
    ["/admin/von-halsky","🐕","InPost Von Halsky","Wystawianie, zamówienia i integracja InPost+"],
  ...
  {id:"rozwoj",ikona:"✨",nazwa:"Rozwój sklepu",opis:"Automatyzacja, wygląd i widoczność",elementy:[
    ...
    ["/admin/konkursy","🏆","Konkursy","Kreator, zgłoszenia, ranking i zwycięzcy"],
    ["/admin/seo","📣","Pozycjonowanie","Widoczność i promocja produktów"],
    ["/admin/edytor-strony","🎨","Edytor strony","Wszystkie elementy i wersja do publikacji",{siteEditor:true}]
  ]},
...
function adminMenuPozycjaAktywna(aktywna,href){
  ...
  if(href==="/admin/personalizacja"&&String(aktywna||"").startsWith("/admin/personalizacja/wersje"))return false;
...
function adminMenuLinkHTML(pozycja,aktywna,powiadomienia,dodatkowaKlasa=""){
  const [href,ikona,nazwa,podpis,options={}]=pozycja,czyAktywna=!options.siteEditor&&adminMenuPozycjaAktywna(aktywna,href),licznik=powiadomienia[href]||0;
  return `<a href="${options.siteEditor?"#/":"#"+href}" ${options.siteEditor?'data-site-editor-launch="1"':""} class="admin-nav-link ${dodatkowaKlasa} ${czyAktywna?"active":""}" title="${esc(nazwa)}${podpis?` — ${esc(podpis)}`:""}" ${czyAktywna?'aria-current="page"':""}><span class="admin-nav-link-main"><i>${ikona}</i><span><b>${esc(nazwa)}</b>${podpis?`<small>${esc(podpis)}</small>`:""}</span></span>${licznik?`<span class="nav-badge" aria-label="${licznik} aktywnych spraw">${licznik}</span>`:""}</a>`;
}
...
function adminMenuUstawPowiadomienie(href,value,label="aktywnych spraw"){
  const count=Math.max(0,Number(value)||0),key=String(href||"");
  if(adminMenuStatCache.powiadomienia)adminMenuStatCache.powiadomienia={...adminMenuStatCache.powiadomienia,[key]:count};
  document.querySelectorAll(`a.admin-nav-link[href="#${key}"]`).forEach(link=>{
    let badge=link.querySelector(".nav-badge");
    if(!count){badge?.remove();return;}
    if(!badge){badge=document.createElement("span");badge.className="nav-badge";link.append(badge);}
    badge.textContent=String(count);badge.setAttribute("aria-label",`${count} ${label}`);
  });
  document.querySelectorAll(".admin-nav-group").forEach(group=>{
    const total=[...group.querySelectorAll(".admin-nav-items .nav-badge")].reduce((sum,badge)=>sum+(Number(badge.textContent)||0),0),meta=group.querySelector(".admin-nav-group-meta");
    let badge=meta?.querySelector(":scope > b");
    if(!total){badge?.remove();return;}
    if(!badge&&meta){badge=document.createElement("b");meta.prepend(badge);}
...
function adminMenuStatystyki(){
  ...
  const metrykiAgentaDostepne=typeof agentAIMetrykiScalone==="function"&&((typeof agentAIOperations!=="undefined"&&agentAIOperations.loaded)||(typeof agentAIRuntime!=="undefined"&&agentAIRuntime.loaded)||(typeof agentAISpecjalisci!=="undefined"&&agentAISpecjalisci.loaded)),metrykiAgenta=metrykiAgentaDostepne?agentAIMetrykiScalone():null;
  const zadaniaAgenta=metrykiAgenta?Math.max(0,Number(metrykiAgenta.tasks||0)+Number(metrykiAgenta.inlineDecisions||0)):typeof agentAIAnalizaAktywna==="function"&&typeof agentAIAnaliza==="function"?agentAIAnalizaAktywna(agentAIAnaliza()).length:0;
