...
;(function productLinkFileImportParserModule(global){
  ...
  const {PRODUCT_DATA_FIELDS, normalizedProductDataValue} = global.productLinkFileImportFields || {};
  if(!Array.isArray(PRODUCT_DATA_FIELDS) || typeof normalizedProductDataValue !== "function") throw parserError("Nie załadowano mapowania kolumn importu produktów.", "MISSING_PRODUCT_FIELD_MAPPER");
  ...
  function chooseColumns(tableRows){
    ...
    for(let index = 0; index < candidates.length; index++){
      ...
      if(linkColumn >= 0){
        ...
        if(!best || score > best.score){
          const dataColumns = Object.create(null), detectedFields = [];
          cells.forEach((cell, column) => {
            const header = normalizeHeader(cell);
            let match = null;
            for(const definition of PRODUCT_DATA_FIELDS){
              const fieldScore = definition.score(header);
              if(fieldScore > (match?.score || 0)) match = {...definition, score:fieldScore};
            }
            if(match && dataColumns[match.key] === undefined){
              dataColumns[match.key] = column;
              detectedFields.push({key:match.key, label:match.label, header:String(cell || "").trim(), column:column + 1});
            }
          });
          if(nameColumn >= 0 && dataColumns.nazwa === undefined){
            dataColumns.nazwa = nameColumn;
            detectedFields.unshift({key:"nazwa", label:"nazwa produktu", header:String(cells[nameColumn] || "").trim(), column:nameColumn + 1});
          }
          best = {headerIndex:index, linkColumn, nameColumn, dataColumns, detectedFields, score};
        }
      }
    }
    if(best) return best;
    ...
    if(detectedColumn >= 0 && detectedCount > 0) return {headerIndex:-1, linkColumn:detectedColumn, nameColumn:-1, dataColumns:{}, detectedFields:[], score:detectedCount};
  ...
  function rowsFromTable(tableRows){
    ...
    for(let index = columns.headerIndex + 1; index < tableRows.length; index++){
      ...
      const fileData = {};
      for(const [field, column] of Object.entries(columns.dataColumns || {})){
        const value = normalizedProductDataValue(field, row.cells?.[column]);
        if(value !== null && value !== undefined && value !== "") fileData[field] = value;
      }
      candidates.push({rowNumber:row.rowNumber || index + 1, name, rawUrl, fileData});
    ...
    return {rows, duplicates, invalid, total:candidates.length, detectedFields:columns.detectedFields || []};
  }
...
let productLinkImportStan={
  initialized:false,jobId:"",job:null,summary:null,items:[],analysisItems:[],parsedRows:[],detectedFields:[],fileName:"",
...
function productLinkImportRozpoznajWiersze(parsed){
  ...
  return source.map((raw,index)=>{
    ...
    const fileData=raw?.fileData&&typeof raw.fileData==="object"&&!Array.isArray(raw.fileData)?{...raw.fileData}:{};
    const name=String(raw?.name||raw?.nazwa||raw?.productName||fileData.nazwa||"").trim();if(name&&!fileData.nazwa)fileData.nazwa=name;
    return {rowNumber:productLinkImportLiczba(raw?.rowNumber||raw?.row||raw?.line,index+1),name,url:String(raw?.url||raw?.link||raw?.sourceUrl||"").trim(),fileData};
  }).filter(row=>/^https?:\/\//i.test(row.url));
}
function productLinkImportEtykietaPola(field){return ({nazwa:"nazwa produktu",cena:"cena sprzedaży",cenaZakupu:"cena zakupu",cenaAllegro:"cena Allegro",cenaVonHalsky:"cena Von Halsky",producent:"producent",marka:"marka",kategoria:"kategoria",ean:"EAN / GTIN",gtin:"EAN / GTIN",kodProducenta:"kod producenta",mpn:"MPN",sku:"SKU / indeks",externalId:"numer katalogowy",opisKrotki:"krótki opis",opis:"opis",zdjecie:"główne zdjęcie",minimalnyWiek:"minimalny wiek",liczbaElementow:"liczba elementów",material:"materiał",kolorProduktu:"kolor produktu",waga:"waga",dlugosc:"długość",szerokosc:"szerokość",wysokosc:"wysokość",vonHalskyCategoryId:"kategoria Von Halsky",vonHalskySafetyInformation:"bezpieczeństwo GPSR",gpsrNazwa:"GPSR — producent",gpsrAdres:"GPSR — adres",gpsrEmail:"GPSR — e-mail",gpsrTelefon:"GPSR — telefon"})[field]||String(field||"");}
function productLinkImportPolaPliku(){
  const keys=new Set((productLinkImportStan.detectedFields||[]).map(entry=>String(entry?.key||entry)).filter(Boolean));
  for(const item of [...productLinkImportStan.parsedRows,...productLinkImportStan.items])for(const key of Object.keys(item?.fileData||{}))keys.add(key);
  return [...keys].filter(key=>key!=="gtin"||!keys.has("ean"));
}
function productLinkImportCenaZPliku(item={}){const value=Number(item?.fileData?.cena);return value>0?value.toLocaleString("pl-PL",{style:"currency",currency:"PLN"}):"";}
...
async function productLinkImportWczytajPlik(file){
  ...
  try{
    ...
    productLinkImportStan.jobId="";productLinkImportStan.job=null;productLinkImportStan.summary=null;productLinkImportStan.parsedRows=rows;productLinkImportStan.detectedFields=Array.isArray(parsed?.detectedFields)?parsed.detectedFields:[];
    ...
    productLinkImportStan.analysisItems=analysisItems;productLinkImportStan.page=1;const fields=productLinkImportPolaPliku().map(productLinkImportEtykietaPola);productLinkImportStan.notice=rows.length?`Gotowe: ${rows.length} ${rows.length===1?"poprawny link":"poprawnych linków"}${fields.length?` • odczytano: ${fields.join(", ")}`:""}${analysisItems.length?` • ${analysisItems.length} pozycji odrzuconych pokazano w raporcie`:""}. Dane z arkusza uzupełnią kartotekę; puste komórki niczego nie usuną.`:`Nie znaleziono poprawnych linków. ${analysisItems.length} odrzuconych pozycji pokazano poniżej — popraw plik przed uruchomieniem importu.`;
    productLinkImportZapiszPamiec();
  }catch(error){productLinkImportStan.error=error?.message||String(error);productLinkImportStan.notice="";productLinkImportStan.parsedRows=[];productLinkImportStan.detectedFields=[];productLinkImportStan.items=[];productLinkImportStan.analysisItems=[];}
...
function productLinkImportElementyPoFiltrze(){
  ...
  return productLinkImportWszystkieElementy().filter(item=>{const status=productLinkImportStatus(item.status),statusMatch=filter==="all"||status===filter||(filter==="file_rejected"&&["invalid_file","duplicate_file"].includes(status));return statusMatch&&(!q||[item.rowNumber,item.name,item.url,item.productId,item.duplicateProductId,item.reason,item.error,...Object.values(item.fileData||{})].join(" ").toLowerCase().includes(q));});
}
...
function productLinkImportEksportujRaport(){
  const rows=productLinkImportWszystkieElementy().map(item=>[item.rowNumber,item.name,item.fileData?.producent||item.fileData?.marka||"",item.fileData?.cena||"",item.url,productLinkImportEtykietaStatusu(item.status),item.productId||item.duplicateProductId||"",item.attempts,item.error||item.reason||""].map(productLinkImportCSVBezpieczne));
  if(typeof adminEksportujCSV==="function")adminEksportujCSV(`import-linkow-${new Date().toISOString().slice(0,10)}.csv`,["Wiersz","Nazwa","Producent z pliku","Cena z pliku","Link","Status","ID produktu","Próby","Informacja"],rows);
}
...
function productLinkImportOdswiezDOM(){
  ...
  const fileMeta=root.querySelector("[data-product-link-file-meta]");if(fileMeta){const fields=productLinkImportPolaPliku().map(productLinkImportEtykietaPola);fileMeta.innerHTML=productLinkImportStan.fileName?`<b>${esc(productLinkImportStan.fileName)}</b><span>${esc(summary.fileTotal||productLinkImportStan.parsedRows.length)} przeanalizowanych wierszy • ${esc(summary.total)} poprawnych do kolejki${summary.fileRejected?` • ${esc(summary.fileRejected)} odrzuconych`:""}${fields.length?`<br>Odczytane dane: ${esc(fields.join(", "))}`:""}</span>`:`<b>Wybierz arkusz lub plik tekstowy</b><span>Obsługiwane: XLSX, CSV i TXT</span>`;}
...
function widokAdminProduktyZPliku(){
  ...
  return asortymentSzkielet("produkty",`<section class="product-link-file-import-page" data-product-link-import-page>
    ...
    <div class="panel product-link-import-upload-panel">
      <div class="order-section-head"><div><span class="order-pro-label">Krok 1</span><h2>Wybierz plik z danymi produktów</h2><p>System automatycznie rozpozna link, nazwę, cenę sprzedaży, producenta oraz inne obsługiwane kolumny. Wartości z arkusza uzupełnią kartotekę, a puste komórki nie nadpiszą danych pobranych ze źródła.</p></div></div>
...
function widokAdminMapowanie(){
  ...
  ${adminOperacjeWynikowHTML({id:"mapowanie",selected,pageCount:page.length,resultCount:rows.length,selectPage:"zaznaczMapowanieZakres('strona',true)",selectAll:"zaznaczMapowanieZakres('filtr',true)",deselectPage:"zaznaczMapowanieZakres('strona',false)",deselectAll:"zaznaczMapowanieZakres('filtr',false)",clear:"odznaczMapowania()",exportSelected:"mapowanieEksportuj('zaznaczone')",exportAll:"mapowanieEksportuj('filtr')",extra:`<label class="mapping-bulk-target">Przenieś zaznaczone do <select id="mapCel">${categoryOptions()}</select></label><button class="btn" onclick="przeniesZaznaczone()" ${selected?"":"disabled"}>🧩 Przenieś</button><button class="btn ghost" onclick="usunMapowanieZaznaczonych()" ${selected?"":"disabled"}>↩️ Przywróć źródło</button>`})}
...
function zaznaczMapowanieZakres(scope,zaznacz=true){(scope==="strona"?mapowanieStronaIds:mapowanieWynikiIds).forEach(id=>zaznacz?zaznaczoneMap.add(String(id)):zaznaczoneMap.delete(String(id)));renderuj();}
