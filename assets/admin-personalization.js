...
/* Import, eksport i kopie danych. Publikacja kodu jest obsługiwana wyłącznie
...
function widokAdminEksport(sekcja="import"){
  ...
  return adminSzkielet("/admin/eksport",`<div class="module-page-stack">${eksportSubnavHTML(aktywna)}${aktywna==="import"?importHTML:aktywna==="eksport"?eksportHTML:kopieHTML}</div>`);
}
function resetujUstawienia(){localStorage.removeItem("artway_ustawienia");loguj("info","Przywrócono domyślne ustawienia");location.reload();}
...
async function zapiszListeRegul(rules){
  const unique=rabatProCanonical(rules),legacy={};for(const r of unique)if(r.aktywny!==false&&r.typ==="procent"&&r.zakres==="wszystkie"&&!r.start&&!r.koniec&&!r.minKoszyk&&!r.maxRabat&&!r.limitUzyc)legacy[r.kod]=r.wartosc;const publicRule=unique.find(x=>x.publiczny&&x.aktywny!==false);rabatProStan.save="saving";rabatProStan.message="Zapisywanie na serwerze…";renderuj();const ok=await zapiszCzescUstawien({kodyRabatoweZaawansowane:unique,kody:legacy,promocjaGlowna:publicRule?.kod||""});rabatProStan.save=ok?"saved":"error";rabatProStan.message=ok?`Serwer potwierdził ${unique.length} reguł`:(chmuraStan.error||"Zmiana czeka w trwałej kolejce");loguj(ok?"info":"ostrzezenie",rabatProStan.message);if(location.hash.includes("/rabaty"))renderuj();return ok;
}
