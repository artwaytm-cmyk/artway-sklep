...
function formularzProduktu(p, tryb){
  ...
  const ofertaAllegro=maTozsamoscProduktu?allegroOfertaDlaProduktuSklepu(p):null,ofertaAllegroId=String(p.allegroOfferId||ofertaAllegro?.id||"").trim(),ofertaAllegroStatus=String(ofertaAllegro?.status||ofertaAllegro?.publication?.status||"").toUpperCase(),domyslnaPublikacjaAllegro=ofertaAllegroStatus==="ACTIVE"?"keep":"activate",rentownosc=allegroRentownoscProduktu(p),rentownoscSklep=sklepRentownoscProduktu(p),seoDanePodgladu=seoEfektywneDaneProduktu(p),zapisanyKanal=productEditorPoczatkowyKanal(),aktywnaSekcja=productEditorPoczatkowaSekcja(),aktywnyKanal=["store","allegro","vonHalsky"].includes(aktywnaSekcja)?aktywnaSekcja:zapisanyKanal;
  return `
