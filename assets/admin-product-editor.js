...
function productEditorKanalDefinicja(p={},channel="store"){
  ...
  const profile=p.manufacturerProfile||{},producer=profile.displayName||profile.manufacturerName||p.producent||"",gpsr=profile.address&&profile.email?`${profile.address} • ${profile.email}`:"";
  const common=[
    {label:"GTIN / EAN",value:text(p.gtin||p.ean),required:true,source:"Dane wspólne"},
    {label:"Kod producenta / SKU",value:text(kodKanonicznyProduktu(p)),required:true,source:"Dane wspólne"},
    {label:"Producent",value:text(producer),required:true,source:"Dane wspólne"},
    {label:"Marka",value:text(p.marka||p.brand||producer),required:false,source:"Dane wspólne"},
    {label:"GPSR / podmiot odpowiedzialny",value:text(gpsr),required:true,source:"Profil producenta"},
    {label:"Zdjęcie główne",value:text(p.zdjecie),required:true,source:"Media wspólne",display:p.zdjecie?"zapisane":""},
    {label:"Galeria",value:count(p.zdjecia),required:false,source:"Media wspólne",display:`${count(p.zdjecia)} zdjęć`},
    {label:"Źródło produktu",value:text(p.sourceUrl||p.producentUrl),required:false,source:"Źródło wspólne",display:text(p.sourceUrl||p.producentUrl)?"zweryfikowane":""},
  ];
  ...
  const items=[...own,...common],required=items.filter(item=>item.required),done=required.filter(item=>item.value!==""&&item.value!==null&&item.value!==undefined&&item.value!==false).length;
  return {items,required:required.length,done,percent:required.length?Math.round(done/required.length*100):100,missing:required.filter(item=>!item.value).map(item=>item.label)};
}
  ...
  return `
    <form class="product-editor-form" data-product-id="${esc(p.id||0)}" ${!edycja?`data-product-add-form data-product-duplicate-fingerprint="${esc(kontrolaDodawania.fingerprint)}" oninput="produktDodawanieZmienione(event,this)" onchange="produktDodawanieZmienione(event,this)"`:""} onsubmit="${edycja?`zapiszProduktAdmin(event,${jsArg(p.id)})`:"dodajProdukt(event)"}">
      ...
      <details id="product-editor-allegro" class="product-editor-section product-channel-section allegro" open>
        ...
        ${productEditorKanalKontrolaHTML(p,"allegro")}
