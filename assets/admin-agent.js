...
function agentAIRuntimePanelHTML(){
  ...
  return `<section class="agent-runtime-dashboard">
    ...
    <section class="panel agent-publication-proof"><div class="order-section-head"><div><span class="order-pro-label">PRACA • TRWAŁY ZAPIS • JEDNOZNACZNY KONIEC</span><h2>Przygotowanie produktów do Allegro</h2><p class="order-detail-lead">Licznik dotyczy wyłącznie najnowszego stanu przygotowania Allegro. Produkt nie jest uznany za gotowy, dopóki nie przejdzie zapisu i odczytu kontrolnego. Korekta jest kontynuowana automatycznie; po wyczerpaniu bezpiecznych metod powstaje dokładna decyzja z listą braków.</p></div><div class="agent-publication-counts"><span><b>${esc(preparationBusy)}</b><small>w toku</small></span><span><b>${esc(preparationWaiting)}</b><small>czeka wyłącznie na AI</small></span><span><b>${esc(preparationDecisions)}</b><small>konkretne decyzje</small></span><span><b>${esc(preparationCompleted)}</b><small>trwale gotowe</small></span></div></div>${preparationQueue.blockedUntil&&Date.parse(preparationQueue.blockedUntil)>Date.now()?`<div class="backend-note warn"><b>Redakcja AI jest czasowo niedostępna do ${esc(new Date(preparationQueue.blockedUntil).toLocaleString("pl-PL"))}.</b><p>${esc(preparationQueue.blockedReason||"Limit dostawcy AI")}. W tym czasie serwer nadal kończy źródła, zdjęcia, parametry, kategorię, GPSR, zapis centralny i walidację. Tylko pozycje wymagające samej redakcji oczekują na usługę.</p></div>`:""}<div class="agent-publication-list">${preparationRows.map(work=>{const runtimeWork=runtimePublicationRows.find(item=>String(item.productId||"")===String(work.productId||"")),row={...runtimeWork,...work,productName:work.name||runtimeWork?.productName||`Produkt ${work.productId||""}`,channel:"allegro",action:"przygotowanie produktu do Allegro"};return `<article class="${esc(row.status)}"><span>${["confirmed","completed"].includes(row.status)?"✓":row.status==="running"?"⋯":row.status==="decision_required"?"?":row.status==="pending"||row.status==="attention"?"↻":"!"}</span><div><b>${esc(row.productName||row.action||"Operacja systemowa")}</b><small>Allegro • ${esc(agentAIEtapPracyLabel(row.phase||row.status||workStatus(row)))}</small><p>${esc(row.error||row.message||(Array.isArray(row.missing)&&row.missing.length?`Pozostało: ${row.missing.join(", ")}`:workStatus(row)))}</p></div><em>${esc(workStatus(row))}<small>${esc(agentAIRuntimeCzas(row.completedAt||row.updatedAt||row.startedAt||row.requestedAt))}</small></em></article>`;}).join("")||`<div class="agent-ops-empty">Brak rozpoczętych zadań przygotowania Allegro.</div>`}</div></section>
