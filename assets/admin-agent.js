...
function agentAISpecjalistaDecyzjaHTML(item={}){
  ...
  return `<article class="agent-decision-ticket ${riskClass} ${item.executionStatus==="failed"?"failed":""}">
    ...
    ${product&&fields.length?`<details class="agent-decision-details"><summary>Porównaj ${fields.length} proponowanych zmian</summary><div class="agent-decision-changes">${fields.map(field=>`<label><input type="checkbox" data-agent-decision-field="${esc(item.id)}" value="${esc(field.key)}" checked><span><small>${esc(field.label||field.key)}</small>${field.currentValue?`<del>${esc(field.currentValue)}</del>`:""}<strong>${esc(field.value||"")}</strong></span></label>`).join("")}</div>${warnings.length?`<div class="agent-specialist-warnings">${warnings.map(value=>`<span>⚠️ ${esc(value)}</span>`).join("")}</div>`:""}</details><details class="agent-decision-correction"><summary>Poproś Agenta o inną wersję</summary><div><textarea data-agent-feedback="${esc(item.id)}" rows="3" placeholder="Napisz konkretnie, co ma zmienić…">${esc(item.feedbackNote||"")}</textarea><button class="btn ghost" onclick="agentAISpecjalistaPoprawDecyzje(${jsArg(item.id)})" ${busy?"disabled":""}>Przygotuj korektę</button></div></details>`:""}
    <footer><div>${item.href?`<a class="btn ghost" href="${esc(item.href)}">Otwórz źródło</a>`:""}</div><div><button class="btn ghost" onclick="agentAISpecjalistaDecyzja(${jsArg(item.id)},'snooze',1)" ${busy?"disabled":""}>Jutro</button><button class="btn ghost" onclick="agentAISpecjalistaDecyzja(${jsArg(item.id)},'dismiss')" ${busy?"disabled":""}>Nie wykonuj</button><button class="btn" onclick="agentAISpecjalistaDecyzja(${jsArg(item.id)},'approve')" ${busy?"disabled":""}>${busy?"Zapisuję…":product?"Zatwierdź i zapisz":"Zatwierdź decyzję"}</button></div></footer>
  </article>`;
}
...
function agentAISpecjalisciPanelHTML(){
  ...
  return `<section class="agent-specialists-page agent-observability-page">
    <section class="panel agent-observer-head"><div><span class="agent-live-dot ${connected?"online":"offline"}"></span><div><small>CENTRUM PRACY AGENTA</small><h2>${connected?"Agent działa i czuwa":"Automatyka wymaga kontroli"}</h2><p>${last.completedAt?`Ostatni pełny cykl ${esc(agentAIRuntimeCzas(last.completedAt))}. Nowa wiadomość uruchamia analizę zdarzeniowo; bez zmian komunikacja jest kontrolowana dopiero po 12 godzinach.`:"Oczekiwanie na pierwszy zapisany cykl serwera."}</p></div></div><button class="btn" onclick="agentAISpecjalisciCykl()" ${state.running?"disabled":""}>${state.running?"⏳ Cykl trwa":"↻ Sprawdź teraz"}</button></section>
    ...
    <section class="panel agent-decision-board"><div class="order-section-head"><div><span class="order-pro-label">Tylko decyzje człowieka</span><h2>Do zatwierdzenia</h2><p class="order-detail-lead">Po podjęciu decyzji karta znika natychmiast i pozostaje w trwałej historii. Ten sam temat nie zostanie utworzony ponownie przez zmianę danych technicznych.</p></div><strong>${esc(decisions.length)}</strong></div><div class="agent-decision-list">${decisions.map(agentAISpecjalistaDecyzjaHTML).join("")||`<div class="agent-decision-empty"><span>✓</span><div><b>Nie czeka żadna decyzja</b><small>Agent nadal wykonuje bezpieczne zadania w tle.</small></div></div>`}</div></section>
