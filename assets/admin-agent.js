...
function agentAIRuntimePanelHTML(){
  ...
  return `<section class="agent-runtime-dashboard">
    ...
    ${warnings.length?`<div class="panel agent-runtime-warning ${aiWarnings.length?"is-ai-warning":"is-external-warning"}"><span>${aiWarnings.length?"⚠️":"🔌"}</span><div><b>${aiWarnings.length?`${aiWarnings.length} elementów AI wymaga kontroli`:`Połączenie Allegro wymaga sprawdzenia`}</b><p>${aiWarnings.length?aiWarnings.map(x=>`${esc(x.label)}: ${esc(x.error)}`).join(" • "):`Agent pozostaje gotowy do pracy lokalnej. Operacje korzystające z Allegro są bezpiecznie zatrzymane do potwierdzenia połączenia.`}</p></div><a class="btn" href="${aiWarnings.length?"#/admin/agent-ai/zasady":"#/admin/allegro/ustawienia"}">${aiWarnings.length?"Sprawdź ustawienia AI":"Sprawdź połączenie"}</a></div>`:""}
