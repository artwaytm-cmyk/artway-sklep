...
function agentAIRuntimePanelHTML(){
  ...
  const stateMeta={ready:["🟢","Agent czeka na sygnał","Nie wykonuje pustych kontroli. Nowe zdarzenie uruchomi wyłącznie właściwy moduł."],online:["🟢","Agent czeka na sygnał","Nie wykonuje pustych kontroli. Nowe zdarzenie uruchomi wyłącznie właściwy moduł."],working:["⚙️","Agent właśnie pracuje",currentWork?.message||eventCurrent?.payload?.action||"Obsługuję odebrane zdarzenie."],degraded:["🟠",aiWarnings.length?"Agent AI wymaga kontroli":"Agent AI działa prawidłowo",aiWarnings.length?"Dostawca AI zgłosił błąd podczas ostatniego zadania.":"Ostrzeżenie dotyczy wyłącznie zewnętrznej integracji sprzedażowej."],stale:["🟡","Agent jest w gotowości","Nie ma cyklu czasowego. Brak pracy oznacza brak nowego zdarzenia."],offline:["🔴","Agent wykonawczy offline","Proces nie wysłał aktualnego sygnału pracy."]}[r.state]||["⚪","Sprawdzam Agenta","Oczekiwanie na pierwszy sygnał procesu."];
  const providerCards=[
...
function agentAIMetrykiScalone(){
  ...
  return {tasks:priorities.length,bad:priorities.filter(x=>x.severity==="critical").length,warn:priorities.filter(x=>x.severity==="warning").length,decisions:priorities.filter(x=>x.requiresApproval).reduce((sum,x)=>sum+Number(x.count||0),0),messages:Number(summary.communicationWaiting||0),onboarding:Number(report.not_started||0)+Number(report.needs_data||0),docs:Number(summary.supplierOrders||0),queue:Number(events.active||queue.active||0),queued:Number(events.queued||queue.counts?.queued||0),working:Number(events.running||0)+Number(queue.counts?.processing||0)+Number(queue.counts?.delivering||0),readyToList:Number(report.ready_to_list||0),productDecisions:Number(report.decision||0),specialistDecisions,history:Number(runtime.activity?.length||0)+Number(events.recent?.length||0)};
}
