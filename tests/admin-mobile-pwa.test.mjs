...
const routerSource=await readFile('src/frontend/06-router-and-storefront.js','utf8');
...
test('kanały sprzedaży nie wracają ze starej kopii DOM, a panel nie przeładowuje pracy administratora samoczynnie',()=>{
  assert.match(routerSource,/value\.startsWith\("\/admin\/allegro"\)\|\|value\.startsWith\("\/admin\/von-halsky"\)/);
  assert.match(routerSource,/!liveChannel/);
  assert.match(pwa,/if\(!location\.hash\.startsWith\("#\/admin"\)\)/);
  assert.match(pwa,/Date\.now\(\)-pwaOstatniaKontrolaWydania<30000/);
  assert.doesNotMatch(pwa,/location\.hash\.startsWith\("#\/admin"\)\?600:3000/);
  assert.match(pwa,/window\.addEventListener\("hashchange"/);
