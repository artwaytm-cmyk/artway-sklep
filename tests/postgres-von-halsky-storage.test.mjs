...
test('Von Halsky ma wersjonowane migracje i rekordowy magazyn PostgreSQL', async () => {
  ...
  assert.match(repository, /async function readProductQueue/);
  assert.match(repository, /async function readRecordPage/);
  assert.match(repository, /async function readDashboardSummary/);
