...
test('edytor ma trzy kompletne i jednoznaczne sekcje kanałów', async () => {
  const [editor, readiness, workspace] = await Promise.all([
    readFile('src/frontend/12-product-editor.js', 'utf8'),
    readFile('src/frontend/12-product-editor-channel-readiness.js', 'utf8'),
  ...
  assert.match(readiness, /function productEditorKanalStanOperacyjny/);
  assert.match(readiness, /function productEditorKanalAutomatykaHTML/);
  assert.match(readiness, /AUTOMATYKA I STAN OPERACYJNY/);
  assert.match(workspace, /Tożsamość katalogowa Allegro/);
  assert.match(workspace, /GPSR Allegro/);
  assert.match(workspace, /GPSR Von Halsky/);
  assert.match(workspace, /operational=productEditorKanalStanOperacyjny\(p,"store"\)/);
  assert.match(workspace, /operational=productEditorKanalStanOperacyjny\(p,"vonHalsky"\)/);
  assert.match(workspace, /Stan API, Agenta i wymaganych pól/);
  assert.match(editor, /productEditorKanalAutomatykaHTML\(p,"allegro"\)/);
  assert.match(editor, /allegroOperational\.ready/);
});

test('konflikt tożsamości Von Halsky blokuje gotowość i pokazuje dokładny błąd w edytorze', async () => {
  const [quality, readiness, workspace] = await Promise.all([
    readFile('src/frontend/11b-von-halsky-product-quality.js', 'utf8'),
    readFile('src/frontend/12-product-editor-channel-readiness.js', 'utf8'),
    readFile('src/frontend/12-product-editor-workspace.js', 'utf8'),
  ]);
  assert.match(quality, /if\(product\.vonHalskyRemotePresent===false\)return null/);
  assert.match(readiness, /"IDENTITY_CONFLICT","DUPLICATE_MAPPING"/);
  assert.match(readiness, /! Konflikt produktu/);
  assert.match(workspace, /Konflikt produktu — aktualizacja zablokowana/);
  assert.match(workspace, /vonHalskyEditorialSyncError/);
  assert.match(workspace, /Dlaczego system zatrzymał produkt/);
  assert.match(workspace, /System usunął błędne powiązanie i nie zmienił cudzej oferty/);
});
...
test('producent i inne dane wspólne nie są powielane w listach pól kanałów', async () => {
  ...
  assert.doesNotMatch(definition, /p\.allegroProductId\|\|p\.gtin\|\|p\.ean/);
});
