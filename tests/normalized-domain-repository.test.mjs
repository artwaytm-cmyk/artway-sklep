...
test('duże domeny są serializowane przed transakcją, a zerwany klient jest usuwany z puli', async () => {
  const source = await readFile(new URL('../src/backend/lib/core/normalized-domain-repository.mjs', import.meta.url), 'utf8');
  const block = source.slice(source.indexOf('const writeDirect = async'), source.indexOf('const readDiagnosticsProjection'));
  assert.ok(block.indexOf('prepareNormalizedDomainWrite(value, config)') < block.indexOf("await client.query('BEGIN')"));
  assert.match(block, /rollbackAfterFailure\(client, error\)/);
  assert.match(block, /client\.release\(releaseError \|\| undefined\)/);
  assert.match(source, /postgresConnectionIsBroken/);
  assert.match(source, /not queryable/);
});

test('migracja priorytetowych odczytów pokrywa kolejkę i duże domeny Allegro', async () => {
  const migration = await readFile(new URL('../db/migrations/0022_priority_queue_read_indexes.sql', import.meta.url), 'utf8');
  assert.match(migration, /artway_allegro_preparation_results_recent_idx/);
  assert.match(migration, /completed_at DESC NULLS LAST/);
  assert.match(migration, /artway_allegro_offers_read_order_idx/);
  assert.match(migration, /artway_allegro_mappings_read_order_idx/);
  assert.match(migration, /artway_agent_records_read_order_idx/);
});
