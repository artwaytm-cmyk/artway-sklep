...
test("publikacja doładowuje ciężkie karty stopniowo, a sprzedaż używa tej samej siedmiokolumnowej tabeli kanału",async()=>{
  ...
  assert.match(source,/function allegroOfertaWierszParityHTML/);
  assert.match(source,/<th>Produkt<\/th><th>Identyfikacja<\/th><th>Gotowość<\/th><th>Cena<\/th><th>Kanał sprzedaży<\/th><th>Akcje<\/th>/);
  assert.match(source,/POWIĄZANIE POTWIERDZONE/);
  assert.match(source,/Oferta i produkt potwierdzone przez API/);
  assert.match(source,/Wszyscy producenci \(\$\{facetBase\.length\}\)/);
  assert.match(source,/Wszystkie kategorie \(\$\{facetBase\.length\}\)/);
  ...
  assert.match(sync,/produktyDodane=\[\];produktyEdytowane=\{\};produktyUkryte=\[\];produktyDefinitywne=\[\]/);
  assert.doesNotMatch(sync,/zapiszLS\(["']artway_produkty_(?:dodane|edytowane|ukryte|definitywne)/);
