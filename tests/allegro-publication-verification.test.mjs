...
test('synchronizacja samej treści może dołączyć wyłącznie ID istniejącego produktu katalogowego', () => {
  ...
  assert.deepEqual(patch.productSet, [{ product: { id: 'catalog-product-123' }, quantity: { value: 2 } }]);
...
test('synchronizacja treści zachowuje wszystkie elementy zestawu i ich ilości', () => {
  const patch = allegroPatchZDraftu({
    productSet: [
      { product: { id: 'catalog-a' }, quantity: { value: 1 } },
      { product: { id: 'catalog-b' }, quantity: { value: 2 } },
    ],
  }, { contentOnly: true, includeCatalogProduct: true });
  assert.deepEqual(patch.productSet, [
    { product: { id: 'catalog-a' }, quantity: { value: 1 } },
    { product: { id: 'catalog-b' }, quantity: { value: 2 } },
  ]);
});
...
test('backend po zapisie ponownie odczytuje ofertę i zwraca zweryfikowany status', async () => {
  ...
  assert.match(source, /if \(!publicationConfirmed\)/);
  assert.match(source, /allegro_publication_unconfirmed/);
