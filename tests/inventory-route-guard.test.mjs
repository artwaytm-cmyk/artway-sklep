...
test('przypisanie półki przechodzi przez chroniony endpoint lokalizacji', async () => {
  let value = { data: {
    artway_stany: { '1': 2 },
    artway_magazyn_produkty: { '1': { lokalizacja: '' } },
    artway_magazyn_lokalizacje: [{ kod: 'PAK-RA-P01', typ: 'półka', aktywna: true }],
  ...
  const route = createInventoryStockRoute({
    ...
    respond: (payload, status = 200) => ({ payload, status }),
    sessionOf: () => ({ email: 'magazyn@example.test' }),
    writeIfVersion: async (_key, next, version) => {
      if (String(version.etag) !== String(etag)) return { modified: false };
      value = structuredClone(next);etag += 1;return { modified: true };
    },
    settingsLimit: 4 * 1024 * 1024,
  });
  const request = new Request('https://artwaytm.pl/api/store?action=warehouse-product-location-assign', {
    method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ productId: '1', location: 'pak-ra-p01' }),
  });
  const result = await route(request, new URL(request.url), 'warehouse-product-location-assign');
  assert.equal(result.status, 200);
  assert.equal(result.payload.assigned, true);
  assert.equal(value.data.artway_magazyn_produkty['1'].lokalizacja, 'PAK-RA-P01');
  assert.equal(value.data.artway_magazyn_produkty['1'].operator, 'magazyn@example.test');
  assert.equal(value.data.artway_stany['1'], 2);
});
