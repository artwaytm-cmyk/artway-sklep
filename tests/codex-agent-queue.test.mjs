...
test('potwierdzona naprawa kodu ma pierwszeństwo przed zwykłym poleceniem panelu', async () => {
  const repo = repository();
  const queue = createCodexAgentQueue({
    readVersioned: repo.readVersioned,
    writeIfVersion: repo.writeIfVersion,
  ...
  await queue.enqueue({ requestId: 'normal-1', text: 'Zwykłe polecenie', user: 'admin' });
  await queue.enqueue({ requestId: 'repair-1', text: '[NAPRAWA KODU] ReferenceError', user: 'Agent diagnostyczny' });
  const claimed = await queue.claim('worker-priority');
  assert.equal(claimed.job.requestId, 'repair-1');
});
