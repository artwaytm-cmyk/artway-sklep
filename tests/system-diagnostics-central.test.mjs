...
test('ten sam błąd analityki na różnych kartach produktów tworzy jedną grupę', async () => {
  const { service, record } = fixture();
  await service.record([
    { level: 'ostrzezenie', message: 'Nie udało się wczytać anonimowej analityki SEO', source: 'przeglądarka', route: '/#/produkt/123' },
    { level: 'ostrzezenie', message: 'Nie udało się wczytać anonimowej analityki SEO', source: 'przeglądarka', route: '/produkt/987' },
  ]);
  assert.equal(record().items.length, 1);
  assert.equal(record().items[0].count, 2);
  assert.equal(record().items[0].route, '/produkt/987');
});

test('telemetria crawlerów nie trafia do centralnego rejestru problemów', async () => {
  const { service, record } = fixture();
  const request = new Request('https://artwaytm.pl/api/store?action=diagnostics-ingest', {
    method: 'POST',
    headers: { 'content-type': 'application/json', 'user-agent': 'Mozilla/5.0 Applebot/0.1' },
    body: JSON.stringify({ events: [{ level: 'ostrzezenie', message: 'Nie udało się wczytać anonimowej analityki SEO', source: 'przeglądarka', route: '/produkt/123' }] }),
  });
  const response = await service.route(request, new URL(request.url), 'diagnostics-ingest');
  assert.equal(response.status, 202);
  assert.equal((await json(response)).ignored, 'automated-client');
  assert.deepEqual(record(), {});
});

test('awaria wspólnego źródła zdjęć tworzy jedną grupę zamiast wiersza dla każdego produktu', async () => {
  const { service, record } = fixture();
  await service.record([
    { level: 'ostrzezenie', message: 'Nie wczytano zdjęcia produktu: 20 Gier', source: 'przeglądarka', route: '/' },
    { level: 'ostrzezenie', message: 'Nie wczytano zdjęcia produktu: 30 Gier', source: 'przeglądarka', route: '/' },
  ]);
  assert.equal(record().items.length, 1);
  assert.equal(record().items[0].count, 2);
  assert.match(record().items[0].message, /30 Gier/);
});
...
test('potwierdzony błąd aplikacji automatycznie trafia do naprawy kodu i zamyka się dopiero po wdrożeniu', async () => {
  const queued = [];
  const diagnosticAgent = {
    status: () => ({ configured: true, model: 'gpt-test' }),
    analyze: async () => ({
      classification: 'application_bug', rootCause: 'Zmienna użyta przed inicjalizacją.', confidence: 0.97,
      evidence: ['ReferenceError'], recommendedActions: [], validationPlan: ['Test regresji'],
      safeAutomaticAction: 'none', requiresHumanApproval: true, summary: 'Błąd kolejności inicjalizacji.',
    }),
  };
  const { service, record } = fixture({
    diagnosticAgent,
    enqueueCodeRepair: async (item, options) => { queued.push({ item, options }); return { jobId: 'CX-CODE-1' }; },
  });
  await service.record([{ level: 'blad', message: "Cannot access 'labelReady' before initialization", source: 'backend:inpost-label' }]);
  const id = record().items[0].id;
  const analyze = new Request('https://artwaytm.pl/api/store?action=diagnostics-central-analyze', {
    method: 'POST', headers: { 'content-type': 'application/json', 'x-admin': '1' }, body: JSON.stringify({ ids: [id] }),
  });
  await service.route(analyze, new URL(analyze.url), 'diagnostics-central-analyze');
  for (let attempt = 0; attempt < 30 && record().items[0].repair.status !== 'queued'; attempt += 1) await new Promise((resolve) => setTimeout(resolve, 5));
  assert.equal(queued.length, 1);
  assert.equal(queued[0].options.automatic, true);
  assert.equal(record().items[0].repair.jobId, 'CX-CODE-1');
  assert.equal(record().items[0].status, 'investigating');
  const report = new Request('https://artwaytm.pl/api/store?action=diagnostics-code-repair-report', {
    method: 'POST', headers: { 'content-type': 'application/json', 'x-admin': '1' },
    body: JSON.stringify({ ids: [id], repairStatus: 'completed', verified: true, commit: 'abcdef123456', release: 'release-1', summary: 'Pełne testy i wydanie potwierdzone.' }),
  });
  const response = await service.route(report, new URL(report.url), 'diagnostics-code-repair-report');
  assert.equal((await json(response)).verified, true);
  assert.equal(record().items[0].status, 'resolved');
  assert.equal(record().items[0].repair.status, 'completed');
  assert.equal(record().items[0].repair.commit, 'abcdef123456');
});
...
test('pozytywny autotest zamyka odpowiadające mu stare zdarzenia przeglądarki, ale nie inne błędy', async () => {
  ...
  await service.record([
    ...
    { level: 'ostrzezenie', message: 'Nie udało się wczytać anonimowej analityki SEO: Nie udało się wczytać podstrony sklepu: analytics', source: 'przeglądarka', kind: 'browser', route: '/produkt/1000845' },
  ...
  const request = new Request('https://artwaytm.pl/api/store?action=diagnostics-checks-sync', {
    ...
    body: JSON.stringify({
      ...
      passedChecks: [
        ...
        { name: 'Moduły sklepu ładowane na żądanie', group: 'Autotest techniczny', details: 'Treści, konto i analityka działają' },
      ],
    }),
  });
  ...
  assert.equal(record().items.find((item) => /analityki SEO/.test(item.message)).status, 'resolved');
  assert.equal(record().items.find((item) => /zamówienia/.test(item.message)).status, 'open');
});

test('aktualne kontrole zamykają stare timeouty, błąd InPost, konflikty magazynu i niedostępne zdjęcie', async () => {
  const { service, record } = fixture();
  await service.record([
    { level: 'blad', message: "server_error: Cannot access 'labelReady' before initialization", source: 'backend:inpost-label', route: '/api/store' },
    { level: 'ostrzezenie', message: 'Nie wczytano zdjęcia produktu: Ale Japa', source: 'przeglądarka', route: '/' },
    { level: 'blad', message: '57014: anulowano polecenie z powodu przekroczonego czasu wykonania', source: 'backend:von-halsky-publication-queue-claim', route: '/api/store' },
    { level: 'ostrzezenie', message: 'Nie potwierdzono sesji z powodu chwilowej niedostępności serwera', source: 'sesja', route: '/' },
    { level: 'ostrzezenie', message: 'artway_stany: wykryto równoległą zmianę; zapis pozostaje w kolejce', source: 'przeglądarka', route: '/admin/magazyn/plan' },
    { level: 'blad', message: 'server_error: Connection terminated due to connection timeout', source: 'backend:von-halsky-reconcile-catalog', route: '/api/store' },
    { level: 'blad', message: '55P03: anulowano polecenie z powodu przekroczenia czasu blokady', source: 'backend:diagnostics-ingest', route: '/api/store' },
    { level: 'ostrzezenie', message: 'Zamówienia odświeżą się po odzyskaniu połączenia: Serwer nie odpowiedział w wyznaczonym czasie', source: 'zamowienia', route: '/#/admin/wysylki/inpost' },
  ], { trusted: true });
  const request = new Request('https://artwaytm.pl/api/store?action=diagnostics-checks-sync', {
    method: 'POST', headers: { 'content-type': 'application/json', 'x-admin': '1' },
    body: JSON.stringify({ checks: [], passedChecks: [
      { name: 'InPost ShipX API', checkedAt: '2026-08-08T15:00:00.000Z' },
      { name: 'Dostępność zdjęć produktów', checkedAt: '2026-08-08T15:00:00.000Z' },
      { name: 'Backend i uniwersalne API', checkedAt: '2026-08-08T15:00:00.000Z' },
      { name: 'Centralna baza zamówień', checkedAt: '2026-08-08T15:00:00.000Z' },
      { name: 'Spójność mapowania', checkedAt: '2026-08-08T15:00:00.000Z' },
    ] }),
  });
  const response = await service.route(request, new URL(request.url), 'diagnostics-checks-sync');
  assert.equal((await json(response)).summary.open, 0);
  assert.ok(record().items.every((item) => item.status === 'resolved'));
  assert.ok(record().items.every((item) => /pełny autotest/i.test(item.resolution)));
});

test('rejestr diagnostyczny sam ponawia krótką blokadę PostgreSQL bez utworzenia nowego alarmu', async () => {
  let value = {}, version = 0, writes = 0;
  const { service } = fixture({
    readVersioned: async () => ({ value: structuredClone(value), etag: `"${version}"`, exists: version > 0 }),
    writeIfVersion: async (_key, next, expected) => {
      writes += 1;
      if (writes === 1) throw Object.assign(new Error('anulowano polecenie z powodu przekroczenia czasu blokady'), { code: '55P03' });
      if (String(expected?.etag || '') !== `"${version}"`) return { modified: false };
      value = structuredClone(next); version += 1; return { modified: true };
    },
  });
  const result = await service.record([{ level: 'ostrzezenie', message: 'Chwilowy test blokady', source: 'test' }], { trusted: false });
