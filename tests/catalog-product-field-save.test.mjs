...
test('idempotentna powtórka akceptuje zgodne pola biznesowe mimo nowszego pokwitowania', async () => {
  let writes = 0;
  let reads = 0;
  const stored = {
    id: '17',
    vonHalskyOfferId: 'VH-17',
    lastAdminMutationId: 'newer-operation',
    lastAdminMutationAt: '2026-08-05T10:00:00.000Z',
  };
  const save = createCatalogProductFieldSaver({
    ...
    writeOperations: async () => {
      ...
      return {
        modified: true,
        skippedProductIds: [],
        publications: [{ productId: '17', idempotent: true, product: stored }],
      };
    },
    readProduct: async () => (++reads === 1 ? { id: '17', vonHalskyOfferId: '' } : stored),
  });
  const result = await save({
    productId: '17',
    fields: { vonHalskyOfferId: 'VH-17' },
    mutationId: 'old-link-operation',
  });
  assert.equal(writes, 1);
  assert.equal(result.idempotent, true);
  assert.equal(result.product.vonHalskyOfferId, 'VH-17');
});

test('idempotentna mutacja odtwarza utracone pole jednym nowym wpisem audytowym', async () => {
  let stored = { id: '18' };
  const operations = [];
  const save = createCatalogProductFieldSaver({
    now: () => '2026-08-05T10:10:00.000Z',
    writeOperations: async ([operation]) => {
      ...
      if (operations.length === 1) {
        return {
          modified: true,
          skippedProductIds: [],
          publications: [{ productId: '18', idempotent: true, product: stored }],
        };
      }
      stored = { ...stored, ...operation.fields };
      return {
        modified: true,
        skippedProductIds: [],
        publications: [{ productId: '18', updated: true, product: stored }],
      };
    },
    readProduct: async () => stored,
  });
  const result = await save({
    productId: '18',
    fields: { vonHalskyOfferId: 'VH-18' },
    mutationId: 'von-halsky-link-18',
  });
  assert.equal(operations.length, 2);
  assert.equal(result.repaired, true);
  assert.equal(result.recoveredMutationId, 'von-halsky-link-18');
  assert.match(result.mutationId, /^von-halsky-link-18:repair:/);
  assert.equal(result.product.vonHalskyOfferId, 'VH-18');
  assert.equal(operations[1].expectedFields.vonHalskyOfferId.present, false);
});
...
test('sukces wystawienia Allegro kończy się dopiero po publikacji w centralnej kartotece', async () => {
  ...
  assert.match(source, /verifiedOffer,\s*imagePublication,\s*catalogRecovery,\s*expectedStatus/);
});
