...
test('aktywne zamówienie Von Halsky trafia do tego samego Planu producenta', async () => {
  const repo = memoryRepository({
    orders: { items: [] },
    settings: { rev: 1, data: {
      artway_stany: { 106: 0 },
      artway_produkty_dodane: [{ id: 106, nazwa: 'Ale Pary', producent: 'Alexander', ean: '5906018023510', externalId: '2351' }],
      artway_agent_ai_zlecenia: [],
    } },
  });
  const workflow = service(repo, {
    readVonHalskyOrders: async () => [{
      id: 'C09Q3VF', status: 'ACCEPTED',
      orderLines: [{ quantity: 2, offer: { externalId: '2351', product: { productId: '106', ean: '5906018023510', sku: '2351', name: 'Ale Pary' } } }],
    }],
  });
  const result = await workflow.reconcileDrafts();
  const line = repo.records.get('settings').data.artway_agent_ai_zlecenia[0].pozycje[0];
  assert.equal(result.diagnostics.vonHalsky.activeOrders, 1);
  assert.equal(line.produktId, '106');
  assert.equal(line.ilosc, 2);
