...
test("pulpit ogranicza ponawianie odczytów podczas chwilowej awarii serwera",async()=>{
  const dashboard=await read("src/frontend/19-admin-dashboard.js");
  assert.match(dashboard,/adminPulpitOdswiezenieTimer/);
  assert.match(dashboard,/adminPulpitNieudaneOdczyty/);
  assert.match(dashboard,/Math\.min\(5\*60\*1000,15000\*\(2\*\*/);
  assert.match(dashboard,/if\(adminPulpitOdswiezaniePromise\|\|adminPulpitOdswiezenieTimer\)return/);
  assert.match(dashboard,/wyniki\.every\(result=>result\.status==="fulfilled"&&result\.value!==false\)/);
});
