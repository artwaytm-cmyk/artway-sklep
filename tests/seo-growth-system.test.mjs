...
test('raport produktów dołącza aktualne nazwy i pomija kartoteki spoza katalogu', () => {
  const report = { products: [
    { productId: '1000090', views: 3, carts: 1 },
    { productId: 'usuniety', views: 8, carts: 0 },
  ] };
  const result = seoAnalyticsInternals.enrichPerformanceProducts(report, [{
    id: '1000090', nazwa: 'GLUTY - Gra karciana', zdjecie: 'https://example.test/gluty.jpg', externalId: '1397', ean: '5906018013977',
  }]);
  assert.equal(result.products.length, 1);
  assert.equal(result.products[0].productName, 'GLUTY - Gra karciana');
  assert.equal(result.products[0].productImage, 'https://example.test/gluty.jpg');
  assert.equal(result.products[0].productEan, '5906018013977');
  assert.equal(result.historicalProductsExcluded, 1);
});
...
test('mapa, feed, RSS i panel wykorzystują komplet bezpłatnych mechanizmów', async () => {
  const [sitemap, feed, rss, frontendCore, frontendReport, analytics, timer, service] = await Promise.all([
    readFile('src/backend/sitemap.mjs', 'utf8'),
    readFile('src/backend/google-products.mjs', 'utf8'),
    readFile('src/backend/products-rss.mjs', 'utf8'),
  ...
  assert.match(feed, /g:sale_price/);
  assert.match(feed, /g:shipping/);
  assert.match(rss, /application\/rss\+xml/);
  assert.match(frontend, /RSS produktów/);
