...
test('końcowe potwierdzenie jakości jest idempotentne przy ponowieniu tego samego zadania', async () => {
  const store = repository();
  const product = {
    id: 'P-QUALITY',
    contentEditorial: {
      ...
      channelStates: {
        store: { status: 'ready', preparedAt: '2026-08-06T00:00:00.000Z' },
        allegro: { status: 'ready', preparedAt: '2026-08-06T00:00:01.000Z' },
        vonHalsky: { status: 'ready', preparedAt: '2026-08-06T00:00:02.000Z' },
      },
    },
  };
  const mutationPayloads = new Map();
  const system = createAgentEventSystem({ ...store });
  const finisher = system.vonHalskyFinisher({
    route: async () => new Response(JSON.stringify({
      ok: true,
      results: [{ productId: 'P-QUALITY', status: 'ready', readbackConfirmed: true, runId: 'vh-run-1' }],
    }), { status: 200, headers: { 'content-type': 'application/json' } }),
    getProduct: async () => structuredClone(product),
    saveProductFields: async ({ fields, mutationId }) => {
      const serialized = JSON.stringify(fields);
      if (mutationPayloads.has(mutationId) && mutationPayloads.get(mutationId) !== serialized) {
        throw Object.assign(new Error('payload conflict'), { code: 'catalog_mutation_payload_conflict' });
      }
      mutationPayloads.set(mutationId, serialized);
      Object.assign(product, structuredClone(fields));
      return { product: structuredClone(product), publication: { readbackConfirmed: true } };
    },
  });
  const task = { id: 'task-quality-1', productId: 'P-QUALITY', savedFields: ['nazwa'] };

  const first = await finisher(task);
  const second = await finisher(task);
  const retryWithNewConfirmedFields = await finisher({ ...task, attempt: 2, savedFields: ['nazwa', 'opis'] });

  assert.equal(first.qualityConfirmed, true);
  assert.equal(second.qualityConfirmed, true);
  assert.equal(retryWithNewConfirmedFields.qualityConfirmed, true);
  assert.equal(mutationPayloads.size, 2);
  assert.ok([...mutationPayloads.keys()].every((key) => /:attempt-[12]$/.test(key)));
});
