...
test("główne menu administratora jest pogrupowane według procesu pracy",async()=>{
  ...
  for(const group of ["Sprzedaż i klienci","Realizacja i finanse","Katalog i rozwój","Ustawienia systemu"])assert.match(source,new RegExp(group));
  assert.ok(source.indexOf('"/admin/zamowienia"')<source.indexOf('"/admin/wysylki"'));
  assert.ok(source.indexOf('"/admin/wysylki"')<source.indexOf('"/admin/asortyment"'));
  assert.ok(source.indexOf('"/admin/asortyment"')<source.indexOf('"/admin/personalizacja"'));
});
...
test("grupy menu można zwijać, a aktywna trasa i liczniki pozostają widoczne",async()=>{
  ...
  assert.match(source,/artway_admin_menu_grupy/);
  assert.match(source,/grupa\.id==="system"/);
  ...
  assert.match(responsive,/\.admin-nav-group\.collapsed \.admin-nav-items\{display:contents\}/);
});
