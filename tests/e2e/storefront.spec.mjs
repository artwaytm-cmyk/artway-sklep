...
test('edytor otwiera po ID także produkt administracyjny spoza publicznego katalogu', async ({ page }) => {
  const productId = 'E2E-EDITOR-ONLY';
  await loginAdmin(page);
  await page.route('**/api/store**', async (route) => {
    const url = new URL(route.request().url()), action = url.searchParams.get('action');
    if (action === 'product-catalog-item' && url.searchParams.get('id') === productId) {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
          ...
          product: {
            id: productId,
            nazwa: 'Produkt wyłącznie administracyjny',
            cena: 39.9,
            kategoria: 'Gry',
            producent: 'Alexander',
            manufacturerProfileId: 'e2e-profile',
            _catalog: { recordStatus: 'active', detailLevel: 'full' },
          },
        }),
      });
      return;
  ...
  await page.goto(`/#/admin/produkty/edytuj/${productId}`);
  await expect(page.locator('form.product-editor-form')).toBeVisible({ timeout: 20_000 });
  await expect(page.locator('form.product-editor-form [name="nazwa"]')).toHaveValue('Produkt wyłącznie administracyjny');
  await expect(page.locator(`form.product-editor-form[data-product-id="${productId}"]`)).toHaveCount(1);
});
...
test('Centrum wysyłki udostępnia książkę adresową i wycenę InPost przed nadaniem', async ({ page }) => {
  ...
  await page.route('**/api/store**', async (route) => {
    ...
    if (action === 'inpost-service-shipments') {
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          serviceAvailability: { locker: true, courier: true, services: ['inpost_locker_standard', 'inpost_courier_standard'] },
          settings: {
            commissionGross: 4,
            ...
            sender: { companyName: 'Artway-TM', email: 'sklep@example.test', phone: '501002003', address: { street: 'Lipowa', buildingNumber: '7', postCode: '84-207', city: 'Bojano' } },
            priceList: {},
          },
          addressBook: [],
          items: [],
    ...
    if (action === 'inpost-service-quote') {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          testOnly: true,
          created: false,
          pricing: { available: true, complete: true, totalGross: 32.3, customerTotalGross: 40.3, commissionGross: 8, commissionTier: { upToGross: 40, commissionGross: 8, index: 2, overflow: false }, currency: 'PLN', source: 'shipx', breakdown: {}, unpricedOptions: [], apiComparison: { totalGross: 32.3 } },
        }),
      });
      return;
    }
    if (action === 'inpost-service-create') {
      createCalls += 1;
      await route.fulfill({ status: 500, contentType: 'application/json', body: JSON.stringify({ ok: false, error: 'Test nie może tworzyć przesyłki' }) });
      return;
    }
    await route.fallback();
  });
  ...
  await expect(page.getByRole('heading', { name: 'Przesyłka InPost', exact: true })).toBeVisible();
  await expect(page.locator('#inpostServiceForm')).toBeVisible();
  await expect(page.getByText('Klient zlecający — opcjonalnie')).toBeVisible();
  await expect(page.locator('#inpostServiceForm').getByRole('button', { name: /Wybierz (klienta|z książki)/ })).toHaveCount(2);
  await expect(page.getByRole('button', { name: '↻ Przelicz' })).toBeVisible();
  await expect(page.locator('#inpostServiceForm [name="sendingMethod"][value="parcel_locker"]')).toBeChecked();
  await expect(page.locator('#inpostServiceForm [name="dropoffPoint"]')).toHaveValue('BOJ01N');
  await expect(page.getByText('Przy tworzeniu paczki nie wysyłamy pola nadawcy.')).toHaveCount(0);
  await expect(page.getByText('Firma i NIP są wymagane tylko dla faktury miesięcznej.')).toBeVisible();
  await expect(page.getByRole('button', { name: 'Przy adresie' })).toBeVisible();
  await expect(page.locator('#inpostServiceForm .inpost-courier-extras')).toBeHidden();
  await page.locator('.inpost-main-actions').getByRole('link', { name: '⚙️ Ustawienia InPost' }).click();
  await expect(page).toHaveURL(/#\/admin\/wysylki\/inpost-ustawienia$/);
  await expect(page.getByRole('heading', { name: 'Ustawienia InPost', exact: true })).toBeVisible();
  const settingsPanel = page.locator('.inpost-service-settings');
  await expect(settingsPanel).toBeVisible();
  await expect(settingsPanel.getByRole('heading', { name: 'Progi mojej prowizji' })).toBeVisible();
  await expect(settingsPanel.getByLabel('Domyślny sposób nadania')).toHaveValue('parcel_locker');
  await expect(settingsPanel.getByLabel('Domyślny automat nadawczy')).toHaveValue('BOJ01N');
  await expect(settingsPanel.locator('[data-inpost-commission-tier]')).toHaveCount(4);
  await expect(settingsPanel.locator('.inpost-tier-remove')).toHaveCount(0);
  await settingsPanel.getByRole('button', { name: '＋ Dodaj kolejny próg' }).click();
  await expect(settingsPanel.locator('[data-inpost-commission-tier]')).toHaveCount(5);
  await settingsPanel.getByRole('button', { name: 'Usuń próg' }).click();
  await expect(settingsPanel.locator('[data-inpost-commission-tier]')).toHaveCount(4);
  await expect(settingsPanel.getByText('Powyżej ostatniego progu obowiązuje ostatnia prowizja.')).toBeVisible();
  await expect(settingsPanel.getByText(/Cennik InPost/)).toBeVisible();
  await settingsPanel.getByText(/Dane Artway‑TM/).click();
  await expect(settingsPanel.getByText('Przy tworzeniu paczki nie wysyłamy pola nadawcy.')).toBeVisible();
  if (process.env.ARTWAY_VISUAL_CAPTURE) await page.screenshot({ path: '/tmp/artway-inpost-settings-desktop.png', fullPage: true });
  await page.goto('/#/admin/wysylki/inpost');
  await expect(page.locator('.inpost-service-settings')).toHaveCount(0);
  const shipmentForm = page.locator('#inpostServiceForm');
  await expect(shipmentForm.getByRole('button', { name: /Test bez tworzenia/ })).toBeVisible();
  await expect(shipmentForm.getByRole('button', { name: /Utwórz prawdziwą przesyłkę/ })).toBeVisible();
  const customerCard = page.locator('#inpost-party-customer');
  await expect(customerCard.locator('details')).not.toHaveAttribute('open', '');
  const customerInputs = customerCard.locator('input:not([type="hidden"]):not([type="checkbox"])');
  expect(await customerInputs.count()).toBe(11);
  for (let index = 0; index < await customerInputs.count(); index += 1) {
    await expect(customerInputs.nth(index)).not.toHaveAttribute('required', '');
  }
  ...
  await expect(shipmentForm.locator('.inpost-courier-extras')).toBeVisible();
  ...
  await expect(shipmentForm.locator('.inpost-courier-extras')).toBeHidden();
  ...
  await customerCard.getByRole('button', { name: /Wybierz klienta/ }).click();
  const addressBook = page.locator('#inpostAddressBookModal');
  await expect(addressBook.getByRole('heading', { name: 'Wybierz klienta zlecającego' })).toBeVisible();
  await expect(addressBook.getByRole('button', { name: 'Klienci zlecający' })).toBeVisible();
  await expect(addressBook.getByRole('button', { name: 'Odbiorcy' })).toBeVisible();
  await addressBook.getByPlaceholder(/Firma, osoba/).fill('Klient testowy');
  await expect(addressBook.getByRole('button', { name: /Klient testowy/ })).toBeVisible();
  await addressBook.getByRole('button', { name: /Klient testowy/ }).click();
  ...
  await expect(customerCard.getByLabel(/Miasto/)).toHaveValue('Bojano');
  await expect(customerCard.getByLabel('Ulica', { exact: true })).toHaveValue('Lipowa');
  await customerCard.getByRole('button', { name: /Nowy klient/ }).click();
  await shipmentForm.getByLabel('E-mail *', { exact: true }).fill('odbiorca@example.test');
  await shipmentForm.getByLabel('Telefon *', { exact: true }).fill('501002003');
  await shipmentForm.locator('[name="targetPoint"]').fill('BOJ01N');
  expect(await page.evaluate(() => inpostServiceMozeWycenic(inpostServicePayload(document.getElementById('inpostServiceForm'))))).toBe(true);
  await page.waitForTimeout(800);
  const quotesBeforeTest = quoteBodies.length;
  await shipmentForm.getByRole('button', { name: /Test bez tworzenia/ }).evaluate((button) => inpostServiceTestuj(button.form, button));
  await expect.poll(() => quoteBodies.length).toBeGreaterThan(quotesBeforeTest);
  await expect(shipmentForm.locator('[data-inpost-test-result]')).toContainText('Nie utworzono przesyłki, etykiety ani numeru nadania.');
  await expect(shipmentForm.locator('[data-inpost-pricing]')).toContainText('32,30 zł');
  await expect(shipmentForm.locator('[data-inpost-pricing]')).toContainText('8,00 zł');
  await expect(shipmentForm.locator('.inpost-tier-badge')).toContainText('do 40,00 zł');
  expect(quoteBodies.length).toBeGreaterThan(0);
  expect(quoteBodies.at(-1).customer).toMatchObject({ companyName: '', firstName: '', lastName: '', email: '', phone: '' });
  expect(createCalls).toBe(0);
  if (process.env.ARTWAY_VISUAL_CAPTURE) await page.screenshot({ path: '/tmp/artway-inpost-service-desktop.png', fullPage: true });
  await page.setViewportSize({ width: 390, height: 844 });
...
test('potwierdzenie nadania jest krótkie, pokazuje koszt, a historia otwiera się osobno', async ({ page }) => {
  const assertRuntime = observeRuntime(page);
  await loginAdmin(page);
  await page.goto('/#/admin/wysylki/inpost');
  await expect(page.getByRole('heading', { name: 'Wysyłka z InPost', exact: true })).toBeVisible();
  ...
  await page.evaluate(() => {
    inpostServiceStan.items = [{
      ...
      pricing: { totalGross: 14.16, customerTotalGross: 18.16 },
      trackingHistory: [
  ...
  await expect(popup.getByRole('heading', { name: 'Potwierdzenie nadania' })).toBeVisible();
  await expect.poll(() => popup.locator('.head').evaluate(element => getComputedStyle(element).display)).toBe('flex');
  await expect(popup.getByText('620000000000000000000001')).toBeVisible();
  await expect(popup.getByText('Nadanie w automacie Paczkomat®', { exact: true })).toHaveCount(1);
  await expect(popup.getByText('BOJ01N', { exact: true })).toBeVisible();
  await expect(popup.getByText('GDA01N', { exact: true })).toBeVisible();
  await expect(popup.getByText('Odbiorca', { exact: true })).toBeVisible();
  await expect(popup.getByText('Koszt nadania', { exact: true })).toBeVisible();
  await expect(popup.getByText('18,16 zł', { exact: true })).toBeVisible();
  await expect(popup.getByText('Gotowa do odbioru', { exact: true })).toHaveCount(0);
  await expect(popup.getByRole('heading', { name: 'Historia przesyłki' })).toHaveCount(0);
  await expect(popup.getByText('Dane Artway‑TM')).toHaveCount(0);
  await expect(popup.getByRole('button', { name: 'Drukuj / zapisz PDF' })).toBeVisible();
  await expect(popup.getByText(/Profesjonalne potwierdzenie/)).toHaveCount(0);
  if (process.env.ARTWAY_VISUAL_CAPTURE) await popup.screenshot({ path: '/tmp/artway-inpost-confirmation.png', fullPage: true });
  await popup.close();
  const historyPopupPromise = page.waitForEvent('popup');
  await page.evaluate(() => inpostServiceHistoriaPrzesylki('IPS-TEST'));
  const historyPopup = await historyPopupPromise;
  await expect(historyPopup.getByRole('heading', { name: 'Historia przesyłki' })).toBeVisible();
  await expect(historyPopup.getByText('Gotowa do odbioru', { exact: true })).toHaveCount(2);
  await historyPopup.close();
  assertRuntime();
});

test('osobna podstrona odbioru kuriera nie wysyła zlecenia bez dodatkowego potwierdzenia', async ({ page }) => {
  const assertRuntime = observeRuntime(page);let pickupCalls=0;
  await page.route('**/api/store**', async route => {
    const action=new URL(route.request().url()).searchParams.get('action');
    if(action==='inpost-service-pickup'){pickupCalls+=1;await route.fulfill({status:500,contentType:'application/json',body:JSON.stringify({ok:false,error:'Test nie zamawia kuriera'})});return;}
    if(action==='inpost-service-shipments'){await route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({ok:true,serviceAvailability:{locker:true,courier:true},settings:{defaultSendingMethod:'parcel_locker',defaultDropoffPoint:'BOJ01N',commissionTiers:[{upToGross:20,commissionGross:4},{upToGross:30,commissionGross:6},{upToGross:40,commissionGross:8},{upToGross:50,commissionGross:10}],sender:{},priceList:{}},addressBook:[],billing:{groups:[]},items:[{id:'IPS-COURIER',reference:'USL-KURIER',inpostId:'SHIP-1',trackingNumber:'620000000000000000000002',status:'label_ready',inpostStatus:'confirmed',labelReady:true,sendingMethod:'dispatch_order',deliveryType:'locker',receiver:{firstName:'Jan',lastName:'Klient',address:{city:'Gdańsk'}},pickup:null}]})});return;}
    await route.fallback();
  });
  await loginAdmin(page);await page.goto('/#/admin/wysylki/odbior-kuriera');
  await expect(page.getByRole('heading',{name:'Zamów kuriera InPost',exact:true})).toBeVisible();
  await expect(page.getByRole('heading',{name:'Odbiór kuriera InPost',exact:true})).toBeVisible();
  await expect(page.getByText('USL-KURIER')).toBeVisible();
  page.once('dialog',dialog=>dialog.dismiss());
  await page.getByRole('button',{name:'🚚 Zamów kuriera'}).click();
  await expect.poll(()=>pickupCalls).toBe(0);assertRuntime();
});
...
test('InPost Von Halsky ma osobny katalog sprzedaży i nie miesza się z nadawaniem paczek', async ({ page }) => {
  ...
  await expect(page.locator('[data-vh-channel-truth]')).toContainText('Po stronie API');
  ...
  await expect(page.locator('.von-halsky-publication-bar')).toContainText('Agent przygotowuje • administrator zleca publikację');
