...
test('instrukcja Agenta rozpoznaje techniczną cenę ShipX dla konta postpaid', async () => {
  const { DIAGNOSTIC_AGENT_INSTRUCTIONS } = await import('../src/backend/lib/domain/diagnostic-agent-workflow.mjs');
  assert.match(DIAGNOSTIC_AGENT_INSTRUCTIONS, /postpaid_calculation_not_contractual/);
  assert.match(DIAGNOSTIC_AGENT_INSTRUCTIONS, /nie użyła go do rozliczenia/);
});
