...
test('karta produktu korzysta ze struktury wzorca bez kopiowania jego treści', async () => {
  ...
  assert.match(script, /catalog-product-classification/);
  assert.match(script, /catalog-product-operational-data/);
  assert.match(script, /catalog-product-actions/);
  assert.doesNotMatch(card, /<progress/);
  assert.doesNotMatch(card, /catalog-product-readiness/);
  ...
  assert.match(css, /@container\(max-width:520px\)\{\.catalog-product-card \.catalog-product-values\{grid-template-columns:minmax\(0,1fr\)/);
});
