...
test('niedobór po rzeczywistym wydaniu pozostaje w Planie do czasu przyjęcia PZ', () => {
  const result = run({
    orders: [],
    products: [product(1)],
    settings: {
      ...
      artway_magazyn_niedobory_wydan: {
        1: { quantity: 2, orders: [{ orderNumber: 'Von Halsky C09Q3VF', quantity: 2 }] },
      },
    },
  });
  const line = result.activeDrafts[0].pozycje[0];
  assert.equal(line.rezerwacje, 0);
  assert.equal(line.legacyShortage, 2);
  assert.equal(line.iloscPotrzebna, 2);
  assert.deepEqual(line.zamowienia, ['Von Halsky C09Q3VF']);
  assert.equal(result.diagnostics.inventoryShortfallProducts, 1);
  assert.equal(result.diagnostics.inventoryShortfallQuantity, 2);
});

test('niedobór zamkniętego zamówienia pozostaje w audycie, ale znika z Planu zatowarowania', () => {
  const result = run({
    orders: [],
    products: [product(1)],
    settings: {
    ...
    closedOrderReferences: ['Von Halsky C09Q3VF'],
  });
  assert.deepEqual(result.activeDrafts, []);
  assert.deepEqual(result.shortages, []);
  assert.equal(result.diagnostics.skippedClosedShortfallQuantity, 2);
});
