...
test("Agent AI ma jeden katalog zdarzeniowy z raportem kanałów",async()=>{
  ...
  for(const route of ["rozmowa","praca","raport","zadania","automatyzacje","audyt"])assert.match(source,new RegExp(`#/admin/agent-ai/${route}`));
  for(const label of ["Centrum","Polecenie","Na żywo","Produkty i kanały","Decyzje","Reguły i role","Historia"])assert.match(source,new RegExp(label));
...
test("stare adresy Agenta zachowują zgodność i prowadzą do scalonych obszarów",async()=>{
  ...
  assert.match(source,/raport:"raport",wyniki:"raport",produkty:"raport",kanaly:"raport"/);
  assert.match(source,/zadania:"zadania",plan:"zadania",zlecenia:"zadania",producenci:"zadania"/);
...
test("każda podstrona renderuje tylko własny zestaw narzędzi",async()=>{
  ...
  assert.match(source,/if\(active==="raport"\)return agentAIProductReportPageHTML\(\)/);
...
test("Praca Agenta i raport kanałów odświeżają dane co 10 sekund bez uruchamiania cyklu",async()=>{
  ...
  assert.match(workspace,/\["pulpit","praca","raport"\]\.includes\(active\)/);
  assert.match(runtime,/chmura\("agent-runtime-status"/);
  assert.match(runtime,/chmura\("agent-product-report"/);
  assert.match(runtime,/setTimeout\(async\(\)=>\{await agentAIRuntimePobierz\(true\);agentAIRuntimePolling\(\);\},10000\)/);
