...
test("publikacja działa jednym kliknięciem, ale zachowuje kontrolę Agenta i blokadę braków",async()=>{
  const listing=(await read("src/frontend/12b-allegro-listing-workspace.js"))+(await read("src/frontend/12c-commerce-catalog-actions.js")),actions=await read("src/frontend/12a-product-actions.js"),worker=await read("scripts/run-agent-panel-worker.mjs");
  assert.match(listing,/allegroBrakiProduktuDoWystawienia/);
  assert.match(listing,/asortymentUruchomAgenta\(ids,"product-full-review"\)/);
  assert.match(listing,/currentStages=document\.querySelector\("\[data-allegro-stage-filters\]"\)/);
  ...
  assert.match(worker,/approval:\s*\{/);
  assert.match(worker,/approved: true/);
  assert.match(worker,/operationId: taskId/);
  assert.match(actions,/eligible=eligibleAll,remaining=0/);
...
test("partia przygotowania zapisuje produkty pojedynczo, a publikacja wymaga odczytu kontrolnego serwera",async()=>{
  const actions=await read("src/frontend/12a-product-actions.js"),worker=await read("scripts/run-agent-panel-worker.mjs"),backend=await read("src/backend/lib/store-app.mjs");
  ...
  assert.match(actions,/allegro-publication-queue-enqueue/);
  assert.match(worker,/allegro-publication-queue-claim/);
  assert.match(worker,/allegro-publication-queue-complete/);
  assert.match(worker,/allegro-create-product-offer/);
  assert.match(backend,/publicationConfirmed/);
  assert.match(worker,/readbackConfirmed/);
  ...
  assert.doesNotMatch(publicationBlock,/asortymentPrzygotujProduktDoAllegro\(/);
  assert.doesNotMatch(publicationBlock,/for\(const sourceProduct of products\)/);
  assert.match(publicationBlock,/setTimeout\(\(\)=>\{location\.hash="#\/admin\/agent-ai\/praca"/);
});
...
test("naprawy Allegro pokazują jedną aktualną kolejkę PostgreSQL zamiast starej ściany kart",async()=>{
  const legacy=await read("src/frontend/11-allegro-operations.js"),current=await read("src/frontend/12c-commerce-catalog-actions.js"),actions=await read("src/frontend/12a-product-actions.js"),sync=await read("src/frontend/03-cloud-sync.js"),styles=await read("src/styles/07-admin-domains.css");
  ...
  assert.match(legacy,/function allegroZadaniaAgentaOfertHTML\(\)\{return "";\}/);
  assert.doesNotMatch(current,/allegroZadaniaAgentaOfertHTML/);
...
test("proces wystawiania ma cztery etapy jak Von Halsky i blokuje duplikat przy niezweryfikowanym ID",async()=>{
  ...
  assert.match(listing,/DOPASOWANIE/);
  assert.match(listing,/UZUPEŁNIENIE/);
  ...
  assert.match(styles,/\.allegro-publication-steps-four/);
  assert.match(styles,/\.allegro-publication-table/);
  assert.match(styles,/\.allegro-publication-card\.verify/);
});

test("Allegro nie ma już starego ekranu awaryjnego, a trwała kolejka pokazuje cztery etapy i czytelny stan dostawcy AI",async()=>{
  const legacy=await read("src/frontend/11-allegro-operations.js"),listing=await read("src/frontend/12b-allegro-listing-workspace.js"),workspace=await read("src/frontend/12c-commerce-catalog-actions.js"),styles=await read("src/styles/27-allegro-listing-workspace.css");
  const legacyPanel=legacy.slice(legacy.indexOf("function allegroWystawianiePanelHTML"));
  assert.match(legacyPanel,/Pobieram aktualne centrum Allegro/);
  assert.doesNotMatch(legacyPanel,/Operacje na ofertach Allegro/);
  assert.doesNotMatch(legacyPanel,/Agent: dodaj szkic/);
  assert.doesNotMatch(legacyPanel,/warehouse-worktable-wrap/);
  assert.match(listing,/function allegroPublikacjaPostepSerweraHTML/);
  assert.match(listing,/TRWAŁY PROCES SERWEROWY/);
  for(const marker of ["Dobór pracy","Agent","Zapis centralny","Publikacja API"]){
    assert.match(listing,new RegExp(marker));
  }
  assert.match(listing,/Dostęp AI jest wstrzymany/);
  assert.match(listing,/Odczytuję stan przygotowania produktów/);
  assert.match(listing,/Brak aktywnej partii przygotowania/);
  assert.match(listing,/asortymentSprawdzKolejkeSerwera\(\{render:true\}\)/);
  assert.doesNotMatch(workspace,/data-allegro-publication-center[^\n]+product-agent-progress/);
  assert.match(styles,/\.allegro-server-preparation-progress/);
  assert.match(styles,/\.allegro-current-workspace-loading/);
});

test("panel procesu jest pod tabelą i pozwala trwale sterować kolejką serwerową",async()=>{
  const listing=await read("src/frontend/12b-allegro-listing-workspace.js"),workspace=await read("src/frontend/12c-commerce-catalog-actions.js"),actions=await read("src/frontend/12a-product-actions.js"),route=await read("src/backend/lib/allegro-preparation-route.mjs"),queue=await read("src/backend/lib/domain/allegro-preparation-postgres-queue.mjs"),styles=await read("src/styles/27-allegro-listing-workspace.css");
  const tableAt=workspace.indexOf('class="admin-standard-table admin-responsive-table allegro-publication-table"'),progressAt=workspace.indexOf('${serverProgress}');
  assert.ok(tableAt>=0&&progressAt>tableAt,"postęp ma być renderowany po tabeli");
  for(const label of ["Wstrzymaj","Wznów","Anuluj wcześniejsze","Anuluj bieżące","Agent AI"])assert.match(listing,new RegExp(label));
  assert.match(actions,/function asortymentSterujKolejkaSerwera/);
  assert.match(actions,/allegro-preparation-queue-control/);
  assert.match(route,/queue\.pause/);
  assert.match(route,/queue\.resume/);
  assert.match(route,/queue\.cancel/);
  assert.match(route,/queue\.cancelPrevious/);
  assert.match(route,/defaultPriority: 200_000/);
  assert.match(route,/priorityReason: 'administrator_manual_selection'/);
  assert.match(queue,/blocked_reason='admin_paused'/);
  assert.match(queue,/status='cancelled'/);
  assert.match(queue,/administrator_cancelled_previous_pending_task/);
  assert.match(styles,/\.allegro-server-progress-controls/);
  assert.match(styles,/\.allegro-server-preparation-progress\.is-paused/);
});

test("kolejność dynamicznego ładowania modułów nie może przywrócić starego renderera Allegro",async()=>{
  const base=await read("src/frontend/12b-allegro-listing-workspace.js"),current=await read("src/frontend/12c-commerce-catalog-actions.js"),legacy=await read("src/frontend/11-allegro-operations.js"),workspace=await read("src/frontend/11-allegro-workspace.js");
  assert.match(legacy,/function allegroWystawianiePanelHTML/);
  assert.match(base,/function allegroWystawianieBazowePanelHTML/);
  assert.match(current,/function allegroWystawianieAktualnePanelHTML/);
  assert.doesNotMatch(base+current,/allegroWystawianiePanelHTML\s*=/);
  assert.match(current,/allegroWystawianieAktualnePanelHTML\(\)/);
  assert.match(workspace,/typeof allegroOfertyIPublikacjaHTML==="function"/);
});

test("liczniki producentów i kategorii pochodzą z pełnych facetów serwera",async()=>{
  const workspace=await read("src/frontend/12c-commerce-catalog-actions.js");
  assert.match(workspace,/Number\(item\.count\)/);
  assert.match(workspace,/Wszyscy producenci \(\$\{Number\(serverData\.summary\?\.active\)/);
  assert.match(workspace,/\$\{esc\(item\.value\)\} \(\$\{item\.count\}\)/);
  assert.match(workspace,/Wszystkie kategorie \(\$\{Number\(serverData\.summary\?\.active\)/);
});
