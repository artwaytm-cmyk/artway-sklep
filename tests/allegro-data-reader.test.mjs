...
function readerHarness() {
  ...
  const records = new Map([
    ...
    ['allegro_offers', { items: [{ id: 'offer-1', status: 'ACTIVE' }, { id: 'offer-2', publication: { status: 'INACTIVE' } }], updated_at: 'offers-at' }],
    ['allegro_mappings', { items: { 'offer-1': { productId: 'product-1' }, 'offer-2': { productId: 'product-2', blocked: true }, 'offer-historyczne': { productId: 'product-old' } } }],
  ]);
...
test('podsumowanie Allegro nie przesyła ciężkich rejestrów', async () => {
  ...
  assert.equal(payload.summary.offers.mappedProducts, 1);
  assert.equal(payload.summary.offers.unmapped, 1);
  assert.equal(payload.summary.offers.active, 1);
  assert.equal(payload.summary.offers.inactive, 1);
  assert.deepEqual(payload.summary.offers.statusCounts, { ACTIVE: 1, INACTIVE: 1 });
...
test('zakres zamówień przesyła tylko dane potrzebne tej podstronie', async () => {
  ...
  assert.equal('offers' in payload, false);
  assert.equal('offers' in payload.summary, false);
  assert.equal(Object.keys(payload.mappings).length, 3);
});
