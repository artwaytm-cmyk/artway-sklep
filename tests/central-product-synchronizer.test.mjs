...
function harness({ applied = 1, lock = true, snapshotFingerprint = 'old-fingerprint' } = {}) {
  const calls = [];
...
test('niezmieniona kartoteka nie jest ponownie blokowana ani zapisywana', async () => {
  const runtime = harness({ snapshotFingerprint: 'fingerprint-1' });
  const result = await runtime.synchronize({}, {
    preferCanonicalCatalog: true,
    sourceRevision: 'revision-unchanged',
  });
  assert.equal(result.synchronized, true);
  assert.equal(result.appliedCount, 1);
  ...
  assert.equal(result.skippedCount, 0);
  assert.equal(runtime.calls.some((query) => query.includes('INSERT INTO artway_products(')), false);
});
