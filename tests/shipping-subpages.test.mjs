...
test('Centrum wysyłki ma trwałe adresy wszystkich podstron', async () => {
  ...
  for (const route of ['#/admin/wysylki/inpost','#/admin/wysylki/odbior-kuriera','#/admin/wysylki/tracking','#/admin/wysylki/automatyzacje','#/admin/wysylki/ustawienia']) assert.match(shipping,new RegExp(route.replaceAll('/','\\/')));
...
test('każda podstrona wysyłek ma własny widok i poprawny aktywny stan nawigacji', async () => {
  ...
  for (const panel of ['panelZlecenWysylkowych','panelWysylkiUslugowejInpost','panelOdbioruKurieraInpost','panelTrackinguWysylek','panelAutomatyzacjiWysylek','panelUstawienBramki']) assert.match(shipping,new RegExp(`function ${panel}\\(`));
