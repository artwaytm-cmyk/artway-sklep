...
test('przypisanie produktu zapisuje wyłącznie istniejącą aktywną półkę', async () => {
  const { service, current } = harness();
  const result = await service.assign({ productId: '1', location: 'pak-rb-p01' }, 'magazyn@artway.pl');
  assert.equal(result.assigned, true);
  assert.equal(result.duplicate, false);
  assert.equal(result.previousLocation, 'PAK-RA-P01');
  assert.equal(current().data.artway_magazyn_produkty['1'].lokalizacja, 'PAK-RB-P01');
  assert.equal(current().data.artway_magazyn_produkty['1'].operator, 'magazyn@artway.pl');
  assert.equal(current().data.artway_stany['1'], 8);
});

test('przypisanie odrzuca regał, nieistniejącą półkę i obcy produkt', async () => {
  const { service, current } = harness(), before = current();
  await assert.rejects(() => service.assign({ productId: '1', location: 'PAK-RB' }), (error) => error.code === 'warehouse_location_target_invalid');
  await assert.rejects(() => service.assign({ productId: '1', location: 'PAK-RZ-P99' }), (error) => error.code === 'warehouse_location_target_invalid');
  await assert.rejects(() => service.assign({ productId: '999', location: 'PAK-RB-P01' }), (error) => error.code === 'warehouse_product_not_found');
  assert.deepEqual(current(), before);
});

test('ponowne przypisanie tej samej półki jest idempotentne', async () => {
  const { service, current } = harness(), before = current();
  const result = await service.assign({ productId: '1', location: 'PAK-RA-P01' });
