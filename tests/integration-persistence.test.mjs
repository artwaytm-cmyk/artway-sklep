...
const shippingSource = await fs.readFile(new URL('../src/frontend/07-admin-shipping.js', import.meta.url), 'utf8');
...
test('panel używa odnawialnej sesji administratora zamiast proszenia o token bazy', () => {
  ...
  assert.match(storeSource, /action === 'session-refresh'/);
});
...
test('InPost i SMTP mają rzeczywiste testy serwerowe z trwałą historią zdrowia', () => {
  ...
  assert.match(storeSource, /action === 'inpost-test'[\s\S]*?organizationId/);
