...
test('dokładny GTIN i literówka Alexader potwierdzają katalogowy wariant nazwy', () => {
  const identity = evaluateAllegroCatalogIdentitySignals({
    gtin: '5906018027945',
    candidateGtins: ['5906018027945'],
    nameScore: 0.4,
    productBrand: 'Alexander',
    productBrands: ['Alexander'],
    candidateBrand: 'Alexader',
    productCode: '2794',
    candidateCodes: ['7945'],
  });
  assert.equal(identity.verified, true);
...
test('dokładny GTIN oraz oficjalny skrót Mg potwierdzają produkt Multigra', () => {
  const identity = evaluateAllegroCatalogIdentitySignals({
    gtin: '5903796605648',
    candidateGtins: ['5903796605648'],
    nameScore: 0.4,
    productBrand: 'Multigra',
    productBrands: ['Multigra'],
    candidateBrand: 'Mg',
    productCode: '0564',
    candidateCodes: ['5903796605648'],
  });
  assert.equal(identity.verified, true);
  assert.equal(identity.brandMatch, true);
  assert.equal(identity.brandConflict, false);
