...
import { createImportedProductCatalog, mergeImportedProductSourceRefresh } from '../src/backend/lib/domain/imported-product-catalog.mjs';
...
import { reviewDraft, reviewPatch } from '../src/backend/lib/domain/product-link-import-support.mjs';
...
test('kolejka zachowuje rozpoznane dane arkusza i przekazuje je do przygotowania produktu', async () => {
  let receivedOptions = null;
  const milliwoodUrl = 'https://milliwood.com/pl/puzzle-drewniane-celebrating-kwiaty-w-wazonie';
  const { service } = harness(async (_sourceUrl, options) => {
    receivedOptions = options;
    return { product: product(7, { sourceUrl: milliwoodUrl }) };
  });
  const created = await service.create({ rows: [{
    rowNumber: 2,
    name: 'Puzzle Celebrating',
    url: milliwoodUrl,
    fileData: { nazwa: 'Puzzle Celebrating', producent: 'Milliwood', cena: '59,99 zł', ignored: 'nie zapisuj' },
  }] });

  assert.deepEqual(created.items[0].fileData, { nazwa: 'Puzzle Celebrating', producent: 'Milliwood', cena: 59.99 });
  await service.processNext(created.job.id);
  assert.deepEqual(receivedOptions.fileData, { nazwa: 'Puzzle Celebrating', producent: 'Milliwood', cena: 59.99 });
});

test('dane z arkusza uzupełniają i korygują nową kartotekę bez używania pustych komórek', async () => {
  const sourceUrl = 'https://milliwood.com/pl/puzzle-drewniane-celebrating-kwiaty-w-wazonie';
  const prepare = createProductLinkImportPreparer({
    ...
    inspect: async () => ({
      ...
      confidence: 85,
      product: { nazwa: 'Nazwa ze strony', cena: 999, producent: 'Błędny producent', opis: 'Opis ze strony.', sourceUrl },
    }),
    offerSettings: async () => ({}),
    recognizeProducer: (entry) => entry.producent,
    chooseCategory: () => ({ name: 'Puzzle' }),
    shortDescription: (value) => value,
    text: (value, max = 1000) => String(value ?? '').trim().slice(0, max),
  });
  const result = await prepare(sourceUrl, { fileData: {
    nazwa: 'Drewniane Puzzle – Celebrating',
    producent: 'Milliwood',
    cena: 59.99,
    cenaZakupu: 31.5,
    cenaAllegro: 64.99,
    ...
    vonHalskySafetyInformation: 'Produkt nieodpowiedni dla dzieci poniżej 3 lat.',
    gpsrNazwa: 'Milliwood Sp. z o.o.',
    gpsrAdres: 'ul. Testowa 1, 00-001 Warszawa, Polska',
    gpsrEmail: 'gpsr@milliwood.example',
    opisKrotki: '',
  } });

  assert.equal(result.product.nazwa, 'Drewniane Puzzle – Celebrating');
  assert.equal(result.product.producent, 'Milliwood');
  assert.equal(result.product.marka, 'Milliwood');
  assert.equal(result.product.cena, 59.99);
  assert.equal(result.product.cenaZakupu, 31.5);
  assert.equal(result.product.cenaZakupuPrywatna, true);
  assert.equal(result.product.cenaAllegro, 64.99);
  assert.equal(result.product.cenaVonHalsky, 62.99);
  assert.equal(result.product.parametryZrodla['Minimalny wiek dziecka'], '8 lat');
  assert.equal(result.product.parametryZrodla['Liczba elementów'], '150');
  assert.equal(result.product.parametryZrodla['Materiał'], 'drewno');
  assert.equal(result.product.vonHalskyCategoryId, '33333333-3333-4333-8333-333333333333');
  assert.equal(result.product.vonHalskySafetyInformation, 'Produkt nieodpowiedni dla dzieci poniżej 3 lat.');
  assert.equal(result.product.vonHalskyResponsibleProducer.legalName, 'Milliwood Sp. z o.o.');
  assert.equal(result.product.vonHalskyResponsibleProducer.email, 'gpsr@milliwood.example');
  assert.equal(result.product.opis, 'Opis ze strony.');
  assert.equal(result.product.kategoria, 'Puzzle');
});

test('aktualizacja uzupełnia brakującą cenę z arkusza, ale nie nadpisuje poprawnej ceny', () => {
  assert.equal(mergeImportedProductSourceRefresh({ cena: 0 }, { cena: 59.99 }).cena, 59.99);
  assert.equal(mergeImportedProductSourceRefresh({ cena: 39.9 }, { cena: 59.99 }).cena, 39.9);
});

test('ręczna korekta importu nie gubi cen, parametrów ani danych GPSR', () => {
  const draft = reviewDraft({
    nazwa: 'Puzzle drewniane', cena: 59.99, cenaZakupu: 31.5, cenaVonHalsky: 62.99,
    parametryZrodla: { 'Minimalny wiek dziecka': '8 lat', Materiał: 'drewno' },
    vonHalskyResponsibleProducer: { legalName: 'Milliwood Sp. z o.o.', address: 'Warszawa, Polska', email: 'gpsr@example.test' },
  }, 'https://milliwood.com/pl/puzzle-drewniane');
  assert.equal(draft.cenaZakupu, 31.5);
  assert.equal(draft.minimalnyWiek, '8 lat');
  assert.equal(draft.material, 'drewno');
  assert.equal(draft.gpsrNazwa, 'Milliwood Sp. z o.o.');
  assert.deepEqual(reviewPatch({ cenaAllegro: '64,99', gpsrEmail: 'gpsr@example.test', minimalnyWiek: '8 lat' }), {
    cenaAllegro: 64.99, minimalnyWiek: '8 lat', gpsrEmail: 'gpsr@example.test',
  });
});
...
test('create jest idempotentne dla tego samego zestawu linków i nie tworzy drugich shardów', async () => {
  ...
  const corrected = await service.create({
    ...request,
    fileName: 'poprawiona-cena.xlsx',
    rows: [{ rowNumber: 2, url: url(1), fileData: { cena: '59,99 zł' } }, { rowNumber: 3, url: url(2) }],
  });
  assert.notEqual(corrected.job.id, first.job.id, 'korekta danych arkusza musi tworzyć nową kolejkę zamiast zwracać stary import');
  assert.equal(corrected.items[0].fileData.cena, 59.99);
});
