...
import {
  ...
  centralCatalogProductFingerprint,
...
test('lekka kartoteka zachowuje serwerowe potwierdzenie przygotowania po odświeżeniu', () => {
  ...
  assert.match(prepared.allegroAgentPreparationFingerprint, /^allegro-preparation-v6-/);
...
test('zmiana zweryfikowanego faktu opakowania unieważnia gotowość Allegro', () => {
  const prepared = {
    id: 'packaging-fact', nazwa: 'Zestaw kreatywny', opisKrotki: 'Opis', opis: 'Pełny opis',
    allegroAgentPreparationStatus: 'ready', allegroAgentPreparationMissing: [],
    allegroAgentPreparationVersion: 14,
    parametryProducenta: { wiek: '6+' },
    productPackagingFacts: { minimumAge: '6+', verifiedAt: '2026-08-06T01:00:00.000Z' },
  };
  prepared.allegroAgentPreparationFingerprint = centralAllegroPreparationFingerprint(prepared);
  assert.equal(centralAllegroPreparationCurrent(prepared), true);
  assert.equal(centralAllegroPreparationCurrent({
    ...prepared,
    productPackagingFacts: { minimumAge: '8+', verifiedAt: '2026-08-06T02:00:00.000Z' },
  }), false);
  assert.equal(centralAllegroPreparationCurrent({
    ...prepared,
    productPackagingFacts: { ...prepared.productPackagingFacts, verifiedAt: '2026-08-06T02:00:00.000Z' },
  }), true, 'sam techniczny czas weryfikacji nie powinien uruchamiać ponownej redakcji');
});

test('techniczna rewizja synchronizacji nie zmienia fingerprintu produktu', () => {
  const data = {
    artway_produkty_katalog: [{
      id: 'stable-1', nazwa: 'Stały produkt', cena: 25,
      producent: 'Alexander', ean: '5901234567890',
    }],
  };
  const [before] = centralCatalogBuildRecords(data, { sourceRevision: 'revision-a' });
  const [after] = centralCatalogBuildRecords(data, { sourceRevision: 'revision-b' });
  assert.notEqual(before.data._catalog.sourceRevision, after.data._catalog.sourceRevision);
  assert.equal(before.fingerprint, after.fingerprint);
  assert.equal(
    centralCatalogProductFingerprint(before.data),
    centralCatalogProductFingerprint({
      ...before.data,
      _catalog: { ...before.data._catalog, sourceRevision: 'revision-c', syncedAt: '2030-01-01T00:00:00.000Z' },
    }),
  );
  assert.notEqual(
    centralCatalogProductFingerprint(before.data),
    centralCatalogProductFingerprint({ ...before.data, cena: 26 }),
  );
  assert.equal(
    centralCatalogProductFingerprint({ id: '1', nested: { z: 1, a: 2 } }),
    centralCatalogProductFingerprint({ nested: { a: 2, z: 1 }, id: '1' }),
  );
});
...
test('pełny snapshot katalogu współdzieli jeden odczyt dla równoległych odbiorców', async () => {
  let listReads = 0;
  const pool = {
    async query(sql) {
      if (sql.includes("to_regclass('public.'")) return { rowCount: 0, rows: [] };
      if (sql.includes('SELECT records.product_id') && sql.includes('records.product_id>$2')) {
        listReads += 1;
        return { rowCount: 1, rows: [{ product_id: '17', data: { id: '17', nazwa: 'Gra' } }] };
      }
  ...
  const [first, second] = await Promise.all([
    catalog.listDataMap({ includeTrash: true }),
    catalog.listDataMap({ includeTrash: true }),
  ]);
  assert.equal(first.get('17').nazwa, 'Gra');
  assert.equal(second.get('17').nazwa, 'Gra');
  assert.notEqual(first, second);
  assert.equal(listReads, 1);
});

test('pełny snapshot czyta rozbudowane kartoteki małymi stronami odpornymi na timeout bazy', async () => {
  const source = await readFile('src/backend/lib/domain/central-product-catalog.mjs', 'utf8');
  assert.match(source, /listDataMap = async \(\{ includeTrash = true, pageSize = 200 \}/);
  assert.match(source, /Number\(pageSize\) \|\| 200/);
});

test('kolejka publikacji może pobrać wiele produktów jednym zapytaniem', async () => {
  let batchReads = 0;
  const pool = {
    async query(sql) {
      if (sql.includes("to_regclass('public.'")) return { rowCount: 0, rows: [] };
      if (sql.includes('records.product_id=ANY')) {
        batchReads += 1;
        return { rowCount: 2, rows: [
          { product_id: '17', product: { id: '17', nazwa: 'Gra' } },
          { product_id: '18', product: { id: '18', nazwa: 'Puzzle' } },
        ] };
      }
      return { rowCount: 0, rows: [] };
    },
  ...
  const catalog = createCentralProductCatalog({ pool, namespace: 'test' });
  const products = await catalog.getMany(['17', '18', '17'], { admin: true });
  assert.deepEqual([...products.keys()], ['17', '18']);
...
test('sklep publiczny używa tej samej centralnej paginacji i pobiera szczegół dopiero po wejściu', async () => {
  ...
  assert.match(storefront, /SKLEP_KATALOG_CACHE_LIMIT=SKLEP_KATALOG_PWA\?256:24/);
