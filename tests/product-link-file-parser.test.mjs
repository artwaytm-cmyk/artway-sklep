...
await import('../src/frontend/19a-product-link-file-import-fields.js');
...
test('arkusz Milliwood odczytuje nazwę, producenta i polską cenę regularną do kartoteki', async () => {
  const csv = [
    'Lp.;Nazwa produktu;Producent;Cena (regularna);Link do produktu',
    '1;Drewniane Puzzle – Celebrating – Kwiaty w wazonie 500 el.;Milliwood;59,99 zł;https://milliwood.com/pl/puzzle-drewniane-celebrating-kwiaty-w-wazonie',
    '2;Drewniane Puzzle – Animals – Sowa;Milliwood;129,99 zł;https://milliwood.com/pl/puzzle-drewniane-animals-sowa',
  ].join('\n');
  const result = await parseProductLinksFile(Buffer.from(csv), {fileName:'Milliwood_puzzle_drewniane_linki.csv'});

  assert.equal(result.rows.length, 2);
  assert.deepEqual(result.detectedFields.map((entry) => entry.key), ['nazwa', 'producent', 'cena']);
  assert.deepEqual(result.rows[0].fileData, {
    nazwa: 'Drewniane Puzzle – Celebrating – Kwiaty w wazonie 500 el.',
    producent: 'Milliwood',
    cena: 59.99,
  });
  assert.equal(result.rows[1].fileData.cena, 129.99);
});

test('import rozpoznaje ceny kanałów, parametry produktu i komplet pól GPSR', async () => {
  const csv = [
    'Nazwa produktu;Link do produktu;Cena zakupu;Cena Allegro;Cena Von Halsky;Minimalny wiek dziecka;Liczba elementów;Materiał;Waga produktu;Bezpieczeństwo GPSR;GPSR nazwa producenta;GPSR adres;GPSR e-mail;GPSR telefon',
    'Puzzle drewniane;https://milliwood.com/pl/puzzle-drewniane-test;32,50 zł;69,99 zł;71,99 zł;3 lata;500;drewno;0,8 kg;Używać pod nadzorem osoby dorosłej.;Milliwood sp. z o.o.;ul. Testowa 1, 00-001 Warszawa;gpsr@milliwood.example;+48123456789',
  ].join('\n');
  const result = await parseProductLinksFile(Buffer.from(csv), { fileName: 'pelne-dane.csv' });
  ...
  assert.deepEqual(result.rows[0].fileData, {
    ...
    cenaAllegro: 69.99,
    cenaVonHalsky: 71.99,
    minimalnyWiek: '3 lata',
    liczbaElementow: '500',
    material: 'drewno',
    waga: '0,8 kg',
    vonHalskySafetyInformation: 'Używać pod nadzorem osoby dorosłej.',
    gpsrNazwa: 'Milliwood sp. z o.o.',
    gpsrAdres: 'ul. Testowa 1, 00-001 Warszawa',
    gpsrEmail: 'gpsr@milliwood.example',
    gpsrTelefon: '+48123456789',
  });
});
...
test('pozycje do decyzji mają edytor braków, wybór masowy i bezpieczny zapis serwerowy', () => {
  ...
  assert.match(ui, /Identyfikacja, zdjęcie i opisy/);
  assert.match(ui, /Ceny, parametry i dane Von Halsky \/ GPSR/);
...
test('podgląd importu pokazuje pola odczytane z arkusza i wysyła je do kolejki', () => {
  const ui = readFileSync(new URL('../src/frontend/21-product-link-file-import-ui.js', import.meta.url), 'utf8');
  assert.match(ui, /fileData=raw\?\.fileData/);
  assert.match(ui, /Odczytane dane:/);
  assert.match(ui, /Dane z pliku:/);
  assert.match(ui, /puste komórki nie nadpiszą danych pobranych ze źródła/);
  assert.match(ui, /Producent z pliku/);
  assert.match(ui, /Cena z pliku/);
});
