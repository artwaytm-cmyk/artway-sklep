...
test('potwierdzony katalog Von Halsky usuwa stare wpisy z licznika publikacji Agenta', async () => {
  const store = memoryStore();
  const current = new Date('2026-08-05T00:00:00.000Z');
  const runtime = createAgentRuntime({ ...store, now: () => current });
  await runtime.report({ event: 'work_progress', work: {
    id: 'vh-old-pending', productId: 'P-VH', productName: 'Produkt Von Halsky',
    channel: 'vonHalsky', action: 'publikacja oferty', status: 'waiting_provider',
  } });
  await runtime.report({ event: 'work_progress', work: {
    id: 'allegro-current-pending', productId: 'P-ALLEGRO', productName: 'Produkt Allegro',
    channel: 'allegro', action: 'publikacja oferty', status: 'pending',
  } });
  const state = await runtime.status({}, {}, {
    vonHalsky: { truth: { pending: 0 }, commandSummary: { pending: 0 } },
  });
  assert.deepEqual(state.publication.pending.map((item) => item.id), ['allegro-current-pending']);
  assert.equal(state.publication.counts.waitingProvider, 0);
  assert.equal(state.publication.counts.pending, 1);
});
