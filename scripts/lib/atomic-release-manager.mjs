...
export async function activateRelease(currentLink, releaseDir) {
  ...
  const canonicalLinkParent = await realpath(path.dirname(link));
  const relativeTarget = path.relative(canonicalLinkParent, canonicalTarget) || '.';
  ...
  if (active !== canonicalTarget) throw new Error('Nie udało się potwierdzić aktywnego symlinku wydania.');
  return active;
}
