...
const tables = [
  ...
  'artway_warehouse_records', 'artway_diagnostic_issues',
...
try {
  ...
  await seed.query(`CREATE TABLE artway_warehouse_records(
    ...
    product_id text GENERATED ALWAYS AS (COALESCE(data->>'productId',data->>'id',record_id)) STORED,
    document_status text GENERATED ALWAYS AS (COALESCE(data->>'status','')) STORED,
...
try {
  ...
  assert.equal(status.dedicatedRecords, 5);
