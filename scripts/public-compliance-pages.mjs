...
const shipping = storeShippingConfig({});
...
export const PUBLIC_COMPLIANCE_PAGES = Object.freeze([
  ...
  },
  {
    ...
    body: `<p><strong>Waluta rozliczeń: PLN.</strong> Wszystkie ceny i koszty widoczne w sklepie są cenami brutto.</p><h2>Dostawa na terenie Polski</h2><ul><li data-live-shipping="paczkomat"><strong>Paczkomat® / PaczkoPunkt InPost:</strong> ${money(shipping.locker.price)} zł; klient wybiera punkt na mapie lub liście.</li><li data-live-shipping="kurier"><strong>Kurier InPost:</strong> ${money(shipping.courier.price)} zł pod wskazany adres.</li><li data-live-shipping="weekend"><strong>Paczka w Weekend:</strong> opcjonalna usługa dodatkowa +${money(shipping.weekendPrice)} zł, gdy jest dostępna dla wybranej przesyłki.</li><li data-live-shipping="darmowa"><strong>Darmowa dostawa:</strong> od ${money(shipping.freeFrom)} zł wartości produktów, zgodnie z aktualnym progiem używanym przez koszyk.</li></ul><p data-live-shipping="czas">Deklarowany czas przygotowania przesyłki wynosi ${shipping.dispatchTime} w dni robocze. Do tego należy doliczyć czas przewozu InPost. Przy płatności online przygotowanie rozpoczyna się po pozytywnej autoryzacji. Status i numer nadania przekazujemy e-mailem.</p><h2>Metody płatności</h2><ul><li data-live-shipping="pobranie"><strong>Płatność przy odbiorze:</strong> opłata ${money(shipping.cashOnDeliveryPrice)} zł, jeśli metoda jest dostępna w formularzu.</li><li><strong>Przelew na telefon:</strong> dane i numer zamówienia są pokazywane po jego złożeniu.</li><li><strong>Paynow:</strong> BLIK, szybki przelew lub karta — metoda pojawia się wyłącznie po aktywacji połączenia i jest przekierowaniem do bezpiecznej bramki.</li><li><strong>InPost Pay:</strong> po zawarciu osobnej umowy i uruchomieniu integracji może działać jako dodatkowy, ekspresowy proces zakupu łączący płatność oraz dostawę w aplikacji InPost.</li></ul><p>Ostateczny zestaw metod, wszystkie opłaty oraz całkowita kwota do zapłaty są zawsze widoczne w podsumowaniu przed przyciskiem „Zamówienie z obowiązkiem zapłaty”.</p>`,
  },
  {
    route: 'zwroty',
...
function pageMain(page) {
  return `<main id="widok" tabindex="-1"><div class="page seo-legal-entry"><div class="crumb"><a href="/">Sklep</a> › ${page.heading}</div><article class="panel legal-content"><span class="seo-legal-kicker">ARTWAY-TM • INFORMACJE DLA KUPUJĄCYCH</span><h1>${page.heading}</h1>${nav}${page.body}<p class="seo-legal-updated">Ostatnia aktualizacja: 9 sierpnia 2026 r.</p></article></div></main>`;
}

function schemaForPage(page) {
  const canonical = `https://artwaytm.pl/${page.route}/`;
  const pageType = page.route === 'kontakt' ? 'ContactPage' : 'WebPage';
  return JSON.stringify({ '@context': 'https://schema.org', '@graph': [
    { '@type': pageType, name: page.heading, description: page.description, url: canonical, inLanguage: 'pl-PL', isPartOf: { '@type': 'WebSite', name: 'Artway-TM', url: 'https://artwaytm.pl/' } },
    { '@type': 'BreadcrumbList', itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Sklep', item: 'https://artwaytm.pl/' },
      { '@type': 'ListItem', position: 2, name: page.heading, item: canonical },
    ] },
    { '@type': 'Organization', name: STORE_LEGAL.shortName, legalName: STORE_LEGAL.name, url: 'https://artwaytm.pl/', taxID: STORE_LEGAL.nip, email: STORE_LEGAL.email, telephone: STORE_LEGAL.phone },
  ] }).replace(/</g, '\\u003c');
}
...
export function renderPublicCompliancePage(indexHtml, page) {
  ...
  html = html.replace(/<script id="artway-seo-schema" type="application\/ld\+json">[\s\S]*?<\/script>/i, `<script id="artway-seo-schema" type="application/ld+json">${schemaForPage(page)}</script>`);
