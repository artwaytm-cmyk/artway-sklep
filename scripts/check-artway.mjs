...
const files = [
  ...
  'src/backend/lib/store-data-account-route.mjs',
...
const storeRuntime = [
  ...
  read('src/backend/lib/store-data-account-route.mjs'),
...
const productsRss = read('src/backend/products-rss.mjs');
...
requireMarkers('połączonych assets JS', app, [
  ...
  'Stany, rozmieszczenie i kontrola',
  'Priorytet sprzedaży',
...
if (!app.includes('metrykiAgenta=metrykiAgentaDostepne?agentAIMetrykiScalone():null') || !app.includes('Number(metrykiAgenta.tasks||0)+Number(metrykiAgenta.inlineDecisions||0)') || !app.includes('"/admin/agent-ai": zadaniaAgenta')) {
  fail('assets/app.js: licznik Agent AI musi używać wspólnych metryk aktywnych spraw i bezpośrednich decyzji');
}
...
if (!app.includes('href="#/admin/produkty/dodaj"') || !app.includes('href="#/admin/produkty/z-linku"') || !app.includes('href="#/admin/produkty/z-pliku"')) {
  fail('assets/app.js: dodawanie ręczne, z jednego linku i z pliku musi prowadzić do właściwych osobnych procesów');
}
...
if ((!store.includes("method: 'PATCH', bodyObj: patch, withMeta: true") && !store.includes('writePatch: (bodyObj) => allegroWywolaj')) || !store.includes("'/pricing/offer-fee-preview'") || !store.includes('allegroDescriptionSections = sections')) {
...
requireMarkers('src/backend/google-products.mjs', googleProducts, [
  ...
  'automaticSeo',
  "unavailable ? 'out_of_stock' : 'in_stock'",
  '<g:price>',
  'loadStorefrontSeoCatalog',
  '<g:shipping>',
  'x-artway-items',
...
requireMarkers('src/backend/products-rss.mjs', productsRss, [
  'application/rss+xml',
  'loadStorefrontSeoCatalog',
  '/produkty.rss',
]);
...
for (const file of ['src/backend/store.mjs', 'src/backend/lib/store-app.mjs', 'src/backend/lib/allegro-compliance.mjs', 'src/backend/cron-inpost-sync.mjs', 'src/backend/cron-allegro-orders.mjs', 'src/backend/cron-allegro-communications.mjs', 'src/backend/cron-allegro-offers.mjs', 'src/backend/cron-supplier-availability.mjs', 'src/backend/cron-infakt-sync.mjs', 'src/backend/cron-seo-daily.mjs', 'src/backend/sitemap.mjs', 'src/backend/google-products.mjs', 'src/backend/products-rss.mjs']) {
  try {
