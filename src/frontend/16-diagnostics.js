...
const SYSTEM_CENTRAL_DIAG_CACHE_KEY="artway_postgres_diagnostics_view_v2";
function systemCentralDiagPusty(){return {loaded:false,loading:false,items:[],summary:{total:0,open:0,errors:0,warnings:0,occurrences:0},agent:{configured:false},source:"",version:"",etag:"",updatedAt:"",fetchedAt:"",lastAttemptAt:"",refreshError:"",error:""};}
function systemCentralDiagZCache(){
  try{
    const value=JSON.parse(sessionStorage.getItem(SYSTEM_CENTRAL_DIAG_CACHE_KEY)||"null");
    const age=Date.now()-Date.parse(value?.fetchedAt||"");
    if(!value||value.source!=="postgresql"||!Array.isArray(value.items)||!value.summary||!Number.isFinite(age)||age>24*60*60*1000)return null;
    return {...systemCentralDiagPusty(),...value,loaded:true,loading:false,refreshError:"",error:""};
  }catch(e){return null;}
}
function systemCentralDiagZapiszCache(value){
  try{
    if(value?.source!=="postgresql"||!value.loaded)return false;
    const confirmed={...value};
    delete confirmed.loading;delete confirmed.lastAttemptAt;delete confirmed.refreshError;delete confirmed.error;
    sessionStorage.setItem(SYSTEM_CENTRAL_DIAG_CACHE_KEY,JSON.stringify({...confirmed,loading:false,error:""}));
    return true;
  }catch(e){return false;}
}
let systemCentralDiag=systemCentralDiagZCache()||systemCentralDiagPusty();
...
async function systemPobierzCentralneBledy(force=false){
  ...
  systemCentralDiag={...systemCentralDiag,loading:true,lastAttemptAt:new Date().toISOString(),refreshError:"",error:systemCentralDiag.loaded?"":systemCentralDiag.error};
  try{
    const headers=systemCentralDiag.etag?{"If-None-Match":systemCentralDiag.etag}:{};
    const data=await chmura("diagnostics-central",{params:{status:"all",limit:500},headers,allowNotModified:true,timeout:15000});
    const fetchedAt=new Date().toISOString();
    if(data.notModified){
      systemCentralDiag={...systemCentralDiag,loaded:true,loading:false,fetchedAt,lastAttemptAt:fetchedAt,refreshError:"",error:""};
    }else{
      if(data.source!=="postgresql")throw new Error("Serwer nie potwierdził źródła PostgreSQL");
      systemCentralDiag={...systemCentralDiag,loaded:true,loading:false,items:Array.isArray(data.items)?data.items:[],summary:data.summary||{total:0,open:0,errors:0,warnings:0,occurrences:0},agent:data.agent||{configured:false},source:"postgresql",version:data.version||"",etag:data.etag||"",updatedAt:data.updatedAt||"",fetchedAt,lastAttemptAt:fetchedAt,refreshError:"",error:""};
    }
    systemCentralDiagZapiszCache(systemCentralDiag);
  }catch(error){
    const message=error.message||String(error),lastAttemptAt=new Date().toISOString();
    systemCentralDiag={...systemCentralDiag,loading:false,lastAttemptAt,refreshError:message,error:systemCentralDiag.loaded?"":message};
  }
...
async function systemZlecNapraweKodu(id){
  try{
    const data=await chmura("diagnostics-code-repair-request",{method:"POST",body:{ids:[id]},timeout:20000});
    await systemOdswiezCentralneBledy(true);
    toast(`Agent kodu rozpoczął kontrolowaną naprawę${data.jobId?` • ${data.jobId}`:""}`);
  }catch(error){toast("Nie udało się uruchomić naprawy kodu: "+error.message);}
}
...
function testyDiagnostyczne(){
  ...
  const centralDiag=systemCentralDiag.summary||{},centralDiagChecked=systemCentralDiag.loaded;
  dodaj("Diagnostyka","Centralny rejestr błędów",!centralDiagChecked?"pending":systemCentralDiag.error?"bad":centralDiag.errors?"bad":centralDiag.warnings?"warn":"ok",!centralDiagChecked?"Pobieram potwierdzoną projekcję PostgreSQL":systemCentralDiag.error||`${centralDiag.open||0} otwartych grup • ${centralDiag.errors||0} błędów • ${centralDiag.warnings||0} ostrzeżeń • PostgreSQL, rewizja ${String(systemCentralDiag.version||"—").replace(/\"/g,"")}${systemCentralDiag.loading?" • sprawdzam nowszą rewizję w tle":""}`);
...
function systemCentralnyRejestrHTML(){
  ...
  const naprawaHTML=item=>{const a=item.analysis||{},r=item.repair||{},status=r.status||"idle",kod=a.classification==="application_bug";if(!kod)return"";const aktywna=["queued","running","testing","ready","deploying"].includes(status),etykiety={queued:"oczekuje",running:"Agent poprawia kod",testing:"pełne testy",ready:"gotowa do wydania",deploying:"wdrażanie",completed:"wdrożona i potwierdzona",failed:"naprawa zatrzymana"};return`<div class="diagnostic-code-repair ${status}"><span>🧰</span><div><b>Naprawa kodu: ${esc(etykiety[status]||"gotowa do zlecenia")}</b><small>${esc(r.summary||r.error||"Izolowana poprawka → test regresji → pełne testy → atomowe wydanie.")}</small>${r.commit?`<code>${esc(String(r.commit).slice(0,12))}</code>`:""}</div><button class="btn" onclick="systemZlecNapraweKodu('${esc(item.id)}')" ${aktywna||status==="completed"?"disabled":""}>${status==="failed"?"Ponów naprawę":"Napraw kod Agentem"}</button></div>`};
  const analizaHTML=item=>{const a=item.analysis||{},status=a.status||"idle";if(status==="idle")return"";if(["queued","running"].includes(status))return`<div class="backend-note"><b>Najmocniejszy agent:</b> ${status==="queued"?"oczekuje w kolejce":"analizuje dowody"} • ${esc(a.model||agent.model||"OpenAI")}</div>`;if(status==="failed")return`<div class="backend-note warn"><b>Analiza nieudana:</b> ${esc(a.error||"brak wyniku")}</div>`;return`<details class="backend-note"><summary><b>Analiza AI:</b> ${esc(a.summary||a.classification||"gotowa")} • pewność ${Math.round(Number(a.confidence||0)*100)}%</summary><p><b>Przyczyna:</b> ${esc(a.rootCause||"—")}</p>${a.evidence?.length?`<p><b>Dowody:</b> ${a.evidence.map(esc).join(" • ")}</p>`:""}${a.recommendedActions?.length?`<p><b>Zalecane działania:</b> ${a.recommendedActions.map(x=>esc(x.action)).join(" • ")}</p>`:""}${a.validationPlan?.length?`<p><b>Weryfikacja:</b> ${a.validationPlan.map(esc).join(" • ")}</p>`:""}<small>${esc(a.model||"")} • reasoning ${esc(a.reasoning||"")} • ${esc(systemDataCzas(a.analyzedAt))}</small></details>${naprawaHTML(item)}`};
...
async function uruchomAutotest(){
  ...
  try{
    const urls=[...new Set(produkty.filter(p=>p?.zdjecie).map(p=>String(p.zdjecie)))].slice(0,16);
    const wyniki=await Promise.all(urls.map(url=>new Promise(resolve=>{const img=new Image(),timer=setTimeout(()=>resolve({url,ok:false}),7000);img.onload=()=>{clearTimeout(timer);resolve({url,ok:true});};img.onerror=()=>{clearTimeout(timer);resolve({url,ok:false});};img.src=url;})));
    const bledne=wyniki.filter(x=>!x.ok);dodaj("Dostępność zdjęć produktów",bledne.length?"warn":"ok",bledne.length?`${bledne.length} z ${wyniki.length} próbek wymaga ponownej kontroli`:`${wyniki.length} próbek zdjęć pobrano poprawnie`);
  }catch(error){dodaj("Dostępność zdjęć produktów","warn",error.message||String(error));}
  try{
    const version=document.querySelector('meta[name="artway-version"]')?.content||"dev";
    await zaladujSklepModul("content",version);
    await Promise.all(["account","analytics"].map(modul=>zaladujSklepModul(modul,version)));
    dodaj("Moduły sklepu ładowane na żądanie","ok","Treści, konto i analityka SEO zostały pobrane i wykonane z aktywnego wydania");
  }catch(error){
    dodaj("Moduły sklepu ładowane na żądanie","bad",error.message||String(error));
  }
