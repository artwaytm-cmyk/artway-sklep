...
function formularzProduktu(p, tryb){
  ...
  const ofertaAllegro=maTozsamoscProduktu?allegroOfertaDlaProduktuSklepu(p):null,ofertaAllegroId=String(p.allegroOfferId||ofertaAllegro?.id||"").trim(),ofertaAllegroStatus=String(ofertaAllegro?.status||ofertaAllegro?.publication?.status||"").toUpperCase(),allegroOperational=productEditorKanalStanOperacyjny(p,"allegro"),domyslnaPublikacjaAllegro=ofertaAllegroStatus==="ACTIVE"?"keep":"activate",rentownosc=allegroRentownoscProduktu(p),rentownoscSklep=sklepRentownoscProduktu(p),seoDanePodgladu=seoEfektywneDaneProduktu(p),zapisanyKanal=productEditorPoczatkowyKanal(),aktywnaSekcja=productEditorPoczatkowaSekcja(),aktywnyKanal=["store","allegro","vonHalsky"].includes(aktywnaSekcja)?aktywnaSekcja:zapisanyKanal;
  return `
    <form class="product-editor-form" data-product-id="${esc(p.id||0)}" data-active-channel="${esc(aktywnyKanal)}" data-editor-section="${esc(aktywnaSekcja)}" ${!edycja?`data-product-add-form data-product-duplicate-fingerprint="${esc(kontrolaDodawania.fingerprint)}" oninput="produktDodawanieZmienione(event,this)" onchange="produktDodawanieZmienione(event,this)"`:""} onsubmit="${edycja?`zapiszProduktAdmin(event,${jsArg(p.id)})`:"dodajProdukt(event)"}">
      ${productEditorNaglowekHTML(p,edycja)}
      ${productEditorAutomatyzacjaHTML(p,edycja)}
      ...
      <div class="diag-actions"><a class="btn ghost" href="#/admin/asortyment/oplacalnosc">Otwórz pełny kalkulator marży</a></div></section>
      ...
      <details id="product-editor-source" class="product-editor-section" ${(p.gtin||p.externalId||p.mpn||p.producent||p.marka||p.kolorProduktu||p.rozmiar||p.material)?"open":""}>
        <summary style="cursor:pointer;font-weight:800;font-size:.88rem">🏷️ Producent, identyfikatory i źródła</summary>
        ...
        <div data-product-manufacturer-card>${productEditorProducentProfilHTML(p.manufacturerProfile||{},{confidence:p.manufacturerProfileConfidence})}</div>
        <div class="f-row">
        ...
        ${productEditorZrodlaPomocniczeHTML(p)}
...
function widokAdminProduktEdytuj(id){
  const key=String(id??"").trim(),p=pobierzProduktAdmin(key);
  if(!p){
    const loadError=productEditorPelnaKartotekaBledy.get(key);
    if(loadError)return asortymentSzkielet("produkty", `<div class="panel product-editor-loading"><div class="crumb"><a href="#/admin/produkty">Produkty</a> › Pełna kartoteka</div><h2>Nie udało się otworzyć produktu #${esc(key)}</h2><p>${esc(loadError)}</p><button class="btn" type="button" onclick="productEditorPobierzPelnaKartoteke(${jsArg(key)},{force:true})">↻ Ponów pobranie pełnej kartoteki</button></div>`);
    if(!productEditorPelnaKartotekaWToku.has(key))setTimeout(()=>void productEditorPobierzPelnaKartoteke(key),0);
    return asortymentSzkielet("produkty", `<div class="panel product-editor-loading"><div class="crumb"><a href="#/admin/produkty">Produkty</a> › Pełna kartoteka</div><div class="loading">⏳ Pobieram produkt #${esc(key)} bezpośrednio z centralnej kartoteki…</div><p>Edytor pobiera także produkty administracyjne niewidoczne obecnie w publicznym katalogu sklepu.</p></div>`);
  }
...
function daneProduktuZFormularza(f, id, poprzedni={}){
  ...
  const vhDefinitions=Array.isArray(poprzedni.vonHalskyAttributeDefinitions)?poprzedni.vonHalskyAttributeDefinitions:[];
  if(vhDefinitions.length){
    const attributes={...(poprzedni.vonHalskyAttributes&&typeof poprzedni.vonHalskyAttributes==="object"?poprzedni.vonHalskyAttributes:{})},missing=[];
    for(const definition of vhDefinitions){
      const attributeId=String(definition.id||definition.attributeId||""),expected=String(definition.expectedValue||"").toUpperCase(),required=definition.required===true||["ONE","ONE_OR_MANY"].includes(expected),multiple=definition.multiple===true||["NULL_OR_MANY","ONE_OR_MANY"].includes(expected),name=String(definition.name||definition.label||attributeId);
      if(!attributeId)continue;
      let values=f.getAll(`vonHalskyAttribute_${attributeId}`).flatMap(value=>multiple?String(value||"").split(/[\n,;]+/):[String(value||"")]).map(value=>value.trim()).filter(Boolean);
      values=[...new Set(values)];
      if(values.length)attributes[attributeId]=multiple?values:values[0];else delete attributes[attributeId];
      if(required&&(!values.length||(!multiple&&values.length!==1)))missing.push(name);
    }
    p.vonHalskyAttributes=attributes;
    p.vonHalskyRequiredAttributesMissing=missing;
    p.vonHalskyAttributeDefinitions=vhDefinitions;
    p.vonHalskyAttributesUpdatedAt=new Date().toISOString();
  }
  const vhGpsr={...(poprzedni.vonHalskyResponsibleProducer&&typeof poprzedni.vonHalskyResponsibleProducer==="object"?poprzedni.vonHalskyResponsibleProducer:{})};
  vhGpsr.legalName=String(f.get("vonHalskyGpsrName")||"").trim();
  vhGpsr.address=String(f.get("vonHalskyGpsrAddress")||"").trim();
  vhGpsr.email=String(f.get("vonHalskyGpsrEmail")||"").trim().toLowerCase();
  vhGpsr.phone=String(f.get("vonHalskyGpsrPhone")||"").trim();
  vhGpsr.source="product-editor";vhGpsr.verifiedAt=new Date().toISOString();
  if(vhGpsr.legalName||vhGpsr.address||vhGpsr.email||vhGpsr.phone)p.vonHalskyResponsibleProducer=vhGpsr;else delete p.vonHalskyResponsibleProducer;
  const vhGpsrMissing=[!vhGpsr.legalName&&"nazwa producenta / osoby odpowiedzialnej",!vhGpsr.address&&"adres producenta / osoby odpowiedzialnej",!vhGpsr.email&&"adres e-mail producenta / osoby odpowiedzialnej"].filter(Boolean);
  p.vonHalskyGpsrRequired=p.vonHalskyDoesNotRequireGpsrInfo!==true;
  p.vonHalskyResponsibleProducerMissing=p.vonHalskyGpsrRequired?vhGpsrMissing:[];
  p.vonHalskyResponsibleProducerStatus=!p.vonHalskyGpsrRequired||!vhGpsrMissing.length?"ready":"requires_data";
  for(const field of ["vonHalskySafetyInformation","vonHalskyBatchNumber"]){const value=String(f.get(field)||"").trim();if(value)p[field]=value;else delete p[field];}
  const vhCe=String(f.get("vonHalskyCeMarking")||"");if(vhCe==="true"||vhCe==="false")p.vonHalskyCeMarking=vhCe==="true";else delete p.vonHalskyCeMarking;
  ...
  const auxiliarySources=productEditorZrodlaPomocniczeZFormularza(f,p.sourceUrl||p.producentUrl);if(auxiliarySources.length)p.auxiliarySources=auxiliarySources;else delete p.auxiliarySources;
...
async function zapiszProduktCentralnie(product={},previous={}){
  ...
  const change=produktRoznicaCentralnegoZapisu(product,previous);let liveChange=change,stagedFields={},stagedRemove=[];
  if(!Object.keys(change.fields).length&&!change.remove.length)return product;
  if(typeof siteReleaseStageProductOperations==="function"){
    const release=await siteReleaseStageProductOperations([{productId,fields:change.fields,remove:change.remove}]);
    if(release?.handled){stagedFields=Object.fromEntries(Object.entries(change.fields).filter(([key])=>SITE_RELEASE_PUBLIC_PRODUCT_FIELDS.has(key)));stagedRemove=change.remove.filter(key=>SITE_RELEASE_PUBLIC_PRODUCT_FIELDS.has(key));const live=release.liveOperations?.[0];if(!live)return product;liveChange={fields:live.fields||{},remove:live.remove||[]};}
  }
  const result=await chmura("catalog-product-fields-update",{method:"POST",body:{
    productId,
    fields:liveChange.fields,
    remove:liveChange.remove,
  ...
  const saved=result.product&&String(result.product.id)===productId?result.product:product,next={...saved,...stagedFields};for(const key of stagedRemove)delete next[key];return next;
}
...
async function automatyczniePobierzDaneZrodlaProduktu(p={},options={}){
  ...
  try{
    ...
    const canonicalUrl=s.sourceUrl||s.producentUrl||d.canonicalUrl||url,identity=value=>String(value||"").toLocaleLowerCase("pl-PL").normalize("NFKD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9]+/g," ").trim(),sameProduct=candidate=>{const aEan=String(s.gtin||s.ean||"").replace(/\D/g,""),bEan=String(candidate?.gtin||candidate?.ean||"").replace(/\D/g,""),aCode=identity(sourceCode),bCode=identity(candidate?.kodProducenta||candidate?.numerReferencyjny||candidate?.mpn||candidate?.externalId||candidate?.sku),aName=identity(s.nazwa||p.nazwa),bName=identity(candidate?.nazwa);return !!((aEan&&aEan===bEan)||(aCode&&aCode===bCode)||(aName&&bName&&aName===bName));};
    const discovered=[{url:d.requestedUrl,label:"Pierwotny adres produktu",origin:"agent",verifiedAt:new Date().toISOString()},...(Array.isArray(d.alternatives)?d.alternatives.filter(item=>sameProduct(item?.product)).map(item=>({url:item.url,label:`Dodatkowa karta produktu • ${productEditorHostZrodla(item.url)}`,origin:"agent",verifiedAt:new Date().toISOString()})):[])],auxiliarySources=productEditorZrodlaPomocnicze({...p,auxiliarySources:[...(Array.isArray(p.auxiliarySources)?p.auxiliarySources:[]),...discovered],sourceUrl:canonicalUrl,producentUrl:canonicalUrl});
    ...
    const sourceImages=Number(s.sourceEvidence?.imagePolicyVersion)>=2?[s.zdjecie,...(Array.isArray(s.zdjecia)?s.zdjecia:[])].filter(Boolean):[],force={producentUrl:canonicalUrl,sourceUrl:canonicalUrl,sourceEvidence:s.sourceEvidence||merged.sourceEvidence||null,...(auxiliarySources.length?{auxiliarySources}:{}),...(sourceImages.length?{zdjecie:sourceImages[0],zdjecia:sourceImages.slice(1,16)}:{}),dostepnoscProducenta:s.dostepnoscProducenta||merged.dostepnoscProducenta||"",stanProducenta:s.stanProducenta??merged.stanProducenta??"",stanProducentaDokladny:s.stanProducentaDokladny===true,stanProducentaZrodlo:s.stanProducentaZrodlo||merged.stanProducentaZrodlo||"",producentStatus:s.producentStatus||merged.producentStatus||"",producentSprawdzonoAt:s.producentSprawdzonoAt||merged.producentSprawdzonoAt||new Date().toISOString()},result={...merged,...force};
