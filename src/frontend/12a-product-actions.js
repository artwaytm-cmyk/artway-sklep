    ...
    if(state.operation==="withdraw"){
      ...
      for(const sourceProduct of products){
        try{
          ...
          const d=await chmura("allegro-create-product-offer",{method:"POST",body:{product:p,...(preparedDraft?{draft:preparedDraft}:{}),options:{stock:allegroStanOfertyProduktu(p),publicationAction,publishNow:publicationAction==="activate"}},timeout:120000});
          ...
          asortymentAllegroDecyzja.ok++;asortymentAllegroDecyzja.results.push({id:String(p.id),name:p.nazwa,ok:true,offerId:d.offer?.id||existing?.id||"",savedFields:preparation.savedFields});
        }catch(error){
          ...
          asortymentAllegroDecyzja.failed++;asortymentAllegroDecyzja.results.push({id:String(p.id),name:p.nazwa,ok:false,error:error.message||String(error)});
