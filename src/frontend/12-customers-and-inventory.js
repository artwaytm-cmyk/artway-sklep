...
function widokAdminProdukty(){
  ...
  return asortymentSzkielet("produkty", `
    <div class="panel assortment-catalog-page">
      ...
      <div class="assortment-bulk-editor"><div><b>Zaznaczone: <span data-product-selection-count>${zaznaczoneProdukty.size}</span></b><small>Operacje dotyczą wyłącznie świadomie zaznaczonych kart.</small></div><button class="btn danger" data-product-selection-required onclick="usunZaznaczoneProd()" ${zaznaczoneProdukty.size?"":"disabled"}>🗑️ Przenieś do kosza</button><label>Zmiana cen<select id="trybCenProduktow"><option value="percent">O procent (+/−)</option><option value="amount">O kwotę (+/−)</option><option value="fixed">Ustaw cenę</option></select></label><input id="procentCen" placeholder="np. 10 lub -5" inputmode="decimal"><button class="btn ghost" data-product-selection-required onclick="zmienCenyZaznaczonych()" ${zaznaczoneProdukty.size?"":"disabled"}>💰 Zmień ceny</button></div>
      ...
      <div class="assortment-table-wrap"><table class="log-table assortment-product-table density-${gestoscAdminProduktow}">
        ...
        ${fragment.map(p=>{
          ...
          <td><input class="assortment-check" type="checkbox" data-assortment-product-id="${esc(p.id)}" ${zaznaczoneProdukty.has(p.id)?"checked":""} onchange="przelaczZaznProd(${jsArg(p.id)})"></td>
...
function agentAIWdrozenieProduktuHTML(p={},edycja=false){
  const state=agentAIStanWdrozeniaProduktu(p),specialists=typeof agentAISpecjalisci!=="undefined"?(agentAISpecjalisci.data||{}):{},decisions=Array.isArray(specialists.decisions)?specialists.decisions:[],history=Array.isArray(specialists.history)?specialists.history:[],pending=decisions.find(x=>x.target?.type==="product"&&String(x.target?.productId)===String(p.id)&&x.kind==="product_content_review"),latest=(pending&&history.find(x=>String(x.id)===String(pending.runId)))||history.find(x=>x.target?.type==="product"&&String(x.target?.productId)===String(p.id)),learning=specialists.learning?.productContent||{},status=p.agentOnboardingStatus||(!p.id?"new":"not_started"),busy=status==="processing"||agentAISpecjalistaDecyzjeWToku?.has?.(String(pending?.id||"")),editorial=p.contentEditorial||{};
  const activity=pending?`Agent przygotował ${latest?.result?.fields?.length||0} konkretnych zmian i pyta, czy może je zapisać.`:editorial.status==="ready"?`Ostatnia redakcja została zapisana ${editorial.preparedAt?agentAIRuntimeCzas(editorial.preparedAt):"wcześniej"}.`:(specialists.updatedAt?"Agent kontroluje katalog co 15 minut. Ten produkt czeka w kolejce albo nie wymaga teraz zmiany.":"Łączę kartę produktu z rejestrem pracy Agenta…");
  return `<section class="product-agent-onboarding ${pending?"needs-decision":state.ready?"is-ready":busy?"is-busy":"needs-work"}"><header><div><span class="order-pro-label">Najwyższy priorytet przy dodawaniu • Agent redakcji • cykl 15 min</span><h3>${pending?"✨ Agent pyta o Twoją decyzję":"🤖 Agent wdrożenia produktu"}</h3><p>${esc(activity)}</p></div><strong>${pending?"?":`${state.done}/${state.total}`}</strong></header><div class="product-agent-checks">${state.checks.map(x=>`<span class="${x.ok?"done":"wait"}">${x.ok?"✓":"○"} ${esc(x.label)}</span>`).join("")}</div><div class="product-agent-learning"><span>${learning.ready?"🧠":"🎓"}</span><div><b>${learning.ready?"Agent zna zatwierdzony styl":"Agent uczy się z Twoich zatwierdzeń"}</b><small>${learning.ready?`${Math.round(Number(learning.acceptanceRate||0)*100)}% zaakceptowanych pól`:`${learning.approvals||0} zatwierdzeń • potrzeba jeszcze ${learning.remainingApprovals??3}`}</small></div><a href="#/admin/agent-ai/specjalisci">Pełna pamięć i historia →</a></div>${pending?`<div class="product-agent-pending">${agentAISpecjalistaDecyzjaHTML(pending)}</div>`:""}<footer><small>${pending?"Nic nie zostanie zapisane, dopóki nie wybierzesz pól i nie zatwierdzisz.":state.ready?"Kompletność podstawowa jest prawidłowa; Agent nadal ocenia jakość tekstu.":"Brakujące pola pozostają widoczne i trafiają do planu Agenta."}</small>${edycja&&!pending?`<button class="btn" type="button" onclick="agentAIUruchomWdrozenieProduktu(${jsArg(p.id)},this)" ${busy?"disabled":""}>${busy?"⏳ Przygotowuję…":"✨ Przygotuj propozycję teraz"}</button>`:""}</footer></section>`;
}
async function agentAIUruchomWdrozenieProduktu(id,button=null){
  ...
  try{const textRun=await agentAISpecjalistaProduktWdrozenie(product);toast("✨ Agent przygotował porównanie przed/po i czeka na Twoją decyzję");return {status:"awaiting_approval",textRun};}
...
function formularzProduktu(p, tryb){
  ...
  if(edycja&&typeof agentAISpecjalisci!=="undefined"&&!agentAISpecjalisci.loaded&&!agentAISpecjalisci.loading)setTimeout(()=>agentAISpecjalisciPobierz(false),0);
