...
async function agentAIUruchomWdrozenieProduktu(id,button=null){
  ...
  try{const textRun=await agentAISpecjalistaProduktWdrozenie(product);await chmuraWczytajStan().catch(()=>{});const status=textRun?.approvalStatus==="auto_applied"?"saved":"awaiting_exception_decision";toast(status==="saved"?"✅ Agent poprawił i zapisał treści produktu":"⚠️ Agent zatrzymał wyjątek wymagający sprawdzenia");return {status,textRun};}
