const vonHalskyStan={
  ...
  settings:{integrationMethod:"api",integrator:"",channelAlias:"VH",merchantStoreName:"Artway-TM",notificationEmail:"",minimumStock:1,maximumStock:25,syncIntervalMinutes:15,automaticPriceSync:true,automaticStockSync:true,automaticResume:true,catalogAutomationEnabled:false,testOfferCode:"1410",customerZone:true,onboarding:{}},
...
function vonHalskySettingsBody(form){
  ...
  return {
    ...
    automaticPriceSync:form.automaticPriceSync.checked,automaticStockSync:form.automaticStockSync.checked,automaticResume:form.automaticResume.checked,catalogAutomationEnabled:form.catalogAutomationEnabled.checked,testOfferCode:fd.get("testOfferCode"),customerZone:form.customerZone.checked,onboarding
  };
}
...
async function vonHalskySynchronizujKatalog(){
  ...
  let productIds=[...vonHalskyZaznaczone];
  if(!productIds.length&&vonHalskyStan.settings.catalogAutomationEnabled!==true){
    const code=String(vonHalskyStan.settings.testOfferCode||"1410").toLowerCase(),testProduct=vonHalskyProdukty().find(product=>[product.id,product.externalId,product.sku,product.kodProducenta,product.mpn,product.ean,product.gtin].some(value=>String(value??"").trim().toLowerCase()===code));
    if(!testProduct){toast(`Nie znaleziono produktu kontrolnego ${code}.`);return;}
    productIds=[String(testProduct.id)];
  }
  const scope=productIds.length?`${productIds.length} wskazanych produktów`:"cały katalog";
  if(!confirm(`Przekazać do InPost Von Halsky: ${scope}? W trybie testowym nowa oferta może powstać wyłącznie dla kodu ${vonHalskyStan.settings.testOfferCode||"1410"}.`))return;
  vonHalskyStan.operation="catalog";renderuj();
  try{const data=await chmura("von-halsky-sync-catalog",{method:"POST",body:{publish:true,batchSize:50,...(productIds.length?{productIds}:{})},timeout:180000});vonHalskyStan.sync={...vonHalskyStan.sync,...(data.sync||{})};toast(`Nowe ${data.created||0} • aktualizacje ${data.updated||0} • zamknięte ${data.closed||0} • wznowione ${data.reopened||0} ✅`);await vonHalskyLaduj(true);}catch(error){toast("Synchronizacja Von Halsky: "+(error.message||error));await vonHalskyLaduj(true);}finally{vonHalskyStan.operation="";renderuj();}
}
