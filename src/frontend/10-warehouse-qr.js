...
function magazynQRPozycje(){
  ...
  if(magazynQRTryb==="lokalizacje")return (typeof magazynLokalizacjeAktywne==="function"?magazynLokalizacjeAktywne():[]).filter(l=>(magazynQRTyp==="wszystkie"||l.typ===magazynQRTyp)&&(!query||`${l.kod} ${l.nazwa||""} ${l.typ||""} ${l.strefa||""} ${l.kodKreskowy||""}`.toLowerCase().includes(query))).slice(0,120).map(l=>({type:"L",id:l.kod,title:l.kod,subtitle:`${l.typ||"lokalizacja"}${l.nazwa?` • ${l.nazwa}`:""}`,meta:sciezkaLokalizacjiMagazynu(l.kod)}));
...
function magazynQRCentrumHTML(){
  ...
  return `<section class="panel warehouse-qr-center"><div class="order-section-head"><div><span class="order-pro-label">Mobilny magazyn bez dodatkowego sprzętu</span><h2>📱 Skanowanie i etykiety QR</h2><p class="order-detail-lead">Na telefonie zeskanuj najpierw QR regału, półki lub miejsca, a potem QR/EAN produktów. Wybrane miejsce pozostaje aktywne dla kolejnych pozycji dokumentu PZ albo WZ.</p></div><span class="warehouse-qr-ready">${locations.length?"✅":"⚠️"} ${locations.length} lokalizacji gotowych do etykiet</span></div><div class="warehouse-qr-flow"><span><b>1</b><strong>Zeskanuj miejsce</strong><small>regał, półkę albo box</small></span><span><b>2</b><strong>Skanuj produkty</strong><small>QR, EAN, GTIN lub Code 128</small></span><span><b>3</b><strong>Sprawdź dokument</strong><small>ilości i lokalizacje</small></span><span><b>4</b><strong>Zatwierdź PZ/WZ</strong><small>dopiero wtedy zmienia się stan</small></span></div><details class="warehouse-qr-generator" open><summary><span><b>🏷️ Generator etykiet QR</b><small>Druk dla regałów, półek, miejsc i konkretnych produktów.</small></span><strong>Otwórz generator</strong></summary><div id="warehouseQrWorkspace">${magazynQRGeneratorWnetrzeHTML()}</div></details></section>`;
}
function magazynQREtykietaDane(type,id){
  if(type==="L"){const l=magazynLokalizacjaPoKodzie(id);if(!l)return null;return {type:"location",payload:magazynQRPayloadLokalizacji(l.kod),title:l.kod,subtitle:l.nazwa||l.typ||"Lokalizacja magazynowa",meta:sciezkaLokalizacjiMagazynu(l.kod)};}
