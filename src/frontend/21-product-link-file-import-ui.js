...
function productLinkImportReviewDraft(item={}){
  ...
  return {...draft,nazwa:draft.nazwa||item.name||"",sourceUrl:draft.sourceUrl||item.url||"",producentUrl:draft.producentUrl||item.url||""};
}
function productLinkImportReviewBraki(item={}){
  ...
  const d=productLinkImportReviewDraft(item),out=[];if(!String(d.nazwa||"").trim())out.push("nazwa");if(!(Number(d.cena)>0))out.push("cena sprzedaży");if(!String(d.producent||d.marka||"").trim())out.push("producent lub marka");if(!String(d.kategoria||"").trim())out.push("kategoria sklepu");return out;
}
function productLinkImportReviewFormHTML(item={}){
  const d=productLinkImportReviewDraft(item),missing=productLinkImportReviewBraki(item),brak=field=>missing.some(value=>String(value).toLowerCase().includes(field));
  return `<details class="product-link-review-editor" ${missing.length?"open":""}><summary><span>✏️ Uzupełnij dane produktu</span><em>${missing.length?`Brakuje: ${esc(missing.join(", "))}`:"Szkic gotowy do zatwierdzenia"}</em></summary><form onsubmit="return productLinkImportZapiszDecyzje(event,${jsArg(item.id)})"><div class="product-link-review-primary"><label class="${brak("cena")?"is-required":""}"><span>Cena sprzedaży brutto *</span><input name="cena" inputmode="decimal" value="${esc(Number(d.cena)>0?d.cena:"")}" placeholder="np. 29,90" required></label><label class="${brak("nazwa")?"is-required":""}"><span>Nazwa produktu *</span><input name="nazwa" value="${esc(d.nazwa||"")}" required></label><label class="${brak("producent")?"is-required":""}"><span>Producent / marka *</span><input name="producent" value="${esc(d.producent||d.marka||"")}" required></label><label class="${brak("kategoria")?"is-required":""}"><span>Kategoria sklepu *</span><input name="kategoria" value="${esc(d.kategoria||"")}" required></label></div><details class="product-link-review-more"><summary>Więcej danych do poprawy</summary><div><label><span>EAN / GTIN</span><input name="ean" value="${esc(d.ean||d.gtin||"")}"></label><label><span>EXTERNAL_ID / SKU</span><input name="externalId" value="${esc(d.externalId||d.sku||"")}"></label><label><span>Kod producenta</span><input name="kodProducenta" value="${esc(d.kodProducenta||d.mpn||"")}"></label><label><span>Główne zdjęcie</span><input name="zdjecie" value="${esc(d.zdjecie||"")}" placeholder="https://…"></label><label><span>Emoji</span><input name="ikona" value="${esc(d.ikona||"🎲")}" maxlength="20"></label><label><span>Kolor karty</span><input name="kolor" type="color" value="${/^#[0-9a-f]{6}$/i.test(String(d.kolor||""))?esc(d.kolor):"#dbeafe"}"></label><label class="wide"><span>Krótki opis</span><textarea name="opisKrotki" rows="2">${esc(d.opisKrotki||"")}</textarea></label><label class="wide"><span>Pełny opis</span><textarea name="opis" rows="5">${esc(d.opis||"")}</textarea></label></div></details><footer><small>System ponownie sprawdzi link, połączy pobrane dane z Twoimi zmianami i doda produkt tylko wtedy, gdy wszystkie wymagane pola będą kompletne.</small><button class="btn" type="submit" ${productLinkImportStan.reviewBusy?"disabled":""}>✅ Zapisz decyzję i dodaj produkt</button></footer></form></details>`;
}
...
async function productLinkImportRozstrzygnij(items,commonPatch={},message="Zapisuję decyzję…"){
  ...
  try{
    const response=await chmura("product-link-import-review-resolve",{method:"POST",body:{jobId:productLinkImportStan.jobId,items,commonPatch},timeout:180000});
    productLinkImportScalOdpowiedz(response);await chmuraWczytajStan().catch(()=>false);zbudujProdukty();
    productLinkImportStan.notice=`Decyzje zapisane: dodano lub połączono ${response.resolved||0}${response.stillNeedsReview?` • nadal wymaga danych ${response.stillNeedsReview}`:""}.`;
    toast(`✅ Uzupełniono ${response.resolved||0} produktów${response.stillNeedsReview?` • ${response.stillNeedsReview} nadal do decyzji`:""}`);return true;
  }catch(error){productLinkImportStan.error=error?.message||String(error);productLinkImportStan.notice="Nie zapisano decyzji.";return false;}
