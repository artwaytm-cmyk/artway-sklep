...
async function agentAISpecjalisciZapiszUstawienia(e){
  e?.preventDefault();const form=e.currentTarget,f=new FormData(form),config={enabled:f.get("enabled")==="on",automaticEnabled:f.get("automaticEnabled")==="on",safeAutoApply:f.get("safeAutoApply")==="on",learningEnabled:f.get("learningEnabled")==="on",approvalWarmupCount:Number(f.get("approvalWarmupCount")||0),learnedAutoApplyThreshold:Number(f.get("learnedAutoApplyThreshold")||86)/100,dailyLimit:Number(f.get("dailyLimit")||60),automaticDailyLimit:Number(f.get("automaticDailyLimit")||30),automaticBatchSize:Number(f.get("automaticBatchSize")||4),cacheHours:Number(f.get("cacheHours")||24),confidenceThreshold:Number(f.get("confidenceThreshold")||92)/100,decisionRetentionDays:Number(f.get("decisionRetentionDays")||30)};agentAISpecjalisci={...agentAISpecjalisci,saving:true};renderuj();
...
async function agentAISpecjalisciCykl(){
  agentAISpecjalisci={...agentAISpecjalisci,running:true};renderuj();try{const data=await chmura("agent-specialist-auto-cycle",{method:"POST",body:{source:"admin-manual"},timeout:180000}),cycle=data.cycle||{},applied=cycle.applied?.length||0,decisions=cycle.decisions?.length||0;toast(applied||decisions?`✅ Agent: ${applied} bezpiecznych zapisów, ${decisions} nowych decyzji`:`ℹ️ ${cycle.reason==="no_candidates"?"Wszystko aktualne — brak nowych zadań":"Cykl zakończony"}`);await agentAISpecjalisciPobierz(true);}catch(error){toast("⚠️ Cykl GPT: "+(error?.message||error));}finally{agentAISpecjalisci={...agentAISpecjalisci,running:false};renderuj();}
}
...
function agentAISpecjalisciPanelHTML(){
  const state=agentAISpecjalisci,data=state.data||{},specialists=Array.isArray(data.specialists)?data.specialists:[],history=Array.isArray(data.history)?data.history:[],decisions=Array.isArray(data.decisions)?data.decisions:[],usage=data.usage||{},stats=data.decisionStats||{},last=data.lastCycle||{},cfg=data.config||{enabled:true,automaticEnabled:true,safeAutoApply:true,learningEnabled:true,approvalWarmupCount:3,learnedAutoApplyThreshold:.86,dailyLimit:60,automaticDailyLimit:30,automaticBatchSize:4,cacheHours:24,confidenceThreshold:.92,decisionRetentionDays:30},learning=data.learning?.productContent||{};
  ...
  const autonomyHTML=`<section class="panel agent-learning-panel"><div><span>${learning.ready?"🧠":"🎓"}</span><div><small>PAMIĘĆ ZATWIERDZEŃ</small><h3>${learning.ready?"Ograniczona autonomia redakcji aktywna":"Agent uczy się Twoich decyzji"}</h3><p>${learning.ready?`Zaakceptowano ${esc(Math.round(Number(learning.acceptanceRate||0)*100))}% ocenionych pól. Agent sam zapisuje tylko kompletne, bezpieczne treści o wysokiej pewności.`:`Agent potrzebuje jeszcze ${esc(learning.remainingApprovals??cfg.approvalWarmupCount??3)} zatwierdzeń. Do tego czasu każdą redakcję produktu przedstawia jako pytanie przed zapisem.`}</p></div></div><div><b>${esc(learning.approvals||0)}</b><small>zatwierdzeń</small><b>${esc(learning.corrections||0)}</b><small>korekt</small><b>${esc(learning.dismissals||0)}</b><small>odrzuceń</small></div></section>`;
...
function agentAIInicjatywaPanelHTML(){
  const data=agentAISpecjalisci.data||{},learning=data.learning?.productContent||{},decisions=(data.decisions||[]).filter(item=>item.kind==="product_content_review"),history=data.history||[],last=data.lastCycle||{};
...
function widokAdminAgentAI(sekcja="pulpit"){
  ...
  if(["pulpit","specjalisci"].includes(aktywna)&&!agentAISpecjalisci.loaded&&!agentAISpecjalisci.loading)setTimeout(()=>agentAISpecjalisciPobierz(false),0);
