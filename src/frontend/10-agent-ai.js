...
async function agentAISpecjalisciZapiszUstawienia(e){
  e?.preventDefault();const form=e.currentTarget,f=new FormData(form),config={enabled:f.get("enabled")==="on",automaticEnabled:f.get("automaticEnabled")==="on",safeAutoApply:f.get("safeAutoApply")==="on",dailyLimit:Number(f.get("dailyLimit")||60),automaticDailyLimit:Number(f.get("automaticDailyLimit")||30),automaticBatchSize:Number(f.get("automaticBatchSize")||4),cacheHours:Number(f.get("cacheHours")||24),confidenceThreshold:Number(f.get("confidenceThreshold")||92)/100,decisionRetentionDays:Number(f.get("decisionRetentionDays")||30)};agentAISpecjalisci={...agentAISpecjalisci,saving:true};renderuj();
...
async function agentAISpecjalistaDecyzja(id,decisionAction,days=1){
  try{await chmura("agent-specialist-decision",{method:"POST",body:{id,decisionAction,days},timeout:60000});toast(decisionAction==="approve"?"✅ Kierunek zatwierdzony":decisionAction==="snooze"?`⏰ Decyzja odłożona na ${days} dni`:decisionAction==="dismiss"?"🗑️ Propozycja odrzucona":"✅ Sprawa zamknięta");await agentAISpecjalisciPobierz(true);renderuj();}catch(error){toast("⚠️ Decyzja Agenta: "+(error?.message||error));}
}
function agentAISpecjalistaDecyzjaHTML(item={}){
  const risk={low:["Niskie ryzyko","lvl-ok"],medium:["Wymaga kontroli","lvl-ostrzezenie"],high:["Decyzja administratora","lvl-blad"]}[item.risk]||["Wymaga kontroli","lvl-ostrzezenie"],product=item.target?.type==="product"&&item.runId;
  return `<article class="agent-proposal-card risk-${esc(item.risk||"medium")}"><header><span>${esc(item.icon||"🧭")}</span><div><b>${esc(item.title||"Propozycja Agenta")}</b><small>${esc(item.specialist||"koordynator")} • ${esc(agentAIRuntimeCzas(item.createdAt))}</small></div><em class="lvl ${risk[1]}">${risk[0]}</em></header><p>${esc(item.summary||"")}</p><section><small>Rekomendacja</small><b>${esc(item.recommendation||"Sprawdź dane i wybierz dalsze działanie.")}</b></section>${item.alternatives?.length?`<details><summary>Możliwe warianty</summary><ul>${item.alternatives.map(value=>`<li>${esc(value)}</li>`).join("")}</ul></details>`:""}<footer>${item.href?`<a class="btn" href="${esc(item.href)}">Otwórz właściwy moduł</a>`:""}<button class="btn ${product?"":"ghost"}" onclick="agentAISpecjalistaDecyzja(${jsArg(item.id)},'approve')">${product?"✅ Zatwierdź i zapisz braki":"✓ Akceptuj kierunek (bez wysyłki)"}</button><button class="btn ghost" onclick="agentAISpecjalistaDecyzja(${jsArg(item.id)},'snooze',1)">⏰ Jutro</button><button class="btn ghost" onclick="agentAISpecjalistaDecyzja(${jsArg(item.id)},'dismiss')">Odrzuć</button></footer></article>`;
}
function agentAISpecjalisciPanelHTML(){
  const state=agentAISpecjalisci,data=state.data||{},specialists=Array.isArray(data.specialists)?data.specialists:[],history=Array.isArray(data.history)?data.history:[],decisions=Array.isArray(data.decisions)?data.decisions:[],usage=data.usage||{},stats=data.decisionStats||{},last=data.lastCycle||{},cfg=data.config||{enabled:true,automaticEnabled:true,safeAutoApply:true,dailyLimit:60,automaticDailyLimit:30,automaticBatchSize:4,cacheHours:24,confidenceThreshold:.92,decisionRetentionDays:30};
  ...
  const active=state.activeRun||history[0],never=(data.policy?.neverAutomatic||[]).join(", ");
  return `<section class="agent-specialists-page"><section class="panel agent-specialists-hero"><div><span class="order-pro-label">Stała gotowość • cykl co ${esc(data.policy?.cycleMinutes||15)} minut</span><h2>✦ Autonomiczny zespół GPT-5 nano</h2><p>Agenci sami wykrywają zadania, przygotowują rozwiązania i uzupełniają wyłącznie bezpieczne, puste pola. Trudne decyzje trafiają poniżej do zatwierdzenia. Nie musisz uruchamiać ich przyciskiem.</p></div><div><strong>${esc(data.model||"gpt-5-nano")} • v${esc(data.promptVersion||"—")}</strong><span class="lvl ${data.configured&&cfg.automaticEnabled!==false?"lvl-ok":"lvl-blad"}">${data.configured&&cfg.automaticEnabled!==false?"● agenci czuwają":"automatyka zatrzymana"}</span><button class="btn ghost" onclick="agentAISpecjalisciCykl()" ${state.running?"disabled":""}>${state.running?"⏳ Kontroluję…":"↻ Sprawdź teraz"}</button></div></section><div class="orders-stat-grid"><div class="order-stat-card money"><span>🧭</span><b>${esc(stats.open||0)}</b><small>decyzji dla Ciebie</small></div><div class="order-stat-card"><span>✅</span><b>${esc(last.autoApplied||0)}</b><small>bezpiecznie zapisano w cyklu</small></div><div class="order-stat-card"><span>✦</span><b>${specialists.length}</b><small>aktywnych ról serwerowych</small></div><div class="order-stat-card"><span>🪙</span><b>${esc(usage.automaticToday||0)} / ${esc(cfg.automaticDailyLimit||30)}</b><small>automatycznych analiz dzisiaj</small></div></div><section class="panel agent-proposal-board"><div class="order-section-head"><div><span class="order-pro-label">Tylko sprawy wymagające człowieka</span><h2>Decyzje i propozycje Agenta</h2><p class="order-detail-lead">Zewnętrzne działania nigdy nie dzieją się bez zgody: ${esc(never)}.</p></div><span class="lvl ${stats.high?"lvl-blad":decisions.length?"lvl-ostrzezenie":"lvl-ok"}">${decisions.length?`${decisions.length} otwartych`:`wszystko pod kontrolą`}</span></div><div class="agent-proposal-list">${decisions.map(agentAISpecjalistaDecyzjaHTML).join("")||`<div class="agent-ops-empty">✅ Brak trudnych decyzji. Zespół nadal kontroluje sklep automatycznie co 15 minut.</div>`}</div></section><div class="agent-specialist-grid">${specialists.map(item=>`<article><span>${esc(item.icon||"✦")}</span><div><b>${esc(item.label)}</b><small>${esc(item.area)} • v${esc(item.promptVersion||data.promptVersion||"—")}</small><p>${esc(item.description)}</p></div><i class="agent-role-live">● aktywny</i></article>`).join("")}</div><details class="panel agent-specialist-manual"><summary>＋ Dodatkowe ręczne zadanie dla wybranej roli</summary><div class="agent-specialist-workspace"><form class="agent-specialist-form" onsubmit="return agentAISpecjalistaFormularz(event)"><label>Specjalista<select name="specialist">${specialists.map(item=>`<option value="${esc(item.id)}">${esc(item.icon)} ${esc(item.label)} • ${esc(item.area)}</option>`).join("")}</select></label><label>Co ma przygotować?<input name="instruction" maxlength="3000" placeholder="Np. przygotuj kampanię weekendową i tekst bannera"></label><label>Potwierdzone fakty<textarea name="material" rows="7" maxlength="12000" placeholder="Produkt, grupa odbiorców, rabat, daty lub obecna treść…"></textarea></label><button class="btn" type="submit" ${state.running?"disabled":""}>✦ Przygotuj dodatkowy szkic</button></form><aside>${active?agentAISpecjalistaWynikHTML(active):`<div class="agent-specialist-empty"><span>✦</span><b>Wynik pojawi się tutaj</b></div>`}</aside></div></details><form class="panel agent-specialist-settings" onsubmit="return agentAISpecjalisciZapiszUstawienia(event)"><div class="order-section-head"><div><span class="order-pro-label">Polityka autonomii i kosztów</span><h2>Granice pracy Agenta</h2><p class="order-detail-lead">Bezpieczny zapis nigdy nie nadpisuje istniejącej treści i nie wykonuje działań zewnętrznych.</p></div><button class="btn" type="submit" ${state.saving?"disabled":""}>💾 Zapisz ustawienia</button></div><div><label><input type="checkbox" name="enabled" ${cfg.enabled!==false?"checked":""}> Role aktywne</label><label><input type="checkbox" name="automaticEnabled" ${cfg.automaticEnabled!==false?"checked":""}> Czuwanie co 15 min</label><label><input type="checkbox" name="safeAutoApply" ${cfg.safeAutoApply!==false?"checked":""}> Uzupełniaj tylko puste pola</label><label>Próg pewności (%)<input type="number" name="confidenceThreshold" min="75" max="100" value="${esc(Math.round(Number(cfg.confidenceThreshold||.92)*100))}"></label><label>Automatyczne / dzień<input type="number" name="automaticDailyLimit" min="0" max="100" value="${esc(cfg.automaticDailyLimit??30)}"></label><label>Analiz na cykl<input type="number" name="automaticBatchSize" min="1" max="5" value="${esc(cfg.automaticBatchSize||4)}"></label><label>Wszystkie / dzień<input type="number" name="dailyLimit" min="1" max="200" value="${esc(cfg.dailyLimit||60)}"></label><label>Pamięć wyniku (godz.)<input type="number" name="cacheHours" min="1" max="168" value="${esc(cfg.cacheHours||24)}"></label><label>Historia decyzji (dni)<input type="number" name="decisionRetentionDays" min="7" max="90" value="${esc(cfg.decisionRetentionDays||30)}"></label></div></form><section class="panel agent-specialist-history"><div class="order-section-head"><div><span class="order-pro-label">Pełny audyt pracy</span><h2>Co agenci rzeczywiście wykonali</h2><p class="order-detail-lead">Każdy wynik ma model, wersję reguł, pewność, użyte tokeny i stan zapisu.</p></div><button class="btn ghost" onclick="agentAISpecjalisciPobierz(false)">↻ Odśwież</button></div><div>${history.slice(0,30).map(run=>agentAISpecjalistaWynikHTML(run,true)).join("")||`<div class="agent-ops-empty">Historia pojawi się po najbliższym cyklu serwera.</div>`}</div></section></section>`;
}
