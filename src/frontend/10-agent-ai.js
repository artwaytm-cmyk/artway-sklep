...
async function agentAIUprawnieniaZapisz(e){
  e?.preventDefault();const f=new FormData(e.currentTarget),current=agentAISpecjalisci.data?.config||{},config={...current,safeAutoApply:true,autoApplyProductEditorial:f.get("autoApplyProductEditorial")==="on",autoUpdateLinkedAllegroContent:f.get("autoUpdateLinkedAllegroContent")==="on"};agentAISpecjalisci={...agentAISpecjalisci,saving:true};renderuj();
...
function agentAIUprawnieniaPanelHTML(){
  ...
  return `<section class="agent-permission-page"><section class="panel agent-permission-hero"><div><span class="order-pro-label">Jedna polityka dla całej strony</span><h2>🛡️ Agent wykonuje rutynę, człowiek zatwierdza ryzyko</h2><p>Bezpieczne treści produktu nie trafiają już do kolejki pytań. Agent zapisuje je w sklepie i aktualizuje warstwę prezentacyjną istniejącej oferty Allegro. Operacje finansowe, sprzedażowe i zewnętrzna komunikacja pozostają zablokowane do decyzji.</p></div><span class="lvl ${cfg.autoApplyProductEditorial!==false&&cfg.autoUpdateLinkedAllegroContent!==false?"lvl-ok":"lvl-ostrzezenie"}">${cfg.autoApplyProductEditorial!==false&&cfg.autoUpdateLinkedAllegroContent!==false?"● autonomia treści aktywna":"część automatyki wyłączona"}</span></section><div class="agent-permission-columns"><form class="panel agent-permission-list automatic" onsubmit="return agentAIUprawnieniaZapisz(event)"><div class="order-section-head"><div><span class="order-pro-label">Bez pytania • pełny audyt</span><h2>⚡ Agent wykonuje sam</h2><p class="order-detail-lead">Tylko działania odwracalne, oparte na pewnych danych i objęte kontrolą zgodności.</p></div><button class="btn" type="submit" ${agentAISpecjalisci.saving?"disabled":""}>💾 Zapisz</button></div><div class="agent-permission-items">${automatic.map(item=>`<article class="${isEnabled(item)?"enabled":"disabled"}"><span>${esc(item.icon||"⚡")}</span><div><b>${esc(item.label)}</b><p>${esc(item.description)}</p></div>${item.configKey?`<label class="switch-check"><input type="checkbox" name="${esc(item.configKey)}" ${isEnabled(item)?"checked":""}><span>${isEnabled(item)?"Automatycznie":"Wstrzymane"}</span></label>`:`<em>✓ zawsze aktywne</em>`}</article>`).join("")}</div><div class="agent-permission-guard"><b>Ochrona synchronizacji Allegro</b><span>Automatyczna aktualizacja treści nie zmienia ceny, stanu, aktywności oferty ani warunków sprzedaży.</span></div></form><section class="panel agent-permission-list approval"><div class="order-section-head"><div><span class="order-pro-label">Zawsze z decyzją administratora</span><h2>🔐 Agent najpierw pyta</h2><p class="order-detail-lead">Tych blokad nie można przypadkowo wyłączyć poleceniem ani automatycznym cyklem.</p></div></div><div class="agent-permission-items">${approval.map(item=>`<article><span>${esc(item.icon||"🔐")}</span><div><b>${esc(item.label)}</b><p>${esc(item.description)}</p></div><em>🔐 potwierdzenie</em></article>`).join("")}</div></section></div><section class="panel agent-permission-blocked"><div><span>⛔</span><div><span class="order-pro-label">Automatyczny bezpiecznik</span><h2>Kiedy Agent zatrzymuje pracę</h2></div></div><div>${blocked.map(item=>`<span>• ${esc(item)}</span>`).join("")}</div><p>W takich przypadkach w Centrum decyzji pojawia się wyłącznie konkretny wyjątek. Zwykłe poprawianie opisów nie wymaga już zatwierdzania.</p></section></section>`;
}
function agentAISpecjalisciPanelHTML(){
  const state=agentAISpecjalisci,data=state.data||{},specialists=Array.isArray(data.specialists)?data.specialists:[],history=Array.isArray(data.history)?data.history:[],decisions=Array.isArray(data.decisions)?data.decisions:[],usage=data.usage||{},stats=data.decisionStats||{},last=data.lastCycle||{},cfg=data.config||{enabled:true,automaticEnabled:true,safeAutoApply:true,autoApplyProductEditorial:true,autoUpdateLinkedAllegroContent:true,learningEnabled:true,approvalWarmupCount:3,learnedAutoApplyThreshold:.86,dailyLimit:60,automaticDailyLimit:30,automaticBatchSize:4,cacheHours:24,confidenceThreshold:.92,decisionRetentionDays:30},learning=data.learning?.productContent||{};
  ...
  return `<section class="agent-specialists-page"><section class="panel agent-specialists-hero"><div><span class="order-pro-label">Stała gotowość • cykl co ${esc(data.policy?.cycleMinutes||15)} minut</span><h2>✦ Zespół agentów OpenAI + GPT-5 nano</h2><p>Każda rola ma własny profil na koncie OpenAI, a tanie wykonanie strukturalne realizuje GPT-5 nano. Agent pokazuje dokładnie: co było, co zaleca, dlaczego i co rzeczywiście zapisano po Twoim zatwierdzeniu.</p></div><div><strong>${esc(data.model||"gpt-5-nano")} • v${esc(data.promptVersion||"—")}</strong><span class="lvl ${data.configured&&data.platformAgents?.configured&&cfg.automaticEnabled!==false?"lvl-ok":"lvl-blad"}">${data.configured&&data.platformAgents?.configured&&cfg.automaticEnabled!==false?"● profile OpenAI aktywne":"automatyka zatrzymana"}</span><button class="btn ghost" onclick="agentAISpecjalisciCykl()" ${state.running?"disabled":""}>${state.running?"⏳ Kontroluję…":"↻ Sprawdź teraz"}</button></div></section>${autonomyHTML}<div class="orders-stat-grid"><div class="order-stat-card money"><span>🧭</span><b>${esc(stats.open||0)}</b><small>decyzji dla Ciebie</small></div><div class="order-stat-card"><span>✅</span><b>${esc(last.autoApplied||0)}</b><small>bezpiecznie zapisano w cyklu</small></div><div class="order-stat-card"><span>✦</span><b>${specialists.length}</b><small>profili agentów OpenAI</small></div><div class="order-stat-card"><span>🪙</span><b>${esc(usage.automaticToday||0)} / ${esc(cfg.automaticDailyLimit||30)}</b><small>automatycznych analiz dzisiaj</small></div></div>${progressHTML}<section class="panel agent-proposal-board"><div class="order-section-head"><div><span class="order-pro-label">Tylko sprawy wymagające człowieka</span><h2>Decyzje i propozycje Agenta</h2><p class="order-detail-lead">Zewnętrzne działania nigdy nie dzieją się bez zgody: ${esc(never)}.</p></div><span class="lvl ${stats.high?"lvl-blad":decisions.length?"lvl-ostrzezenie":"lvl-ok"}">${decisions.length?`${decisions.length} otwartych`:`wszystko pod kontrolą`}</span></div><div class="agent-proposal-list">${decisions.map(agentAISpecjalistaDecyzjaHTML).join("")||`<div class="agent-ops-empty">✅ Brak trudnych decyzji. Zespół nadal kontroluje sklep automatycznie co 15 minut.</div>`}</div></section><div class="agent-specialist-grid">${specialists.map(item=>`<article><span>${esc(item.icon||"✦")}</span><div><b>${esc(item.platformName||item.label)}</b><small>${esc(item.area)} • OpenAI ${esc(item.assistantId?item.assistantId.slice(-6):"profil lokalny")} • v${esc(item.promptVersion||data.promptVersion||"—")}</small><p>${esc(item.description)}</p></div><i class="agent-role-live">● ${item.platformAvailable===false?"tryb awaryjny":"połączony"}</i></article>`).join("")}</div><details class="panel agent-specialist-manual"><summary>＋ Dodatkowe ręczne zadanie dla wybranej roli</summary><div class="agent-specialist-workspace"><form class="agent-specialist-form" onsubmit="return agentAISpecjalistaFormularz(event)"><label>Specjalista<select name="specialist">${specialists.map(item=>`<option value="${esc(item.id)}">${esc(item.icon)} ${esc(item.label)} • ${esc(item.area)}</option>`).join("")}</select></label><label>Co ma przygotować?<input name="instruction" maxlength="3000" placeholder="Np. przygotuj kampanię weekendową i tekst bannera"></label><label>Potwierdzone fakty<textarea name="material" rows="7" maxlength="12000" placeholder="Produkt, grupa odbiorców, rabat, daty lub obecna treść…"></textarea></label><button class="btn" type="submit" ${state.running?"disabled":""}>✦ Przygotuj dodatkowy szkic</button></form><aside>${active?agentAISpecjalistaWynikHTML(active):`<div class="agent-specialist-empty"><span>✦</span><b>Wynik pojawi się tutaj</b></div>`}</aside></div></details><form class="panel agent-specialist-settings" onsubmit="return agentAISpecjalisciZapiszUstawienia(event)"><div class="order-section-head"><div><span class="order-pro-label">Polityka autonomii i kosztów</span><h2>Granice pracy Agenta</h2><p class="order-detail-lead">Agent najpierw prosi o decyzje, zapamiętuje zatwierdzone pola i dopiero po osiągnięciu progu może sam zapisywać powtarzalne, bezpieczne redakcje.</p></div><button class="btn" type="submit" ${state.saving?"disabled":""}>💾 Zapisz ustawienia</button></div><div><label><input type="checkbox" name="enabled" ${cfg.enabled!==false?"checked":""}> Role aktywne</label><label><input type="checkbox" name="automaticEnabled" ${cfg.automaticEnabled!==false?"checked":""}> Czuwanie co 15 min</label><label><input type="checkbox" name="learningEnabled" ${cfg.learningEnabled!==false?"checked":""}> Ucz się z moich decyzji</label><label><input type="checkbox" name="safeAutoApply" ${cfg.safeAutoApply!==false?"checked":""}> Po nauce zapisuj bezpieczne redakcje</label><label>Zatwierdzeń przed autonomią<input type="number" name="approvalWarmupCount" min="0" max="20" value="${esc(cfg.approvalWarmupCount??3)}"></label><label>Min. akceptacja pól (%)<input type="number" name="learnedAutoApplyThreshold" min="60" max="100" value="${esc(Math.round(Number(cfg.learnedAutoApplyThreshold||.86)*100))}"></label><label>Próg pewności (%)<input type="number" name="confidenceThreshold" min="75" max="100" value="${esc(Math.round(Number(cfg.confidenceThreshold||.92)*100))}"></label><label>Automatyczne / dzień<input type="number" name="automaticDailyLimit" min="0" max="100" value="${esc(cfg.automaticDailyLimit??30)}"></label><label>Analiz na cykl<input type="number" name="automaticBatchSize" min="1" max="5" value="${esc(cfg.automaticBatchSize||4)}"></label><label>Wszystkie / dzień<input type="number" name="dailyLimit" min="1" max="200" value="${esc(cfg.dailyLimit||60)}"></label><label>Pamięć wyniku (godz.)<input type="number" name="cacheHours" min="1" max="168" value="${esc(cfg.cacheHours||24)}"></label><label>Historia decyzji (dni)<input type="number" name="decisionRetentionDays" min="7" max="90" value="${esc(cfg.decisionRetentionDays||30)}"></label></div></form><section class="panel agent-specialist-history"><div class="order-section-head"><div><span class="order-pro-label">Pełny audyt pracy</span><h2>Co agenci rzeczywiście wykonali</h2><p class="order-detail-lead">Każdy wynik ma profil OpenAI, model wykonawczy, wersję reguł, pewność, użyte tokeny i stan zapisu.</p></div><button class="btn ghost" onclick="agentAISpecjalisciPobierz(false)">↻ Odśwież</button></div><div>${history.slice(0,30).map(run=>agentAISpecjalistaWynikHTML(run,true)).join("")||`<div class="agent-ops-empty">Historia pojawi się po najbliższym cyklu serwera.</div>`}</div></section></section>`;
}
