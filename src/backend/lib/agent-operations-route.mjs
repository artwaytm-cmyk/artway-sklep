...
export function agentInternalOrigin(portValue = process.env.PORT) {
  const port = Number(String(portValue || '3000').trim());
  return `http://127.0.0.1:${Number.isInteger(port) && port > 0 && port <= 65_535 ? port : 3000}`;
}

export function createAgentOperationsRoute({
  respond, isAdmin, text, read, write, getOperationalCenter, fetchImpl = fetch, internalOrigin = agentInternalOrigin,
}) {
  return async function agentOperationsRoute(req, url, action) {
    ...
    const adminToken = String(process.env.ARTWAY_ADMIN_TOKEN || '').trim(), origin = internalOrigin(req), startedAt = new Date().toISOString();
    const results = [];
    for (const area of selected) {
      ...
      try {
        if (definition.local) {
          ...
          results.push({ area, label: definition.label, status: missing.length ? 'error' : 'completed', detail: missing.length ? `Wymagają konfiguracji: ${missing.join(', ')}` : `Integracje: ${Object.keys(integrations).join(', ')} • baza i API odpowiadają`, error: missing.length ? `Wymagają konfiguracji: ${missing.join(', ')}` : '', durationMs: Date.now() - started });
          continue;
        }
        const response = await fetchImpl(`${origin}/api/store?action=${encodeURIComponent(definition.action)}`, { method: 'POST', headers: { 'x-admin-token': adminToken, 'content-type': 'application/json', accept: 'application/json' }, body: JSON.stringify({ source: 'agent-safe-plan' }), signal: AbortSignal.timeout(45_000) });
        ...
        results.push({ area, label: definition.label, status: 'completed', count, scanned: area === 'allegro-orders' ? Number(data?.fetched || 0) : count, newItems: area === 'allegro-orders' ? Number(data?.imported_new || 0) : 0, refreshed: area === 'allegro-orders' ? Number(data?.refreshed || 0) : 0, durationMs: Date.now() - started });
      } catch (error) {
        results.push({ area, label: definition.label, status: 'error', error: text(error?.message || error, 500), durationMs: Date.now() - started });
      }
    }
