export const VON_HALSKY_PRODUCT_QUEUE_SQL = `
  WITH source AS (
    SELECT
      ...
      END channel_price,
      NOT (lower(COALESCE(p.admin_list_data->>'vonHalskyDoesNotRequireGpsrInfo','false')) IN ('true','1','yes','tak')) gpsr_required,
      ...
      CASE
        ...
        ELSE 0
      END gpsr_missing,
      jsonb_typeof(p.admin_list_data->'vonHalskyAttributeDefinitions')='array' attribute_schema_known,
      CASE
        WHEN jsonb_typeof(p.admin_list_data->'vonHalskyRequiredAttributesMissing')='array'
          THEN jsonb_array_length(p.admin_list_data->'vonHalskyRequiredAttributesMissing')
        ELSE 0
      END required_attribute_missing,
      NOT (lower(COALESCE(p.admin_list_data->>'vonHalskyCategoryTreeValid','true')) IN ('false','0','no','nie')) category_tree_valid,
      CASE
  ...
  classified AS (
    SELECT source.*,
      (
        ...
        AND channel_price > 0
        AND category_id <> ''
        AND category_tree_valid
        AND attribute_schema_known
        AND required_attribute_missing=0
        AND (NOT gpsr_required OR (gpsr_missing=0 AND lower(gpsr_status) IN ('ready','complete','confirmed','source_confirmed')))
        AND sale_available
        AND agent_issues=0
      ) ready,
      CASE
        WHEN remote_status='SOLDOUT' THEN 'wyprzedane'
        WHEN NOT sale_available OR remote_status IN ('CLOSED','INACTIVE') THEN 'wstrzymane'
        WHEN remote_status IN ('PENDING','PROCESSING','VERIFYING') THEN 'publikowanie'
        WHEN remote_status IN ('REJECTED','ERROR','VERIFICATION_ERROR') THEN 'aktualizacja'
        ...
        WHEN (
          ...
          AND category_tree_valid AND attribute_schema_known AND required_attribute_missing=0
          AND (NOT gpsr_required OR (gpsr_missing=0 AND lower(gpsr_status) IN ('ready','complete','confirmed','source_confirmed')))
