...
const FULL_DATA_MAP_CACHE_MS = 10_000;
...
function centralCatalogListProduct(product = {}, catalogMeta = {}, { admin = false } = {}) {
  const fields = [
    ...
    'sourceUrl', 'producentUrl', 'urlProducenta', 'allegroOfferId', 'allegroProductId', 'allegroCategoryId',
    'allegroCategoryName', 'allegroCategoryResolution', 'allegroCatalogIdentityConfirmed',
    'allegroParameters', 'allegroResponsibleProducer', 'allegroSafetyInformation', 'badge',
    ...
    'allegroAgentPreparationNextRetryAt', 'allegroAgentPreparationCheckedAt',
    'allegroAgentPreparationError', 'allegroAgentPreparationSource', 'allegroAgentSavedFields',
  ];
  if (admin) fields.push(
    ...
    'vonHalskyCategoryId', 'vonHalskyAttributes', 'vonHalskyAttributeDefinitions',
    'vonHalskyRequiredAttributesMissing', 'vonHalskyAttributesVerifiedAt', 'vonHalskyAttributesUpdatedAt',
    'vonHalskyOfferId', 'vonHalskyCommandId',
    ...
    'vonHalskyDoesNotRequireGpsrInfo', 'vonHalskySafetyInformation',
...
export function centralCatalogProductFingerprint(product = {}) {
  const stable = { ...asObject(product) };
  if (own(stable, '_catalog')) {
    stable._catalog = { ...asObject(stable._catalog) };
    delete stable._catalog.sourceRevision;
    delete stable._catalog.syncedAt;
  }
...
export function centralCatalogBuildRecords(data = {}, {
  ...
  return merged.products.map((product) => {
    ...
    // Rewizja całego źródła i czas przebudowy nie są zmianą produktu. Gdyby
    // weszły do fingerprintu, każda synchronizacja blokowałaby i zapisywała
    // wszystkie kartoteki nawet przy identycznych danych biznesowych.
    return {
      ...
      fingerprint: centralCatalogProductFingerprint(adminProduct), updatedAt: now, image,
    };
  }).filter(Boolean);
}
...
export function createCentralProductCatalog({ pool, namespace = 'artway-sklep' } = {}) {
  ...
  const synchronizeCatalog = createCentralProductSynchronizer({
  ...
  const synchronize = async (...args) => {
    const result = await synchronizeCatalog(...args);
    if (result?.synchronized) clearCatalogCaches();
    return result;
  };
  ...
  const getMany = async (ids = [], { admin = false } = {}) => {
    if (!available) return new Map();
    await ensureSchema();
    const productIds = [...new Set(asArray(ids).map((id) => text(id, 120)).filter(Boolean))].slice(0, 1000);
    if (!productIds.length) return new Map();
    const result = admin
      ? await pool.query(`
          SELECT records.product_id,
            (records.data || ${agentReviewJsonSql('records.namespace', 'records.product_id')}) product
          FROM artway_product_records records
          WHERE records.namespace=$1 AND records.product_id=ANY($2::text[])
            AND records.record_status<>'removed'
        `, [ns, productIds])
      : await pool.query(`
          SELECT product_id,public_data product
          FROM artway_storefront_products
          WHERE namespace=$1 AND product_id=ANY($2::text[]) AND record_status='active'
        `, [ns, productIds]);
    return new Map(result.rows.map((row) => [String(row.product_id), row.product]));
  };
  ...
  const listDataMap = async ({ includeTrash = true, pageSize = 200 } = {}) => {
    // Pełna kartoteka zawiera rozbudowane treści, zdjęcia i dane kanałów.
    // Mniejsze strony chronią pamięć procesu i mieszczą każdy odczyt w limicie
    // PostgreSQL również wtedy, gdy równolegle pracują synchronizacje kanałów.
    const safePageSize = Math.max(1, Math.min(2000, Number(pageSize) || 200));
    const cacheKey = `${includeTrash ? 'all' : 'active'}:${safePageSize}`;
    let cached = dataMapCache.get(cacheKey);
    if (!cached || Date.now() - cached.at >= FULL_DATA_MAP_CACHE_MS) {
      const promise = (async () => {
        const products = new Map();
        let afterId = '';
        do {
          const page = await listDataPage({ afterId, limit: safePageSize, includeTrash });
      ...
      dataMapCache.set(cacheKey, cached);
      promise.catch(() => {
        if (dataMapCache.get(cacheKey)?.promise === promise) dataMapCache.delete(cacheKey);
      });
    }
    // Osobna mapa chroni współdzielony snapshot przed dodaniem lub usunięciem
    // wpisu przez odbiorcę. Dane produktu są traktowane jak niemutowalny odczyt.
    return new Map(await cached.promise);
  };
  ...
  const mutations = createCentralProductMutations({
    ...
    centralCatalogMissingFields, centralCatalogProductFingerprint,
  });
  const invalidateAfterMutation = (operation) => async (...args) => {
    const result = await operation(...args);
    if (result?.updated || result?.deleted) clearCatalogCaches();
    return result;
  };
  const upsertProduct = invalidateAfterMutation(mutations.upsertProduct);
  const upsertImportedProduct = invalidateAfterMutation(mutations.upsertImportedProduct);
  const setRecordStatus = invalidateAfterMutation(mutations.setRecordStatus);
  const purgeProduct = invalidateAfterMutation(mutations.purgeProduct);
  const patchProductFields = invalidateAfterMutation(mutations.patchProductFields);
