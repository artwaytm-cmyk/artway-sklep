...
function executionPolicy(priority = {}, text = (value) => String(value || '')) {
  ...
  const definitions = {
    orders_start: { actionId: 'orders_start', execution: 'approval', requiresApproval: true, actionLabel: 'Otwórz nowe zamówienia', deadlineMinutes: 30, owner: 'obsługa zamówień', doneWhen: 'Każde nowe zamówienie ma rozpoczętą obsługę i sprawdzoną dostępność.' },
    allegro_reply: { actionId: 'allegro_reply', execution: 'approval', requiresApproval: true, actionLabel: 'Otwórz wiadomości', deadlineMinutes: 60, owner: 'obsługa klienta', doneWhen: 'Klient otrzymał zatwierdzoną odpowiedź albo sprawa została zamknięta wewnętrznie.' },
    supplier_availability: { actionId: 'supplier_availability', execution: 'approval', requiresApproval: true, actionLabel: 'Podejmij decyzje', deadlineMinutes: 120, owner: 'administrator / Agent AI', doneWhen: 'Każdy brak ma decyzję: termin dalszej sprzedaży, ukrycie albo automatyczne wznowienie.' },
    inpost_prepare: { actionId: 'inpost_prepare', execution: 'approval', requiresApproval: true, actionLabel: 'Przejdź do wysyłek', deadlineMinutes: 120, owner: 'centrum wysyłek', doneWhen: 'Przesyłka ma etykietę, numer nadania i zapisany status InPost.' },
    allegro_warehouse: { actionId: 'allegro_warehouse', execution: 'draft', requiresApproval: false, actionLabel: 'Sprawdź zamówienia', deadlineMinutes: 60, owner: 'Agent AI', doneWhen: 'Pozycje zlecenia są sprawdzone, a realne braki dopisane do szkicu producenta.' },
    warehouse_location: { actionId: 'warehouse_location', execution: 'draft', requiresApproval: false, actionLabel: 'Otwórz lokalizacje', deadlineMinutes: 240, owner: 'magazyn', doneWhen: 'Towar pozostaje zarezerwowany do zamówienia, a magazyn przypisał mu fizyczną lokalizację.' },
    ...
    supplier_order_draft: { actionId: 'supplier_order_draft', execution: 'draft', requiresApproval: false, actionLabel: 'Otwórz plan zakupu', deadlineMinutes: 240, owner: 'Agent AI', doneWhen: 'Bieżący dokument producenta zawiera wszystkie niepokryte braki i czeka na zatwierdzenie.' },
    invoice_draft: { actionId: 'invoice_draft', execution: 'draft', requiresApproval: false, actionLabel: 'Otwórz faktury', deadlineMinutes: 240, owner: 'Agent AI / inFakt', doneWhen: 'Zamówienie firmowe ma szkic lub powiązaną fakturę inFakt.' },
    catalog_quality: { actionId: 'catalog_quality', execution: 'draft', requiresApproval: false, actionLabel: 'Otwórz audyt katalogu', deadlineMinutes: 240, owner: 'Agent AI • jakość danych', doneWhen: 'Każdy krytyczny błąd katalogu ma poprawkę albo udokumentowaną przyczynę.' },
    diagnostics_review: { actionId: 'diagnostics_review', execution: 'draft', requiresApproval: false, actionLabel: 'Otwórz diagnostykę', deadlineMinutes: 15, owner: 'Codex • diagnostyka', doneWhen: 'Źródło błędu zostało naprawione, a kontrola po wdrożeniu nie odtwarza wpisu.' },
    producer_link_check: { actionId: 'producer_link_check', execution: 'safe_check', safeProfile: 'data', requiresApproval: false, actionLabel: 'Sprawdź kolejną partię', deadlineMinutes: 360, owner: 'Agent AI', doneWhen: 'Link został sprawdzony, a wynik i brakujące pola zapisane przy produkcie.' },
    site_function_check: { actionId: 'site_function_check', execution: 'safe_check', safeProfile: 'health', requiresApproval: false, actionLabel: 'Uruchom kontrolę', deadlineMinutes: 15, owner: 'Agent AI', doneWhen: 'Baza oraz wszystkie wymagane integracje odpowiadają poprawnie.' },
    data_sync: { actionId: 'data_sync', execution: 'safe_check', safeProfile: 'data', requiresApproval: false, actionLabel: 'Odśwież dane', deadlineMinutes: 15, owner: 'Agent AI', doneWhen: 'Dane sklepu, Allegro, InPost i inFakt mają świeży znacznik synchronizacji.' },
  };
  let key = 'orders_start';
  if (title.includes('centralna diagnostyka')) key = 'diagnostics_review';
  else if (area === 'system') key = 'site_function_check';
  ...
  else if (title.includes('katalog produktów') || title.includes('kontroli jakości')) key = 'catalog_quality';
...
export function createAgentOperationalCenter(deps = {}) {
  ...
  return async function agentOperationalCenter() {
    ...
    addPriority('critical', 'system', missingIntegrations.length, 'Funkcje krytyczne strony wymagają kontroli', '#/admin/system/diagnostyka', `Sprawdź brakujące integracje: ${missingIntegrations.join(', ')}.`);
    addPriority('critical', 'synchronizacja', staleSources.length, 'Dane operacyjne są nieaktualne', '#/admin/agent-ai/obsluga', `Uruchom bezpieczne odświeżenie: ${staleSources.join(' • ')}.`);
    ...
    addPriority('info', 'produkty', producerLinks.length, 'Linki producentów czekają na pobranie danych', '#/admin/agent-ai/producenci', 'Ponów analizę linków i uzupełnij kartoteki.');
