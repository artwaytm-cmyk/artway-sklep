...
import { shippingMethod, storeShippingConfig } from './store-shipping-config.mjs';
...
export function bezpieczneZamowienieKlienta(raw, settingsData = {}) {
  ...
  const shipping = storeShippingConfig(settingsData);
  const delivery = shippingMethod(shipping, deliveryId);
  const deliveryCost = discountResult.freeDelivery || afterDiscount >= shipping.freeFrom ? 0 : kwota(delivery.price);
  const weekend = source.paczkaWeekend === true;
  const weekendCost = weekend ? shipping.weekendPrice : 0;
  ...
  const paymentDefaults = { pobranie: { nazwa: 'Za pobraniem — płatność przy odbiorze', oplata: shipping.cashOnDeliveryPrice }, telefon: { nazwa: 'Przelew na telefon', oplata: 0 }, paynow: { nazwa: 'mBank Paynow', oplata: 0 } };
  ...
  return {
    ...
    razem: total,
    status: 'nowe',
    dostawa: tekst(delivery.name, 160), dostawaId: deliveryId, dostawaKoszt: deliveryCost,
