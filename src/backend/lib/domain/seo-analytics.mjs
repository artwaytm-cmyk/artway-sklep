...
import { loadStorefrontSeoProductsByIds } from './storefront-seo-catalog.mjs';
...
function enrichPerformanceProducts(report = {}, activeProducts = []) {
  const catalog = new Map((Array.isArray(activeProducts) ? activeProducts : [])
    .filter((product) => product?.id != null)
    .map((product) => [String(product.id), product]));
  let historicalProductsExcluded = 0;
  const products = [];
  for (const item of Array.isArray(report.products) ? report.products : []) {
    const product = catalog.get(String(item?.productId ?? ''));
    if (!product) { historicalProductsExcluded += 1; continue; }
    products.push({
      ...item,
      productName: String(product.nazwa || product.name || '').trim(),
      productImage: String(product.zdjecie || product.image || product.imageUrl || '').trim(),
      productExternalId: String(product.externalId || '').trim(),
      productSku: String(product.sku || '').trim(),
      productEan: String(product.ean || product.gtin || '').trim(),
    });
  }
  return { ...report, products, historicalProductsExcluded };
}
...
export async function handleSeoAnalytics(request) {
  if (request.method === 'GET') {
    ...
    const report = performance(state, { days: url.searchParams.get('days'), from: url.searchParams.get('from'), to: url.searchParams.get('to'), entryDomain: url.searchParams.get('domain'), channel: url.searchParams.get('channel') });
    const activeProducts = await loadStorefrontSeoProductsByIds(report.products.map((item) => item.productId));
    return Response.json({ ok: true, ...enrichPerformanceProducts(report, activeProducts) }, { headers: { 'cache-control': 'no-store' } });
  }
...
export const seoAnalyticsInternals = Object.freeze({ incrementState, performance, normalizeRange, enrichPerformanceProducts });
