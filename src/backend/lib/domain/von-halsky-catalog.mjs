...
export function vonHalskyDefaultSettings() {
  return {
    ...
    agentPreparationEnabled: true,
    agentCategoryAutoMatchEnabled: true,
    agentAttributeAutoMatchEnabled: true,
    agentMinimumConfidence: 0.82,
    newOfferPublicationMode: 'manual_selection',
...
export function normalizeVonHalskySettings(raw = {}, previous = {}) {
  ...
  const defaultStock = Math.max(minimumStock, Math.min(maximumStock, integer(raw.defaultStock, integer(previous.defaultStock, defaults.defaultStock, 0, 99999), 0, 99999)));
  ...
  return {
    ...
    agentPreparationEnabled: typeof raw.agentPreparationEnabled === 'boolean' ? raw.agentPreparationEnabled : previous.agentPreparationEnabled !== false,
    agentCategoryAutoMatchEnabled: typeof raw.agentCategoryAutoMatchEnabled === 'boolean' ? raw.agentCategoryAutoMatchEnabled : previous.agentCategoryAutoMatchEnabled !== false,
    agentAttributeAutoMatchEnabled: typeof raw.agentAttributeAutoMatchEnabled === 'boolean' ? raw.agentAttributeAutoMatchEnabled : previous.agentAttributeAutoMatchEnabled !== false,
    agentMinimumConfidence: Math.max(0.55, Math.min(0.99, Number(raw.agentMinimumConfidence ?? previous.agentMinimumConfidence ?? defaults.agentMinimumConfidence) || defaults.agentMinimumConfidence)),
    newOfferPublicationMode: 'manual_selection',
    catalogAutomationEnabled: false,
...
function vonHalskyAttributeDefinitions(product = {}) {
  return (Array.isArray(product.vonHalskyAttributeDefinitions) ? product.vonHalskyAttributeDefinitions : [])
    .map((item) => ({
      id: uuid(item?.id || item?.attributeId || item?.uuid),
      name: text(item?.name || item?.label || item?.displayName, 240),
      required: item?.required === true || item?.isRequired === true,
      multiple: item?.multiple === true || item?.multiselect === true,
      type: text(item?.type || item?.valueType || item?.dataType, 80).toUpperCase(),
    }))
    .filter((item) => item.id && item.name);
}

function vonHalskyMissingRequiredAttributes(product = {}, definitions = vonHalskyAttributeDefinitions(product)) {
  const source = product.vonHalskyAttributes && typeof product.vonHalskyAttributes === 'object'
    ? product.vonHalskyAttributes
    : {};
  return definitions.filter((definition) => {
    ...
    return definition.multiple !== true && values.length !== 1;
  }).map((definition) => definition.name);
}

function vonHalskySafetyInformation(product = {}) {
  const direct = text(product.vonHalskySafetyInformation || product.safetyInformation, 100_000);
  if (direct) return direct;
  const allegro = product.allegroSafetyInformation;
  return allegro && typeof allegro === 'object' && text(allegro.type, 40).toUpperCase() === 'TEXT'
    ? text(allegro.description, 100_000)
    : '';
}

function gpsrPhone(value = '') {
...
export function vonHalskyProductReadiness(product = {}) {
  ...
  const categoryTreeValid = product.vonHalskyCategoryTreeValid !== false
    && product.vonHalskyCategoryRejection?.rejected !== true;
  const doesNotRequireGpsrInfo = product.vonHalskyDoesNotRequireGpsrInfo === true;
  const gpsrRequired = !doesNotRequireGpsrInfo;
  ...
  const attributeDefinitions = vonHalskyAttributeDefinitions(product);
  const attributeSchemaKnown = Array.isArray(product.vonHalskyAttributeDefinitions);
  const missingRequiredAttributes = vonHalskyMissingRequiredAttributes(product, attributeDefinitions);
  const safetyInformation = vonHalskySafetyInformation(product);
  const issues = [];
  const warnings = [];
  ...
  if (!text(product.vonHalskyExternalId || product.externalId || product.sku || product.id, 160)) warnings.push('Brak stabilnego EXTERNAL_ID');
  if (images.length === 1) warnings.push('Warto dodać więcej niż jedno zdjęcie');
  if (!attributeSchemaKnown) warnings.push('Nie sprawdzono aktualnych parametrów kategorii Von Halsky');
  if (missingRequiredAttributes.length) warnings.push(`Brak wymaganych parametrów: ${missingRequiredAttributes.join(', ')}`);
  if (gpsrRequired && !safetyInformation) warnings.push('Brak informacji bezpieczeństwa GPSR; uzupełnij ją, jeśli dotyczy produktu');
  const publicationIssues = [
    ...(!categoryId ? ['Brak kategorii Von Halsky'] : []),
    ...(categoryId && !categoryTreeValid ? ['Kategoria nie występuje w aktualnym drzewie Von Halsky'] : []),
    ...(!brand ? ['Brak marki wymaganej przez kontrakt Von Halsky'] : []),
    ...(gpsrMissing.length ? [`Niekompletne dane GPSR: ${gpsrMissing.join(', ')}`] : []),
    ...(!attributeSchemaKnown ? ['Nie pobrano aktualnej listy parametrów kategorii Von Halsky'] : []),
    ...(missingRequiredAttributes.length ? [`Brak wymaganych parametrów kategorii: ${missingRequiredAttributes.join(', ')}`] : []),
  ];
  ...
  return {
    ...
    responsibleProducer,
    gpsrRequired,
    doesNotRequireGpsrInfo,
    gpsrMissing,
    safetyInformation,
    attributeSchemaKnown,
    categoryTreeValid,
    attributeDefinitions,
    missingRequiredAttributes,
    publishable: issues.length === 0 && publicationIssues.length === 0,
    publicationIssues,
...
export function vonHalskyOfferProjection(product = {}, settings = {}) {
  ...
  const defaultStock = Math.max(minimumStock, Math.min(maximumStock, Number(settings.defaultStock) || minimumStock || 1));
  const physicalStock = Math.max(0, Number(product.stan ?? product.stock) || 0);
  return {
    externalId: text(product.vonHalskyExternalId || product.externalId || product.sku || product.id, 160),
    ...
    doesNotRequireGpsrInfo: readiness.doesNotRequireGpsrInfo,
    safetyInformation: readiness.safetyInformation,
    batchNumber: text(product.vonHalskyBatchNumber, 500),
    ceMarking: typeof product.vonHalskyCeMarking === 'boolean' ? product.vonHalskyCeMarking : undefined,
    images: productImages(product),
    price: readiness.price,
    currency: 'PLN',
    available,
    stock: available ? Math.max(minimumStock, Math.min(maximumStock, physicalStock > 0 ? physicalStock : defaultStock)) : 0,
    readiness,
  };
}
...
export function vonHalskyOfferProposal(projection = {}, {
  ...
  if (projection?.readiness?.publishable !== true || !categoryId) {
  ...
  if (projection.doesNotRequireGpsrInfo === true) {
    proposal.gpsr = { doesNotRequireGpsrInfo: true };
  } else {
    const party = projection.responsibleProducer || {};
    const manufacturer = {
      name: text(party.legalName || party.name, 500),
      email: text(party.email, 500).toLowerCase(),
      unstructuredAddress: text(party.address, 300),
    };
    const phone = gpsrPhone(party.phone);
    if (phone) manufacturer.phone = phone;
    proposal.gpsr = {
      manufacturer,
      doesNotRequireGpsrInfo: false,
    };
    if (projection.safetyInformation) proposal.gpsr.safetyInformation = text(projection.safetyInformation, 100_000);
    if (projection.batchNumber) proposal.gpsr.batchNumber = text(projection.batchNumber, 500);
    if (typeof projection.ceMarking === 'boolean') proposal.gpsr.ceMarking = projection.ceMarking;
  }
  return proposal;
}
