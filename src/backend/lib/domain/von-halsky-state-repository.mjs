...
import { applyVonHalskyOrderRecordFilters, VON_HALSKY_ORDER_FACETS_SELECT, vonHalskyRecordOrderBy } from './von-halsky-order-record-query.mjs';
...
const effectiveOfferStatusSql = `CASE
  WHEN status NOT IN ('PUBLISHED','CLOSED','SOLDOUT','INACTIVE','REJECTED','ERROR')
    AND (
      (CASE WHEN jsonb_typeof(data->'validationErrors')='array' THEN jsonb_array_length(data->'validationErrors') ELSE 0 END)>0
      OR (CASE WHEN jsonb_typeof(data->'rejectionReasons')='array' THEN jsonb_array_length(data->'rejectionReasons') ELSE 0 END)>0
      OR (CASE WHEN jsonb_typeof(data#>'{metadata,validationErrors}')='array' THEN jsonb_array_length(data#>'{metadata,validationErrors}') ELSE 0 END)>0
      OR (CASE WHEN jsonb_typeof(data#>'{metadata,rejectionReasons}')='array' THEN jsonb_array_length(data#>'{metadata,rejectionReasons}') ELSE 0 END)>0
    )
...
export function createVonHalskyStateRepository({
  ...
  async function readStatus(fallback) {
    ...
    const [state, statuses, commands] = await Promise.all([
      ...
      pool.query(`
        SELECT ${effectiveOfferStatusSql} status,COUNT(*)::integer count
    ...
    const truth = {
      ...
      verificationErrors: statusMap.VERIFICATION_ERROR || 0,
      rejected: (statusMap.REJECTED || 0) + (statusMap.ERROR || 0),
      soldout: statusMap.SOLDOUT || 0,
  ...
  async function readDashboardSummary(fallback = {}) {
    ...
    const [status, orders, daily, recent, rejected, operationalOrders] = await Promise.all([
      readStatus(fallback),
      pool.query(`
      ...
      pool.query(`
        ...
        FROM artway_von_halsky_records
        WHERE namespace=$1 AND kind='offers'
          AND (${effectiveOfferStatusSql}) IN ('VERIFICATION_ERROR','REJECTED','ERROR')
        ORDER BY ordinal
        LIMIT 200
      `, [namespace]),
      pool.query(`
        SELECT COUNT(*)::integer active
        FROM artway_von_halsky_records
        WHERE namespace=$1 AND kind='orders'
          AND status IN ('CREATED','NEW','PAID','ACCEPTED','PROCESSING','READY')
          AND COALESCE(NULLIF(data#>>'{_artwayShipment,trackingNumber}',''),NULLIF(data#>>'{_artwayShipment,inpostId}',''),NULLIF(data#>>'{delivery,parcels,0,trackingNumber}',''),'')=''
      `, [namespace]),
    ]);
    ...
    const activeOrders = Number(operationalOrders.rows[0]?.active) || 0;
  ...
  async function readRecordPage(kind, raw = {}) {
    ...
    const fulfillment = text(raw.fulfillment, 40).toLowerCase();
    const period = text(raw.period, 20).toLowerCase();
    const delivery = text(raw.delivery, 30).toLowerCase();
    const sort = text(raw.sort, 30).toLowerCase();
    ...
    if (period === 'dzisiaj') clauses.push("updated_at>=date_trunc('day',NOW())");
    else if (['7', '30', '90'].includes(period)) {
      values.push(Number(period));
      clauses.push(`updated_at>=NOW()-($${values.length}::integer*INTERVAL '1 day')`);
    }
    const facetsClauses = kind === 'orders' ? applyVonHalskyOrderRecordFilters(clauses, { delivery, fulfillment }) : [...clauses];
    const facetsValues = [...values];
    const where = clauses.join(' AND '), countValues = [...values];
    const orderBy = vonHalskyRecordOrderBy(sort);
    ...
    const [rows, count, facets] = await Promise.all([
      ...
      kind === 'orders' ? pool.query(`${VON_HALSKY_ORDER_FACETS_SELECT}\nWHERE ${facetsClauses.join(' AND ')}`, facetsValues) : Promise.resolve({ rows: [{}] }),
    ]);
    const hasMore = rows.rows.length > limit, selected = rows.rows.slice(0, limit);
    return {
      items: selected.map((row) => {
        const data = { ...object(row.data), _recordId: row.record_id, _status: row.status, _updatedAt: row.updated_at };
        return kind === 'orders' ? { ...data, _fulfillment: vonHalskyOrderShippingStage(data) } : data;
      }),
  ...
  async function readProductQueue(raw = {}) {
    ...
    if (stage === 'sprzedaz') clauses.push("remote_status='PUBLISHED'");
    else if (stage && stage !== 'wszystkie') add('stage=?', stage);
    const channel = text(raw.channel, 40);
    if (channel === 'aktywne') clauses.push("remote_status='PUBLISHED'");
    else if (channel === 'wyprzedane') clauses.push("remote_status='SOLDOUT'");
    else if (channel === 'weryfikacja') clauses.push("remote_status IN ('PENDING','PROCESSING','VERIFYING')");
    else if (channel === 'blad-weryfikacji') clauses.push("remote_status='VERIFICATION_ERROR'");
    ...
    if (text(raw.selection, 40) === 'ids') {
      const selected = await pool.query(`${VON_HALSKY_PRODUCT_QUEUE_SQL}
        SELECT product_id
        FROM classified
        WHERE ${where}
      ...
      return {
        items: [],
        selectedIds: selected.rows.map((row) => String(row.product_id || '')).filter(Boolean),
      ...
      pool.query(`${VON_HALSKY_PRODUCT_QUEUE_SQL}
        SELECT
          ...
          (SELECT jsonb_build_object(
            ...
            'selling',COUNT(*) FILTER(WHERE remote_status='PUBLISHED')::integer,
            ...
            'sold_out',COUNT(*) FILTER(WHERE stage='wyprzedane')::integer,
