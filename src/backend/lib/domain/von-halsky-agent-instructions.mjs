export const VON_HALSKY_AGENT_VERSION = '2026-08-06.5';
...
ZAPAMIĘTANE ROZWIĄZANIA PROBLEMÓW
- Każdy przycisk „Przygotuj” w sklepie, Allegro albo Von Halsky uruchamia ten
  sam pełny przegląd centralnej kartoteki. Samo ukończenie Allegro nie jest
  sukcesem; wymagane są także gotowy Von Halsky i końcowy odczyt jakości.
- Ręczne zaznaczenie administratora ma pierwszeństwo przed backlogiem i jest
  wykonywane po bezpiecznym dokończeniu aktualnej kartoteki.
- Odrzucona albo błędnie zweryfikowana oferta wraca do tej samej centralnej
  kartoteki. Zachowaj externalId i potwierdzone offerId; nie twórz duplikatu.
- Przed wysłaniem odśwież kategorię końcową oraz jej aktualny schemat
  atrybutów. Usuń wartości z poprzedniej kategorii, których nowy schemat nie
  zna, i uzupełniaj wyłącznie wartości potwierdzone.
- Sprzeczny EAN, kod producenta, externalId albo wiele ofert dla jednej
  tożsamości oznacza kwarantannę i konkretny brak do decyzji, nigdy zgadywanie.
- Po komendzie publikacji status PENDING lub PROCESSING jest prawidłowym
  oczekiwaniem. PUBLISHED potwierdza sukces; REJECTED, ERROR i
  VERIFICATION_ERROR uruchamiają naprawę dokładnych pól zwróconych przez API.
- Brak oferty po pojedynczym odczycie nie oznacza porażki. Wykonaj wymagane
  odczyty kontrolne i zachowaj identyfikatory komendy, aby ponowienie nie
  utworzyło drugiej oferty.
- Każdy zapis zakończ ponownym odczytem centralnej kartoteki. Każdą publikację
  zakończ odczytem zdalnego offerId i statusu. Bez tych dowodów wynik nie jest
  zakończony.
`.trim();
