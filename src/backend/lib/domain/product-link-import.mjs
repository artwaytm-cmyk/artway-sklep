    ...
    for (const decision of decisions) {
      ...
      try {
        prepared = asObject(await prepareProduct(current.url, {
          jobId: job.id, itemId: current.id, rowNumber: current.rowNumber, name: current.name, fileData: current.fileData, review: true,
        }));
      } catch (error) {
      ...
      product.parametryZrodla = {
        ...asObject(product.parametryZrodla),
        ...Object.fromEntries([
          ['Minimalny wiek dziecka', product.minimalnyWiek],
          ['Liczba elementów', product.liczbaElementow],
          ['Materiał', product.material],
          ['Kolor produktu', product.kolorProduktu],
          ['Waga produktu', product.waga],
          ['Długość produktu', product.dlugosc],
          ['Szerokość produktu', product.szerokosc],
          ['Wysokość produktu', product.wysokosc],
        ].filter(([, value]) => clean(value, 500))),
      };
      if (product.gpsrNazwa || product.gpsrAdres || product.gpsrEmail || product.gpsrTelefon) {
        product.vonHalskyResponsibleProducer = {
          ...asObject(product.vonHalskyResponsibleProducer),
          legalName: clean(product.gpsrNazwa, 300),
          address: clean(product.gpsrAdres, 500),
          email: clean(product.gpsrEmail, 320).toLowerCase(),
          phone: clean(product.gpsrTelefon, 80),
          source: 'spreadsheet-import-review',
        };
      }
      for (const field of ['gpsrNazwa', 'gpsrAdres', 'gpsrEmail', 'gpsrTelefon']) delete product[field];
