...
export function deterministicProductEditorialFallback(product = {}) {
  ...
  const base = {
    ...
    // Nie zachowujemy starego pola kanałowego, jeżeli zawierało skopiowany
    // interfejs strony producenta. Krótki opis powstaje z tego samego,
    // oczyszczonego materiału co opis sklepu.
    allegroShortDescription: shortDescription,
    ...
    vonHalskyDescription: longDescription,
    vonHalskyContentUpdatedAt: timestamp,
    vonHalskyContentSource: 'deterministic-source-policy',
  ...
  if (!editorialProductContentReport(base, 'store').ready
    || !editorialProductContentReport(base, 'allegro').ready
    || !editorialProductContentReport(base, 'vonHalsky').ready) return null;
