...
export function mergeImportedProductSourceRefresh(existing = {}, incoming = {}, refreshedAt = timestamp()) {
  ...
  // Cena z arkusza może uzupełnić wyłącznie kartotekę bez poprawnej ceny.
  // Istniejąca dodatnia cena nadal pozostaje chroniona przed importem źródłowym.
  if (!(Number(result.cena) > 0) && Number(incoming.cena) > 0) result.cena = Math.round(Number(incoming.cena) * 100) / 100;
