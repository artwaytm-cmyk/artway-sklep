export function allegroPatchZDraftu(draft = {}, options = {}) {
  ...
  if ((options.contentOnly !== true || options.includeCatalogProduct === true) && Array.isArray(draft.productSet) && draft.productSet[0]?.product?.id) {
    out.productSet = options.contentOnly === true
      ? draft.productSet.map((item) => ({
          product: { id: item.product.id },
          ...(item.quantity?.value ? { quantity: { value: item.quantity.value } } : {}),
        }))
      : draft.productSet;
  }
...
export function allegroPublicationConfirmation({ expectedStatus = '', publicationWait = null, verifiedOffer = null } = {}) {
  const expected = String(expectedStatus || '').trim().toUpperCase();
  const status = String(verifiedOffer?.publication?.status || verifiedOffer?.status || '').trim().toUpperCase();
  return { status, confirmed: expected ? publicationWait?.completed === true && status === expected : Boolean(verifiedOffer?.id) };
}
