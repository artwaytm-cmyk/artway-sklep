...
export function reconcileSupplierOrderDrafts(input = {}) {
  ...
  const closedOrderReferences = new Set(array(input.closedOrderReferences).map((value) => text(value, 180)).filter(Boolean));
  const diagnostics = {
    skippedInactiveOrders: 0,
    skippedItemsWithoutProductId: 0,
    inventoryShortfallProducts: 0,
    inventoryShortfallQuantity: 0,
    skippedClosedShortfallQuantity: 0,
    legacyWithoutMovement: [],
    duplicateEditableDrafts: [],
    duplicateDraftIds: [],
  };
  ...
  // Plan dotyczy wyłącznie bieżącego zapotrzebowania. Niedobór historycznie
  // zapisany przy wydaniu pozostaje w audycie magazynowym, ale nie może tworzyć
  // zakupu dla zamówienia, które zostało już wysłane lub zamknięte.
  for (const [rawProductId, rawShortfall] of Object.entries(object(settings.artway_magazyn_niedobory_wydan))) {
    const productId = text(rawProductId, 160);
    if (!productId) continue;
    const shortfall = typeof rawShortfall === 'number' ? { quantity: rawShortfall } : object(rawShortfall);
    const allDescribedOrders = array(shortfall.orders).map((row) => ({
      orderId: text(firstValue(row?.orderNumber, row?.number), 180),
      quantity: quantity(row?.quantity),
    })).filter((row) => row.orderId && row.quantity > 0);
    const describedOrders = allDescribedOrders.filter((row) => !closedOrderReferences.has(row.orderId));
    diagnostics.skippedClosedShortfallQuantity += allDescribedOrders
      .filter((row) => closedOrderReferences.has(row.orderId))
      .reduce((sum, row) => sum + row.quantity, 0);
    // Gdy rejestr zawiera rozpisane zamówienia, suma tych pozycji jest źródłem
    // prawdy. Nie wolno ponownie dodać odfiltrowanej, zamkniętej ilości jako
    // anonimowego „zaległego wydania”. Dla starego formatu liczbowego zachowujemy
    // zgodność i nadal traktujemy ilość jako nierozpisany brak.
    const aggregateQuantity = allDescribedOrders.length ? 0 : quantity(shortfall.quantity);
    const requests = [...describedOrders];
    if (aggregateQuantity > 0) requests.push({ orderId: 'zaległe wydanie', quantity: aggregateQuantity });
    if (!requests.length) continue;
    const record = demandByProduct.get(productId) || {
      productId,
      reservedQuantity: 0,
      legacyShortage: 0,
      orderRefs: new Set(),
    ...
    for (const request of requests) {
      record.orderRefs.add(request.orderId);
      record.legacyShortage += request.quantity;
      record.orderRequests.push({
        orderId: request.orderId,
        quantity: request.quantity,
        mode: 'deducted_on_create',
        legacyShortage: request.quantity,
      });
      diagnostics.inventoryShortfallQuantity += request.quantity;
    }
    diagnostics.inventoryShortfallProducts += 1;
    demandByProduct.set(productId, record);
  }
