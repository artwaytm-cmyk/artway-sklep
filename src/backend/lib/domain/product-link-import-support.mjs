...
const REVIEW_PATCH_FIELDS = new Set([
  ...
  'cenaZakupu', 'cenaAllegro', 'cenaVonHalsky',
  'minimalnyWiek', 'liczbaElementow', 'material', 'kolorProduktu',
  'waga', 'dlugosc', 'szerokosc', 'wysokosc',
  'vonHalskyCategoryId', 'vonHalskySafetyInformation',
  'gpsrNazwa', 'gpsrAdres', 'gpsrEmail', 'gpsrTelefon',
]);
...
function normalizeImportedPrice(value) {
  if (typeof value === 'number') return Number.isFinite(value) && value > 0 && value <= 1_000_000 ? Math.round(value * 100) / 100 : 0;
  let raw = clean(value, 100).replace(/[\s\u00a0\u202f]/g, '').replace(/[^0-9,.-]/g, '');
...
function importedFileValue(row, aliases) {
  const nested = asObject(row?.fileData || row?.spreadsheetData || row?.sourceData);
  for (const source of [nested, asObject(row)]) {
...
function normalizeFileData(row = {}) {
  ...
  const result = {};
  const textFields = [
    ['nazwa', ['nazwa', 'name', 'productName', 'Nazwa produktu'], 500],
    ['kategoria', ['kategoria', 'category', 'Kategoria', 'Kategoria produktu'], 180],
    ['opisKrotki', ['opisKrotki', 'shortDescription', 'Opis krótki', 'Krótki opis'], 500],
    ['opis', ['opis', 'description', 'Opis', 'Opis produktu'], 6000],
    ['sku', ['sku', 'SKU', 'indeks', 'Indeks'], 160],
    ['externalId', ['externalId', 'external_id', 'Numer katalogowy'], 160],
    ['kodProducenta', ['kodProducenta', 'manufacturerCode', 'Kod producenta', 'Symbol producenta'], 160],
    ['mpn', ['mpn', 'MPN'], 160],
    ['minimalnyWiek', ['minimalnyWiek', 'Minimalny wiek dziecka', 'Wiek minimalny', 'Wiek od'], 120],
    ['liczbaElementow', ['liczbaElementow', 'Liczba elementów', 'Ilość elementów'], 120],
    ['material', ['material', 'Materiał', 'Materiał wykonania'], 300],
    ['kolorProduktu', ['kolorProduktu', 'Kolor produktu', 'Kolor'], 200],
    ['waga', ['waga', 'Waga', 'Waga produktu'], 120],
    ['dlugosc', ['dlugosc', 'Długość', 'Długość produktu'], 120],
    ['szerokosc', ['szerokosc', 'Szerokość', 'Szerokość produktu'], 120],
    ['wysokosc', ['wysokosc', 'Wysokość', 'Wysokość produktu'], 120],
    ['vonHalskyCategoryId', ['vonHalskyCategoryId', 'ID kategorii Von Halsky', 'Kategoria Von Halsky'], 100],
    ['vonHalskySafetyInformation', ['vonHalskySafetyInformation', 'Bezpieczeństwo GPSR', 'Informacja bezpieczeństwa', 'Ostrzeżenia'], 100_000],
    ['gpsrNazwa', ['gpsrNazwa', 'GPSR nazwa producenta', 'GPSR producent'], 300],
    ['gpsrAdres', ['gpsrAdres', 'GPSR adres'], 500],
    ['gpsrEmail', ['gpsrEmail', 'GPSR e-mail', 'GPSR email'], 320],
    ['gpsrTelefon', ['gpsrTelefon', 'GPSR telefon'], 80],
  ];
  for (const [field, aliases, max] of textFields) {
    const value = clean(importedFileValue(row, aliases), max);
    if (value) result[field] = value;
  }
  const producer = canonicalManufacturerName(importedFileValue(row, ['producent', 'manufacturer', 'Producent', 'Nazwa producenta']));
  const brand = canonicalManufacturerName(importedFileValue(row, ['marka', 'brand', 'Marka']));
  if (producer) result.producent = producer;
  if (brand) result.marka = brand;
  const price = normalizeImportedPrice(importedFileValue(row, ['cena', 'price', 'regularPrice', 'Cena', 'Cena regularna', 'Cena (regularna)', 'Cena sprzedaży']));
  if (price > 0) result.cena = price;
  for (const [field, aliases] of [
    ['cenaZakupu', ['cenaZakupu', 'Cena zakupu', 'Koszt zakupu', 'Cena hurtowa']],
    ['cenaAllegro', ['cenaAllegro', 'Cena Allegro']],
    ['cenaVonHalsky', ['cenaVonHalsky', 'Cena Von Halsky', 'Cena InPost']],
  ]) {
    const value = normalizeImportedPrice(importedFileValue(row, aliases));
    if (value > 0) result[field] = value;
  }
  const ean = clean(importedFileValue(row, ['ean', 'gtin', 'EAN', 'GTIN', 'EAN / GTIN', 'Kod EAN']), 40).replace(/\D/g, '');
  if (ean) { result.ean = ean; result.gtin = ean; }
  const image = clean(importedFileValue(row, ['zdjecie', 'image', 'imageUrl', 'Zdjęcie', 'Link do zdjęcia', 'URL zdjęcia']), 3000);
  if (image) {
    try {
      const url = new URL(image);
      if (['http:', 'https:'].includes(url.protocol) && isPublicProductHost(url.hostname) && !url.username && !url.password) result.zdjecie = url.href;
    } catch { /* błędny adres obrazu jest pomijany, nie blokuje całego importu */ }
  }
  return result;
}
...
function normalizeRows(rows) {
  ...
  return rows.map((row, index) => {
    ...
    const fileData = normalizeFileData(row);
    const name = clean(row?.name || row?.nazwa || row?.['Nazwa produktu'] || fileData.nazwa, 500);
    if (name && !fileData.nazwa) fileData.nazwa = name;
    return {
...
function importFingerprint(rows, updateExisting = false) {
  return crypto.createHash('sha256').update(`${updateExisting ? 'refresh' : 'add-only'}\n${rows.map((row) => `${row.rowNumber}\t${row.url}\t${JSON.stringify(asObject(row.fileData))}`).join('\n')}`).digest('hex');
}
...
function reviewDraft(value = {}, sourceUrl = '') {
  const source = asObject(value);
  const parameters = asObject(source.parametryZrodla);
  const gpsr = asObject(source.vonHalskyResponsibleProducer);
  const draft = {
    ...
    cenaZakupu: normalizeImportedPrice(source.cenaZakupu),
    cenaAllegro: normalizeImportedPrice(source.cenaAllegro),
    cenaVonHalsky: normalizeImportedPrice(source.cenaVonHalsky),
    minimalnyWiek: clean(source.minimalnyWiek || parameters['Minimalny wiek dziecka'], 120),
    liczbaElementow: clean(source.liczbaElementow || parameters['Liczba elementów'], 120),
    material: clean(source.material || parameters['Materiał'], 300),
    kolorProduktu: clean(source.kolorProduktu || parameters['Kolor produktu'], 200),
    waga: clean(source.waga || parameters['Waga produktu'], 120),
    dlugosc: clean(source.dlugosc || parameters['Długość produktu'], 120),
    szerokosc: clean(source.szerokosc || parameters['Szerokość produktu'], 120),
    wysokosc: clean(source.wysokosc || parameters['Wysokość produktu'], 120),
    vonHalskyCategoryId: clean(source.vonHalskyCategoryId, 100),
    vonHalskySafetyInformation: clean(source.vonHalskySafetyInformation, 100_000),
    gpsrNazwa: clean(source.gpsrNazwa || gpsr.legalName || gpsr.name, 300),
    gpsrAdres: clean(source.gpsrAdres || gpsr.address, 500),
    gpsrEmail: clean(source.gpsrEmail || gpsr.email, 320).toLowerCase(),
    gpsrTelefon: clean(source.gpsrTelefon || gpsr.phone, 80),
...
function reviewPatch(value = {}) {
  ...
  for (const field of REVIEW_PATCH_FIELDS) {
    ...
    if (['cena', 'cenaZakupu', 'cenaAllegro', 'cenaVonHalsky'].includes(field)) {
      ...
      patch[field] = Math.round(number * 100) / 100;
    } else {
      const limits = { opis: 6000, opisKrotki: 500, zdjecie: 3000, nazwa: 500, vonHalskySafetyInformation: 100_000, gpsrAdres: 500, gpsrEmail: 320, gpsrNazwa: 300, material: 300 };
...
function publicItem(item = {}) {
  ...
  const fileData = normalizeFileData({ fileData: item.fileData });
  if (Object.keys(fileData).length) result.fileData = fileData;
...
export { MAX_ROWS, ITEMS_PER_SHARD, CAS_ATTEMPTS, LEASE_MS, ITEM_STATUSES, TERMINAL, REVIEW_PATCH_FIELDS, ALEXANDER_HOSTS, BLOCKED_HOST_SUFFIXES, asArray, asObject, clean, clone, serviceError, dateFrom, isPublicProductHost, safeUrl, rowUrl, normalizeImportedPrice, normalizeFileData, normalizeRows, importFingerprint, normalizeJob, reviewDraft, reviewPatch, reviewMissing, publicItem, summary, normalizeStoredSummary };
