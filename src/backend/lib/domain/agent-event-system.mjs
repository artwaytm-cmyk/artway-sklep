  ...
  function vonHalskyFinisher({
    ...
    return async (task = {}) => {
      ...
      if (channelsReady && typeof saveProductFields === 'function') {
        const confirmationRunId = String(task.id || result.runId || '');
        const confirmationAttempt = Math.max(1, Number(task.attempt) || 1);
        const confirmedAt = String(product?.agentQualityRunId || '') === confirmationRunId
          && product?.agentQualityConfirmedAt
          ? String(product.agentQualityConfirmedAt)
          : new Date().toISOString();
        const fields = {
          agentQualityReviewStatus: 'confirmed',
          agentQualityConfirmedAt: confirmedAt,
          agentQualityReadbackConfirmed: true,
          agentQualityRunId: confirmationRunId,
        ...
        qualityConfirmation = await saveProductFields({
          productId,
          fields,
          mutationId: `agent-quality-confirmed:${productId}:${confirmationRunId || confirmedAt}:attempt-${confirmationAttempt}`,
      ...
      return {
        ...
        ready: result.status === 'ready' && qualityConfirmation?.publication?.readbackConfirmed === true,
