...
- InPost /shipments/calculate dla konta umownego postpaid:
  brak ceny albo techniczna wartość domyślna nie są rzeczywistą stawką umowną. Jeżeli aplikacja
  oznaczyła wynik jako postpaid_calculation_not_contractual i nie użyła go do rozliczenia, jest to
  prawidłowe zabezpieczenie, nie błąd kodu. Błąd aplikacji zachodzi dopiero, gdy taki wynik został
  zapisany jako koszt klienta; wtedy wymagaj testu trybu postpaid i pełnego cennika umownego.
