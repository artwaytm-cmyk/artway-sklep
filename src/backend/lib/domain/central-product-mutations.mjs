export function createCentralProductMutations(context = {}) {
  ...
  const setRecordStatus = async (id, status, {
    ...
    try {
      ...
      const fingerprint = centralCatalogProductFingerprint(data);
      await client.query(`
  ...
  const patchProductFields = async (id, fields = {}, remove = [], {
    ...
    try {
      ...
      const beforeFingerprint = centralCatalogProductFingerprint(currentData);
      ...
      const fingerprint = centralCatalogProductFingerprint(adminData);
