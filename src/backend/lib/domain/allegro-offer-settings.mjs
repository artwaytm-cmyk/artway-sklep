...
export function normalizeAllegroOfferSettings(raw = {}) {
  ...
  return {
    ...
    returnPolicyId: String(raw?.returnPolicyId || '').trim().slice(0, 120),
    returnPolicyName: String(raw?.returnPolicyName || '').trim().slice(0, 160),
    impliedWarrantyId: String(raw?.impliedWarrantyId || '').trim().slice(0, 120),
    impliedWarrantyName: String(raw?.impliedWarrantyName || '').trim().slice(0, 160),
    warrantyId: String(raw?.warrantyId || '').trim().slice(0, 120),
    warrantyName: String(raw?.warrantyName || '').trim().slice(0, 160),
