...
export const INDEXNOW_CATALOG_VERSION = '2026-08-09-visible-out-of-stock-v2';
...
export async function runIndexNowPromotion({ catalogProducts = [], changedProducts = [], config = {}, fetchImpl = globalThis.fetch } = {}) {
  const fullCatalogSubmission = !config.indexNowFullCatalogAt || config.indexNowCatalogVersion !== INDEXNOW_CATALOG_VERSION;
  ...
  try {
    ...
    return { ...await submitIndexNow(['https://artwaytm.pl/', ...products.map((product) => `https://artwaytm.pl/produkt/${encodeURIComponent(product.id)}`)], { fetchImpl }), scope, catalogVersion: INDEXNOW_CATALOG_VERSION };
  } catch (error) {
