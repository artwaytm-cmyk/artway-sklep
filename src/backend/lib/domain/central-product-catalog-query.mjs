...
export function centralCatalogQueryOptions(raw = {}) {
  ...
  // UI używa czytelnych wartości „wszystkie/wszyscy”. Nie są to dane
  // katalogowe i nie mogą trafić do WHERE category/producer. Normalizujemy je
  // przy granicy API, aby ten sam kontrakt działał dla każdego klienta.
  const selection = (value, max = 300) => {
    const selected = text(value, max);
    return ['wszystkie', 'wszyscy', 'all', '*'].includes(normalize(selected)) ? '' : selected;
  };
  return {
    query: normalize(raw.query || raw.q), category: selection(raw.category), producer: selection(raw.producer), status: text(raw.status || 'active', 40), source: text(raw.source || 'wszystkie', 40), stock: text(raw.stock || 'wszystkie', 40), allegro: text(raw.allegro || 'wszystkie', 40), data: text(raw.data || 'wszystkie', 40), sale: text(raw.sale || 'wszystkie', 40), promotion: text(raw.promotion || 'wszystkie', 40), link: text(raw.link || 'wszystkie', 40),
