...
export function reusableVonHalskyEditorial(product = {}, runId = '') {
  const report = editorialProductContentReport(product, 'vonHalsky');
  if (!report.ready) return null;
  return {
    run: { id: String(runId || `von-halsky-reused:${product.id || 'product'}`), reused: true },
    applied: { applied: false, patch: {}, persistedPatch: {} },
    automatic: true,
    retryScheduled: false,
    reused: true,
    policyReason: 'existing_verified_channel_editorial',
    violations: [],
  };
}
...
export function createVonHalskyAgentRoute(context = {}) {
  const {
    ...
    vonHalskyPublicConfig, normalizeVonHalskySettings, env, injectedApiAvailable,
  } = context;
        ...
        try {
          ...
          if (
            ...
            && (
              ...
              || (Number(workingProduct?.sourceEvidence?.imagePolicyVersion) || 0) < SOURCE_IMAGE_POLICY_VERSION
            )
          ) {
            try {
          ...
          if (categoryId && categoryIndex.size && !categoryIndex.ids.has(categoryId)) {
            ...
            deterministicFields.vonHalskyCategoryTreeValid = false;
          }
          ...
          if (!categoryId && categoryMatch.autoApplicable && state.settings.agentCategoryAutoMatchEnabled !== false) {
            ...
            deterministicFields.vonHalskyCategoryTreeValid = true;
          ...
          const selectedCategory = categoryId
            ? state.categories.find((item) => String(item?.id || '') === categoryId)
            : null;
          if (selectedCategory?.leaf === true) deterministicFields.vonHalskyCategoryTreeValid = true;
          deterministicFields.vonHalskyDoesNotRequireGpsrInfo = selectedCategory?.doesNotRequireGpsrInfo === true;
          deterministicFields.vonHalskyGpsrRequired = selectedCategory?.doesNotRequireGpsrInfo !== true;
          if (categoryId && (config.configured || injectedApiAvailable) && state.settings.agentAttributeAutoMatchEnabled !== false) {
            try {
              ...
              deterministicFields.vonHalskyAttributeDefinitions = attributeMatch.definitions;
              deterministicFields.vonHalskyRequiredAttributesMissing = attributeMatch.missingRequired;
              deterministicFields.vonHalskyAttributesVerifiedAt = timestamp;
          ...
          const agent = reusableVonHalskyEditorial(workingProduct, workId)
            || await prepareProductWithAgent(productId, actor, { source: body.source === 'automatic' ? 'automatic' : 'manual' });
          ...
          const previousConfirmation = {
            runId: String(workingProduct?.vonHalskyAgentPreparationRunId || ''),
            confirmedAt: String(workingProduct?.vonHalskyAgentConfirmedAt || ''),
          };
          ...
          const confirmationRunId = String(agent?.run?.id || workId);
          const confirmedAt = previousConfirmation.runId === confirmationRunId
          ...
          const confirmedSave = await saveProductFields({
            ...
            mutationId: `von-halsky-agent-confirmed:${productId}:${confirmationRunId}`,
            actor: matchingText(actor?.email || actor?.name || actor?.source || 'von-halsky-agent', 200),
