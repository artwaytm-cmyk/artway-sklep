...
export function allegroApplyProductSetSafety({
  ...
  // Artway sprzedaje nowe produkty. Od 12.08.2025 Allegro dopuszcza tę flagę
  // przy pierwszym wystawieniu wyłącznie dla wybranych stanów używanych.
  // Nie przenosimy więc starego pola kartoteki do nowej oferty i zawsze
  // wymagamy identyfikatora podmiotu odpowiedzialnego oraz informacji GPSR.
  delete item.marketedBeforeGPSRObligation;

  const catalogManagedSafety = Boolean(text(item?.product?.id, 120) && !text(item?.product?.idType, 40));
  const missing = [];
  // Dla dokładnie wskazanego product.id Allegro uzupełnia GPSR z Katalogu
  // Produktów. Nie tworzymy fikcyjnego tekstu ostrzeżeń i nie zatrzymujemy
  // poprawnego szkicu tylko dlatego, że wyszukiwarka nie zwróciła pełnego
  // obiektu productSafety. Dla nowego produktu bez UUID oba pola pozostają
  // obowiązkowe i muszą pochodzić z potwierdzonego źródła.
  if (!catalogManagedSafety && !item.responsibleProducer?.id) missing.push('odpowiedzialny producent GPSR');
  if (!catalogManagedSafety && !item.safetyInformation?.type) missing.push('informacja o bezpieczeństwie GPSR');
  return {
    draft: payload,
    ...
    marketedBeforeGpsrIgnored: product.marketedBeforeGPSRObligation === true,
    catalogManagedSafety,
    source: catalogManagedSafety || catalogProducers.length ? 'katalog Allegro' : (responsibleProducer ? 'kartoteka odpowiedzialnych producentów Allegro' : ''),
  };
}
