export const ALLEGRO_AGENT_OFFER_PROCEDURE = Object.freeze([
  ...
  'Przy potwierdzonym product.id uwzględnij w limicie 16 także zdjęcia automatycznie dołączane z Katalogu Allegro; najpierw rezerwuj miejsce dla galerii katalogowej, a dopiero potem dodawaj własne zdjęcia.',
  'Przy potwierdzonym product.id korzystaj z GPSR produktu katalogowego. Nie twórz fikcyjnego tekstu bezpieczeństwa tylko dlatego, że skrócony wynik wyszukiwania nie zawiera pełnego productSafety.',
  ...
  'Jeżeli zapis zwróci stan niepotwierdzony, przed ponowieniem odczytaj zapisane offerId i uzgodnij je z kartoteką. Potwierdzony stan zamyka zadanie, konflikt tożsamości zatrzymuje automat; nigdy nie twórz w ciemno drugiej oferty.',
...
export function buildAllegroPublicationSuccessFields({
  ...
  return {
    ...
    allegroAgentPreparationError: '', allegroAgentPublicationError: '', allegroAgentPublishedAt: now,
...
export function buildAllegroPublicationUnconfirmedFields({
  text = (value, limit = 1000) => String(value ?? '').slice(0, limit).trim(),
  details = {}, now = new Date().toISOString(),
} = {}) {
  const remoteStatus = text(details.verifiedOffer?.publication?.status || details.offer?.publication?.status || '', 80).toUpperCase();
  return {
    allegroOfferId: text(details.offerId, 100), ...(remoteStatus ? { allegroStatus: remoteStatus } : {}),
    allegroSyncedAt: now, allegroSyncSource: 'artway-store', allegroAgentPreparationStatus: 'ready',
    allegroPublicationAgentStatus: 'provider_unconfirmed', allegroPublicationLastAttemptAt: now,
    allegroPublicationLastErrorCode: 'allegro_publication_unconfirmed',
    allegroPublicationLastError: 'Oferta ma zapisany identyfikator, ale API nie potwierdziło jeszcze oczekiwanego statusu.',
    allegroAgentPublicationError: 'API Allegro nie potwierdziło jeszcze oczekiwanego statusu oferty.',
  };
}
