export function inpostErrorDetails(value, prefix = '') {
  ...
  const visit = (item, path) => {
    ...
    if (['string', 'number', 'boolean'].includes(typeof item)) {
      const rawMessage = String(item);
      const message = rawMessage === '[object Object]' && String(path).startsWith('custom_attributes')
        ? 'Wybrany sposób nadania nie pasuje do usługi albo wymaga poprawnego punktu nadania.'
        : rawMessage;
