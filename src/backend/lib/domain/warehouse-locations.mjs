...
export function createWarehouseLocationService({ readVersioned, writeIfVersion, now = () => new Date(), maxAttempts = 4 } = {}) {
  ...
  async function assign(body = {}, actor = 'administrator') {
    const productId = text(body.productId, 160), target = code(body.location);
    if (!productId) fail('Nie wskazano produktu do przypisania.', 'warehouse_product_required');
    if (!target) fail('Wybierz półkę dla produktu.', 'warehouse_location_required');
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      const locations = locationList(data), location = locations.find((item) => code(item?.kod) === target);
      if (!location || location.aktywna === false || location.typ !== 'półka') {
        fail('Wybrana lokalizacja nie istnieje, jest nieaktywna albo nie jest półką.', 'warehouse_location_target_invalid', 409);
      }
      const stocks = object(data.artway_stany), cards = object(data.artway_magazyn_produkty);
      if (!Object.prototype.hasOwnProperty.call(stocks, productId) && !Object.prototype.hasOwnProperty.call(cards, productId)) {
        fail('Nie znaleziono kartoteki magazynowej produktu.', 'warehouse_product_not_found', 404);
      }
      const previous = object(cards[productId]), previousLocation = code(previous.lokalizacja);
      if (previousLocation === target) return {
        ok: true, assigned: true, duplicate: true, productId, location: target,
        previousLocation, warehouseProduct: previous, rev: Math.max(0, Number(record.rev) || 0), updated_at: record.updated_at || null,
      };
      const timestamp = now().toISOString(), updatedBy = text(actor, 200) || 'administrator';
      const nextCards = {
        ...cards,
        [productId]: { ...previous, lokalizacja: target, aktualizacja: timestamp, operator: updatedBy },
      };
      const next = {
        ...record,
        data: { ...data, artway_magazyn_produkty: nextCards },
        rev: Math.max(0, Number(record.rev) || 0) + 1,
        updated_at: timestamp,
      };
      const write = await writeIfVersion('settings', next, version);
      if (write?.modified) return {
        ok: true, assigned: true, duplicate: false, productId, location: target,
        previousLocation, updatedBy, warehouseProduct: nextCards[productId], rev: next.rev, updated_at: timestamp,
      };
    }
    fail('Kartoteka magazynowa zmieniła się podczas zapisu. Odśwież widok i spróbuj ponownie.', 'warehouse_location_write_conflict', 409);
  }
  ...
  return Object.freeze({ assign, preview, remove });
}
