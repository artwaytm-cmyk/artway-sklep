...
} from './domain/von-halsky-catalog-reconciliation.mjs';
...
function initialState() {
  return {
    ...
    sync: {
      ...
      lastRequestId: '',
      postSales: {
        returns: { status: 'unknown', checkedAt: null, message: '', code: '' },
        claims: { status: 'unknown', checkedAt: null, message: '', code: '' },
      },
    },
    diagnostics: [],
    offers: [],
...
function remoteOfferSummary(details = {}) {
  ...
  const validationErrors = (Array.isArray(details?.metadata?.validationErrors)
    ? details.metadata.validationErrors
    : Array.isArray(details?.validationErrors) ? details.validationErrors : []).slice(0, 30);
  const rejectionReasons = (Array.isArray(details?.metadata?.rejectionReasons)
    ? details.metadata.rejectionReasons
    : Array.isArray(details?.rejectionReasons) ? details.rejectionReasons : []).slice(0, 30);
  const summary = {
    ...
    providerStatus: String(offer.status || ''),
    updatedAt: offer.updatedAt || null,
    validationErrors,
    rejectionReasons,
  };
  summary.status = vonHalskyEffectiveOfferStatus(summary);
  return summary;
}
...
export function createVonHalskyRoute({
  ...
  const api = injectedApi || createVonHalskyApiClient({ env: new Proxy({}, { get: (_target, key) => env()?.[key] }), fetchImpl });
  ...
  const routeContext = {
    ...
    injectedApiAvailable: Boolean(injectedApi),
    channelState,
    inpost,
    coordinateWarehouseOrders,
    inspectWarehouseOrders,
    draftMessageWithAgent,
    sendEmail,
    emailPublicConfig,
  };
  ...
  return async function vonHalskyRoute(req, url, action) {
    ...
    if (action === 'von-halsky-records') {
      ...
      const [page, overview] = await Promise.all([
        readRecordPage(kind, {
          ...
          fulfillment: url.searchParams.get('fulfillment'),
        ...
        ['returns', 'claims'].includes(kind)
          ? (typeof readOverview === 'function'
            ? readOverview(initialState())
            : readVersioned(STORE_KEY, initialState()).then((result) => result.value))
      ...
      return respond({
        ...
        kind,
        ...page,
        sourceHealth: overview?.sync?.postSales?.[kind] || null,
      });
    }
    ...
    if (action === 'von-halsky-product-category') {
      ...
      const requestedAttributes = body.attributes && typeof body.attributes === 'object' && !Array.isArray(body.attributes)
        ? body.attributes
        : (product.vonHalskyAttributes || {});
      let attributeMatch;
      try {
        const attributesResult = await api.fetchCategoryAttributes(categoryId);
        attributeMatch = matchVonHalskyAttributes({ ...product, vonHalskyAttributes: requestedAttributes }, attributesResult.payload);
      } catch (error) {
        const safe = safeError(error);
        return respond({ ok: false, error: `Nie udało się pobrać aktualnych parametrów tej kategorii: ${safe.message}`, code: safe.code, details: safe.details }, safe.status);
      }
      const fields = {
        ...
        vonHalskyDoesNotRequireGpsrInfo: category?.doesNotRequireGpsrInfo === true,
        vonHalskyGpsrRequired: category?.doesNotRequireGpsrInfo !== true,
        vonHalskyAttributeDefinitions: attributeMatch.definitions,
        vonHalskyRequiredAttributesMissing: attributeMatch.missingRequired,
        vonHalskyAgentAttributeCoverage: attributeMatch.coverage,
        vonHalskyAttributesVerifiedAt: timestamp,
      };
      fields.vonHalskyAttributes = { ...requestedAttributes, ...attributeMatch.mapped };
