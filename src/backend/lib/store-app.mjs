...
import { accountSessionHeaders, clearAccountSessionHeaders, createAccountSession, createOrderAccess, hashPassword, legacyPasswordHash, publicUser, rateLimit, requestSession as requestSignedSession, sessionMatchesAccount, verifyOrderAccess, verifyPassword } from './core/security.mjs';
import { createAdminMfaChallenge, createMfaEmailRecovery, createMfaEnrollment, decryptMfaSecret, mfaProvisioningUri, verifyAdminMfaChallenge, verifyMfaCode, verifyMfaEmailRecoveryChallenge, verifyMfaEmailRecoveryCode } from './core/mfa.mjs';
...
import { createVonHalskyStateRepository, VON_HALSKY_STORE_KEY } from './domain/von-halsky-state-repository.mjs';
...
import { createDiagnosticAgentWorkflow } from './domain/diagnostic-agent-workflow.mjs'; import { enqueueDiagnosticCodeRepair } from './domain/diagnostic-code-repair-queue.mjs';
...
import { buildSeoAutomationStatus, buildSeoChannelReport, duplicateScheduledSeoResult, isScheduledSeoSource, scheduledSeoRunForDay, seoAutomationDay } from './domain/seo-daily-automation.mjs';
import { buildStorefrontSeoCatalogSnapshot } from './domain/storefront-seo-catalog.mjs';
...
import { createAllegroPublicationQueueRoute } from './allegro-publication-queue-route.mjs';
import { allegroAutomaticPreparationDisposition, productFullPreparationReport } from './domain/allegro-preparation-queue.mjs';
...
import { createCustomerPublicationsRoute } from './customer-publications-route.mjs'; import { createContestCenterRoute } from './contest-center-route.mjs'; import { createSiteReleaseCenterRoute } from './site-release-center-route.mjs';
import { allegroOfferTitle } from './domain/allegro-offer-content.mjs';
import { allegroGalleryBudget } from './domain/allegro-gallery-budget.mjs';
import { evaluateAllegroPublicationReadback } from './domain/allegro-publication-readback.mjs';
...
import { allegroMessagePlainText, buildAllegroReplyStyleProfile, classifyAllegroMessageAuthor, fetchAllegroReplyHistory, improvePolishReplyStyle, mergeAllegroReplyHistory } from './domain/allegro-reply-assistant.mjs';
import { createStoreOrderSupplierReconciliation } from './store-order-supplier-reconciliation.mjs';
import { createVonHalskyWarehouseCoordinator } from './domain/von-halsky-warehouse-coordinator.mjs';
...
import { ALLEGRO_AGENT_OFFER_PROCEDURE, buildAllegroPublicationSuccessFields, buildAllegroPublicationUnconfirmedFields, createAllegroPublicationAgent } from './domain/allegro-publication-agent.mjs';
...
import { allegroConfiguredProductSet, allegroProductSetIdentityMatches, allegroProductSetPayload } from './domain/allegro-product-set.mjs';
...
import { allegroOfferVerification, allegroPatchZDraftu, allegroPublicationConfirmation } from './domain/allegro-offer-patch.mjs';
...
import { allegroIdentifierRecoveryProduct, executeAllegroOfferWriteWithRecovery } from './domain/allegro-publication-recovery.mjs';
...
import { allegroCatalogParameterValue, allegroCatalogProductParameterAssessment, allegroCatalogProductReference } from './domain/allegro-catalog-parameters.mjs';
...
import { buildAllegroOfferSyncResponse } from './domain/sales-channel-coordination.mjs';
...
import { mergeImportedProductSourceRefresh } from './domain/imported-product-catalog.mjs';
...
import { allegroCategoryConsensus, allegroCategoryIntentPhrases, allegroCategoryParentPath, allegroCategoryResolution, allegroCategorySpecificScore, allegroCategorySuggestedSelection, allegroCorrectCategorySelection, allegroPreferredOfferCategory } from './domain/allegro-category-classifier.mjs';
import { ALLEGRO_COMPLIANCE_POLICY, allegroCheckText, allegroSanitizePlainText, allegroSanitizeDescription, allegroEnforceDraft, allegroSecureOfferWrite } from './allegro-compliance.mjs';
import { ustawieniaPubliczneBezDanychPrywatnych, infaktDostawcyDozwoleni, produktBezDanychPrywatnych } from './infakt-purchase.mjs';
...
const { pobierzProduktProducenta, pobierzProduktProducentaZPamiecia, produktLinkDuplikaty, produktLinkKategoriaSklepu, inspectProductUrl, inspectProductUrlViaReader } = createProductSourceInspectionService({
  read: czytaj,
  write: zapisz,
...
const { publicznyOrigin, paynowKonfiguracja, paynowDiagnostyka, podpisPaynowPowiadomienia, porownajPodpis, kluczIdempotencji, grosze, statusPlatnosciPaynow, paynowWywolaj, payloadPlatnosciPaynow, aktualizujZamowieniePaynow } = createPaynowService({ read: czytaj, write: zapisz });
const { infaktPublicConfig, infaktDostawcyUstawienia, infaktKosztDoZwrotu, infaktPobierzKosztyDozwolone, infaktSynchronizujCenyZakupu, infaktPrzypiszCeneZakupu, infaktCofnijDopasowanieCeny, infaktErrorText, infaktWywolaj, infaktPayloadZamowienia, infaktRef, infaktInvoiceFromTask } = createInfaktService({
  read: czytaj,
  write: zapisz,
  loadProducts: (data) => allegroAgentProduktyKompletne(data),
  saveProductFields: (input) => zapiszIOpublikujPolaProduktuCentralnie(input),
});
const { emailKonfiguracja, emailPublicConfig, sprawdzEmailSMTP, wyslijEmailSMTP, zlSerwer, htmlEscape, wiadomoscKlientaZamowienie, dopiszHistorieEmaila, wyslijEmaileNowegoZamowienia, wyslijEmailStatusowy, polaczPowiadomienia, obsluzEmailePrzejsciaStatusu } = createEmailService({ read: czytaj, write: zapisz });
const { inpostKonfiguracja, inpostPublicConfig, inpostWywolaj, inpostSzukajPunktow, czyDostawaPaczkomatInPost, walidujPrzesylkeInPost, inpostDostepnoscUslug, inpostOrganizacja, przesylkaShipXPayload, numerZShipX, inpostStatusZShipX, inpostEtykietaGotowa, inpostOfertaId, inpostCzekajNaEtykiete, inpostWebhookSecret, inpostWebhookAutoryzowany, inpostZdarzeniaZWebhooka, inpostDaneZWebhooka, zapiszLogInpostWebhook, zastosujWebhookInpost, zapiszPrzesylkeNaZamowieniu } = createInpostService({ read: czytaj, write: zapisz, onOrderStatusTransition: obsluzEmailePrzejsciaStatusu, getOrderReconciliation: () => storeOrderSupplierReconciliation });
...
const vonHalskyWarehouseCoordinator = createVonHalskyWarehouseCoordinator({ readSettings: () => czytaj('settings', { data: {}, rev: 0, updated_at: null }), mergeSettings: (settings) => productLinkImport.mergeSettings(settings), catalogProducts: (settings) => mergeCatalogProducts(settings).products, reconciliation: () => storeOrderSupplierReconciliation, text: tekst });
const vonHalskyRoute = createVonHalskyRoute({
  ...
  coordinateWarehouseOrders: (orders) => vonHalskyWarehouseCoordinator.coordinate(orders),
  inspectWarehouseOrders: (orders) => vonHalskyWarehouseCoordinator.inspect(orders),
  draftMessageWithAgent: (input, actor) => agentSpecialists.run(input, actor),
  sendEmail: wyslijEmailSMTP,
  emailPublicConfig,
  inpost: { configure: inpostKonfiguracja, call: inpostWywolaj, validateShipment: walidujPrzesylkeInPost, serviceAvailability: inpostDostepnoscUslug, organization: inpostOrganizacja, shipmentPayload: przesylkaShipXPayload, trackingNumber: numerZShipX, shipmentStatus: inpostStatusZShipX, labelReady: inpostEtykietaGotowa, offerId: inpostOfertaId, waitForLabel: inpostCzekajNaEtykiete },
  loadCatalog: async () => {
...
const allegroPreparationRoute = createAllegroPreparationRoute({
  ...
  pool: postgresPool, listenerPool: postgresListenerPool, namespace: STORE_NAME,
  // Ciężkie przygotowanie buduje pełny kontekst katalogu i wykonuje odczyty
  // kanałów. Dwa workery utrzymują responsywność sklepu na VPS; większą liczbę
  // można włączyć wyłącznie świadomie przez zmienną środowiskową.
  workerConcurrency: Number(process.env.ALLEGRO_PREPARATION_WORKERS || 2),
  coordinate: coordinateProductEvent, afterPrepare: przygotujProduktVonHalskyPoPelnejKontroli,
  worker: {
...
const allegroPublicationQueueRoute = createAllegroPublicationQueueRoute({ pool: postgresPool, namespace: STORE_NAME, respond: odpowiedz, isAdmin: czyAdmin, sessionOf: requestSession, getProduct: (productId) => centralProductCatalog.get(productId, { admin: true }), getProducts: (productIds) => centralProductCatalog.getMany(productIds, { admin: true }), runtime: agentRuntime, readVersioned: czytajWersjonowane, writeIfVersion: zapiszJesliWersja, prepareProducts: (productIds, options) => allegroPreparationRoute.prepareProducts(productIds, options), reconcileTasks: allegroUzgodnijZadaniaPublikacji, preparationReport: productFullPreparationReport });
enqueueAllegroPublicationRepair = (productId) => allegroPreparationRoute.prepareProducts([productId], { operation: 'allegro-repair', requestedBy: 'operator-publikacji-allegro' });
...
const agentRuntimeRoute = createAgentRuntimeRoute({
  ...
  runtime: agentRuntime,
  integrationStatus: async (req) => {
    const [allegro, vonHalsky] = await Promise.all([
      allegroStatus(req),
      vonHalskyRepository.readStatus({ settings: {}, sync: {}, truth: {}, commandSummary: {} }),
    ]);
    return { allegro, vonHalsky };
  },
...
const agentOperationsRoute = createAgentOperationsRoute({ respond: odpowiedz, isAdmin: czyAdmin, text: tekst, read: czytaj, write: zapisz, getOperationalCenter: agentCentrumOperacyjne });
...
const storeOrderSupplierReconciliation = createStoreOrderSupplierReconciliation({
  ...
  readVonHalskyOrders: async () => (await vonHalskyRepository.readVersioned(VON_HALSKY_STORE_KEY, { orders: [] }))?.value?.orders || [],
...
function allegroBrakujaceParametryWymagane(product = {}, categoryParameters = [], seedParameters = []) {
  ...
  const present = new Set([...auto, ...custom, ...(Array.isArray(seedParameters) ? seedParameters : [])].map((x) => String(x?.id || '')).filter(Boolean));
...
async function allegroDraftZAutoKategoria(req, product = {}, opt = {}) {
  const options = { ...(opt || {}) };
  const configuredProductSet = allegroConfiguredProductSet(product);
  if (configuredProductSet.ready && !options.categoryId && !product.allegroCategoryId) {
    options.categoryId = configuredProductSet.items[0]?.categoryId || '';
  }
  ...
  const categoryConsensusApplicable = !configuredProductSet.ready && categoryConsensus.selected
  ...
  const [salesConditions, catalogLookup] = await Promise.all([
    ...
    configuredProductSet.ready
      ? Promise.resolve({ searchedBy: 'verified-product-set', selected: null, products: [] })
  ...
  const preferredCategory = allegroPreferredOfferCategory({
    fallbackCategoryId: categoryId,
    existingOffer,
    catalogMatch,
  });
  let effectiveCategoryId = tekst(preferredCategory.categoryId, 80).trim();
  ...
  const categoryCorrection = !configuredProductSet.ready && !preferredCategory.locked
    ? await allegroCorrectCategorySelection({ product, categoryId: effectiveCategoryId, parameters: categoryParameters.parameters, suggest: () => allegroSugerujKategorie(req, product, { limit: 8 }), loadParameters: (id) => allegroParametryKategorii(req, id) })
    : { changed: false };
  if (categoryCorrection.changed) {
    categorySuggestion = categoryCorrection.suggestion; effectiveCategoryId = categoryCorrection.categoryId;
    options.categoryId = effectiveCategoryId; categoryParameters = categoryCorrection.parameters;
    // Po korekcie kategorii ponawiamy deterministyczne wzbogacenie parametrów i GPSR.
    product = enrichAllegroProductEvidence({ ...product, allegroCategoryId: effectiveCategoryId }, relatedProducts).product;
  }
  ...
  const catalogParameterAssessment = allegroCatalogProductParameterAssessment(catalog, categoryParameters.parameters);
  const catalogDependencyMissing = existingCatalogProductId || !catalogIdentityVerified ? [] : catalogParameterAssessment.incompatibleParameterIds.map((id) => `produkt katalogowy Allegro ma sprzeczny parametr zależny: ${categoryParameters.parameters.find((parameter) => String(parameter?.id || '') === id)?.name || id}`);
  ...
  } else if (catalogIdentityVerified && catalogMatch?.selected?.id && !catalogParameterAssessment.incompatibleParameterIds.length) {
    options.catalogProductId = catalogMatch.selected.id;
    options.catalogIdentityVerified = true;
  }
  options.catalogProductParameters = (options.catalogProductId || catalogIdentityVerified) ? catalogParameterAssessment.parameters : [];
  options.catalogImages = allegroZdjecia(catalog);
  ...
  const requiredParameters = options.catalogProductId || configuredProductSet.ready
    ? []
    : allegroBrakujaceParametryWymagane(preparedProduct, categoryParameters.parameters, options.catalogProductParameters);
  options.requiredParameters = requiredParameters;
  const draft = allegroDraftZProduktu(preparedProduct, options); if (catalogDependencyMissing.length) draft.missing = [...new Set([...draft.missing, ...catalogDependencyMissing])];
  ...
  if (!configuredProductSet.ready && (!Array.isArray(catalog?.productSafety?.responsibleProducers) || !catalog.productSafety.responsibleProducers.length)) {
  ...
  return {
    ...
    autoFilled: {
      ...
      allegroCatalogIdentityConfirmed: options.catalogIdentityVerified === true,
      allegroCatalogCheckedAt: options.catalogIdentityVerified === true ? new Date().toISOString() : (product.allegroCatalogCheckedAt || ''),
      allegroCatalogSource: options.catalogIdentityVerified === true ? 'allegro-product-catalog-exact-gtin' : (product.allegroCatalogSource || ''),
      ...
      allegroSafetyInformationProvenance: options.catalogIdentityVerified === true && gpsr.safetyInformation
        ? {
            source: 'Katalog Produktów Allegro',
            method: 'exact_gtin_verified_catalog_product',
            catalogProductId: options.catalogProductId || '',
            fetchedAt: new Date().toISOString(),
          }
        : (product.allegroSafetyInformationProvenance || null),
      allegroResponsibleProducer: gpsr.responsibleProducer,
      allegroParameterEvidence: preparedProduct.allegroParameterEvidence || {},
    },
    publicationReadiness: {
      catalogIdentityVerified: options.catalogIdentityVerified === true || configuredProductSet.ready,
      productSetIdentityVerified: configuredProductSet.ready,
      productSetCount: configuredProductSet.items.length,
      categoryResolved: !!effectiveCategoryId,
      requiredParametersResolved: requiredParameters.length === 0,
      gpsrReady: gpsr.ready || !!existingOffer,
...
function allegroDraftZProduktu(product = {}, opt = {}) {
  const p = product || {};
  const configuredProductSet = allegroProductSetPayload(p);
  ...
  const galleryBudget = allegroGalleryBudget({
    offerImages: [p.zdjecie, ...(Array.isArray(p.zdjecia) ? p.zdjecia : [])],
    catalogImages: opt.catalogImages,
    catalogProductId: allegroProductId,
  });
  const images = galleryBudget.images;
  const parameters = [];
  ...
  const productObj = allegroCatalogProductReference({
    catalogProductId: allegroProductId,
    categoryId,
    gtin,
    name: offerTitle,
    parameters: Array.isArray(opt.catalogProductParameters) && opt.catalogProductParameters.length
      ? opt.catalogProductParameters
      : [...parameters.filter((x) => x.id), ...productParameters],
    images,
  });
  // Wyłącz obrazy katalogowe przy własnej galerii: Allegro liczy oba źródła do wspólnego limitu 16.
  if (productObj.id && !productObj.idType && images.length) productObj.images = [];
  const stockRaw = Number(opt.offerStock ?? ALLEGRO_DEFAULT_OFFER_STOCK);
  const payload = {
    name: offerTitle,
    category: categoryId ? { id: categoryId } : undefined,
    productSet: configuredProductSet.ready
      ? configuredProductSet.productSet
      : [{ product: productObj }],
  ...
  if (configuredProductSet.configured && !configuredProductSet.ready) {
    for (const problem of configuredProductSet.missing) missing.push(`zestaw produktowy Allegro: ${problem}`);
  }
  ...
  if (!images.length && !allegroProductId) missing.push('zdjęcia');
  ...
  return { payload: enforced.draft, missing: [...new Set(missing)], compliance: enforced.compliance, galleryBudget };
}
async function allegroZweryfikujTozsamoscPublikacji(req, product = {}, draft = {}, prepared = {}, opt = {}) {
  ...
  const configuredProductSet = allegroConfiguredProductSet(product);
  if (configuredProductSet.configured) {
    const match = allegroProductSetIdentityMatches(product, draft?.productSet);
    return match.ok
      ? { ok: true, mode: 'verified_product_set', productCount: configuredProductSet.items.length }
      : { ok: false, code: 'allegro_product_set_identity_mismatch', reason: match.reason || 'Zestaw produktowy Allegro nie jest w pełni potwierdzony.' };
  }
...
async function allegroUzgodnijZadaniaPublikacji(tasks = []) {
  const list = Array.isArray(tasks) ? tasks : [];
  if (!list.length) return [];
  const [offersRec, products] = await Promise.all([
    czytaj('allegro_offers', { items: [] }),
    allegroAgentProduktyKompletne(),
  ]);
  const offers = allegroOfertyItems(offersRec);
  const offerById = new Map(offers.map((offer) => [String(offer?.id || ''), offer]));
  const results = [];
  for (const task of list) {
    const productId = tekst(task?.productId, 100).trim();
    const product = products.get(productId);
    const offerId = tekst(task?.targetId || task?.result?.offerId || product?.allegroOfferId, 120).trim();
    const offer = offerById.get(offerId);
    if (!product || !offerId || !offer) {
      results.push({ taskId: task?.id, confirmed: false, offerId, reason: 'Brak pewnego ID oferty albo świeżego odczytu tej oferty z Allegro.' });
      continue;
    }
    const evaluation = evaluateAllegroPublicationReadback({ task, product, products, offer });
    if (!evaluation.confirmed) {
      results.push({ taskId: task?.id, ...evaluation });
      continue;
    }
    const remoteStatus = evaluation.status;
    const now = new Date().toISOString();
    const published = remoteStatus === 'ACTIVE';
    await zapiszIOpublikujPolaProduktuCentralnie({
      productId,
      fields: {
        allegroOfferId: offerId,
        allegroStatus: remoteStatus,
        allegroPublicationStatus: remoteStatus,
        allegroSyncedAt: now,
        allegroSyncSource: 'allegro-readback-reconciliation',
        allegroAgentPreparationStatus: published ? 'published' : 'ready',
        allegroAgentPreparationMissing: [],
        allegroAgentPreparationError: '',
        allegroAgentPublicationError: '',
        allegroPublicationAgentStatus: 'completed',
        allegroPublicationLastSuccessAt: now,
        allegroPublicationLastErrorCode: '',
        allegroPublicationLastError: '',
      },
      mutationId: `allegro-publication-readback:${productId}:${offerId}:${remoteStatus}`,
      actor: 'agent-allegro-readback',
      area: 'allegro-publication-reconciliation',
    });
    await allegroPreparationRoute.resolveProduct(productId, { reason: 'allegro_publication_readback_confirmed', offerId }).catch(() => null);
    results.push({ taskId: task?.id, ...evaluation });
  }
  return results;
}
async function allegroZapiszPowiazanieProduktu(product = {}, details = {}) {
  ...
  const correctedGtin = details.catalogRecovery?.kind === 'identifier_value' ? canonicalGtin(details.catalogRecovery?.correctedParameters?.[0]?.expectedValue || '') : '';
  if (correctedGtin) {
    const exactGtin = tekst(details.catalogRecovery.correctedParameters[0].expectedValue, 80).replace(/\D/g, '');
    autoPatch.ean = exactGtin; autoPatch.gtin = exactGtin;
  }
  ...
  if (details.resolveTasks !== false) await allegroPreparationRoute.resolveProduct(productId, { reason: 'allegro_publication_confirmed', offerId });
  return fields;
}

async function allegroZapiszNiepotwierdzonePowiazanieProduktu(product = {}, details = {}) {
  const productId = tekst(product.id, 100).trim(), offerId = tekst(details.offerId, 100).trim();
  if (!productId || !offerId) return null;
  const fields = buildAllegroPublicationUnconfirmedFields({ text: tekst, details: { ...details, offerId } });
  await zapiszIOpublikujPolaProduktuCentralnie({ productId, fields, mutationId: `allegro-publication-unconfirmed:${productId}:${offerId}:${Date.now()}`, actor: 'allegro-api', area: 'allegro-publication' });
  return fields;
}
...
async function seoWykonajDziennyPlan({ limit, source } = {}) {
  ...
  const today = new Date().toISOString().slice(0, 10), products = [...canonical.values()]
    .filter((product) => Number(product.cena || product.price) > 0)
    ...
    const aa = a.product?._catalog?.availability?.saleAvailable === false ? 1 : 0, ba = b.product?._catalog?.availability?.saleAvailable === false ? 1 : 0;
  ...
  for (const item of selected) {
    ...
    if (['seoTitle', 'seoDescription', 'seoKeywords'].some((field) => patch[field] !== undefined && String(patch[field]) !== String(item.product[field] || ''))) patch.seoContentUpdatedAt = now;
    patch.seoScore = seoOcena({ ...item.product, ...patch });
    await zapiszIOpublikujPolaProduktuCentralnie({ productId: String(item.product.id), fields: patch, mutationId: `seo:${String(item.product.id)}:${now}`, actor: 'seo-automation', area: 'seo' });
  }
  const seoCatalog = buildStorefrontSeoCatalogSnapshot({ data, canonicalProducts: products.map((item) => item.product) }), previousRunAt = Date.parse(config.lastRunAt || '');
  const recentlyChanged = Number.isFinite(previousRunAt) ? seoCatalog.indexableProducts.filter((product) => Date.parse(product?._catalog?.syncedAt || product.updatedAt || '') > previousRunAt) : [];
  const indexableIds = new Set(seoCatalog.indexableProducts.map((product) => String(product.id))), changedProducts = [...new Map([...selected.map((item) => item.product), ...recentlyChanged].filter((product) => indexableIds.has(String(product.id))).map((product) => [String(product.id), product])).values()];
  const promotion = await runIndexNowPromotion({ catalogProducts: seoCatalog.indexableProducts, changedProducts, config });
  const fullCatalogSubmission = promotion.scope === 'full-catalog';
  const channels = buildSeoChannelReport({ selectedProducts: selected.map((item) => item.product), catalogProducts: seoCatalog.indexableProducts, promotion, runAt: now });
  data.artway_seo_ustawienia = { ...config, dailyLimit: amount, lastRunAt: now, lastRunCount: selected.length, lastScheduledDay: isScheduledSeoSource(runSource) ? scheduledDay : (config.lastScheduledDay || ''), lastChannels: channels, lastPromotionAt: promotion.submitted ? now : (config.lastPromotionAt || ''), lastPromotionStatus: promotion.submitted ? promotion.status : (config.lastPromotionStatus || promotion.status), lastPromotionCount: promotion.submitted ? promotion.count : (Number(config.lastPromotionCount) || 0), lastPromotionHttpStatus: promotion.submitted ? promotion.httpStatus : (config.lastPromotionHttpStatus || null), indexNowFullCatalogAt: fullCatalogSubmission && promotion.accepted ? now : (config.indexNowFullCatalogAt || ''), indexNowFullCatalogCount: fullCatalogSubmission && promotion.accepted ? Math.max(0, promotion.count - 1) : (Number(config.indexNowFullCatalogCount) || 0), indexNowCatalogVersion: promotion.accepted && promotion.catalogVersion ? promotion.catalogVersion : (config.indexNowCatalogVersion || '') };
...
const systemDiagnostics = createSystemDiagnosticsRoute({
  ...
  agentRuntime,
  diagnosticAgent,
  enqueueCodeRepair: (item, options) => enqueueDiagnosticCodeRepair(codexAgentQueue, item, options),
...
const customerPublicationsRoute = createCustomerPublicationsRoute({ read: czytaj, readVersioned: czytajWersjonowane, writeIfVersion: zapiszJesliWersja, respond: odpowiedz, isAdmin: czyAdmin }); const contestCenterRoute = createContestCenterRoute({ read: czytaj, readVersioned: czytajWersjonowane, writeIfVersion: zapiszJesliWersja, respond: odpowiedz, isAdmin: czyAdmin, sessionOf: requestSession, rateLimit: ograniczRuch }); const siteReleaseCenterRoute = createSiteReleaseCenterRoute({ read: czytaj, readVersioned: czytajWersjonowane, writeIfVersion: zapiszJesliWersja, catalog: centralProductCatalog, respond: odpowiedz, isAdmin: czyAdmin, sessionOf: requestSession });
agentEvents.connect({
...
export default async (req) => {
  ...
  try {
    ...
    const siteReleaseResponse = await siteReleaseCenterRoute(req, url, action); if (siteReleaseResponse) return siteReleaseResponse; const customerPublicationsResponse = await customerPublicationsRoute(req, url, action); if (customerPublicationsResponse) return customerPublicationsResponse; const contestCenterResponse = await contestCenterRoute(req, url, action); if (contestCenterResponse) return contestCenterResponse;
    ...
    if (action === 'seo-automation-status') { if (req.method !== 'GET') return odpowiedz({ ok: false, error: 'Metoda niedozwolona' }, 405); if (!czyAdmin(req, url)) return odpowiedz({ ok: false, error: 'Brak uprawnień administratora', code: 'auth' }, 401); const rec = await czytaj('settings', { data: {}, rev: 0 }); return odpowiedz({ ok: true, ...buildSeoAutomationStatus(rec.data, { updatedAt: rec.updated_at }) }); }
    ...
    const allegroPublicationQueueResponse = await allegroPublicationQueueRoute(req, url, action);
    if (allegroPublicationQueueResponse) return allegroPublicationQueueResponse;
    ...
    if (action === 'allegro-create-product-offer') {
      ...
      let categorySuggestion = null;
      let prepared = await allegroDraftZAutoKategoria(req, publicationProduct, { ...(body.options || {}), relatedProducts: authoritativeProducts });
      ...
      try {
        ...
        const execution = await executeAllegroOfferWriteWithRecovery({
          ...
          prepareRecovery: async ({ hint, catalog, decision }) => {
            const recoveryProduct = allegroIdentifierRecoveryProduct(publicationProduct, hint, decision.categoryId);
            const next = await allegroDraftZAutoKategoria(req, recoveryProduct, { ...(body.options || {}), relatedProducts: authoritativeProducts, categoryId: decision.categoryId, ...(catalog ? { catalogProductId: decision.productId, catalogIdentityVerified: true, catalogMatchOverride: { selected: catalog, searchedBy: 'metadata błędu Allegro' } } : {}) });
            const gate = allegroEnforceDraft(next.payload || {}), check = await allegroZweryfikujTozsamoscPublikacji(req, recoveryProduct, gate.draft, next);
      ...
      let verifiedOffer = null, publicationWait = null, expectedStatus = '';
      try {
        const publicationAction = String(body.options?.publicationAction || '').toLowerCase();
        expectedStatus = publicationAction === 'activate' ? 'ACTIVE' : publicationAction === 'deactivate' ? 'INACTIVE' : '';
        if (expectedStatus) {
      ...
      const confirmation = allegroPublicationConfirmation({ expectedStatus, publicationWait, verifiedOffer });
      const verifiedStatus = confirmation.status, publicationConfirmed = confirmation.confirmed;
      if (offerId) {
        const offersRec = await czytaj('allegro_offers', { items: [] });
        ...
        if (productId) {
          ...
          const products = new Map(authoritativeProducts), centralProduct = await centralProductCatalog.get(productId, { admin: true }) || products.get(productId) || publicationProduct; products.set(productId, centralProduct);
          ...
          const linkDetails = { offerId, prepared, draft, offer: result || normalized, verifiedOffer, imagePublication, catalogRecovery, expectedStatus };
          if (publicationConfirmed) await allegroZapiszPowiazanieProduktu(publicationProduct, linkDetails);
          else await allegroZapiszNiepotwierdzonePowiazanieProduktu(publicationProduct, linkDetails);
        }
      }
      const responseBody = { ok: publicationConfirmed, offer: { ...(existing?.offer || {}), ...(result || {}), id: offerId }, mode: existing ? 'updated' : 'created', duplicatePrevented: !!existing, match: existing ? { score: existing.score, reason: existing.reason } : null, identityCheck, catalogRecovery, catalogMatch: prepared.catalogMatch || null, autoFilled: prepared.autoFilled || null, improvedDescriptions: prepared.improvedDescriptions || null, imagePublication, compliance: complianceGate.compliance, verification: { ...allegroOfferVerification(result, !!verifiedOffer), publicationConfirmed, publicationChecks: publicationWait?.checks || 0, expectedStatus }, agentDecision: prepared.agentDecision || null, agentProcedure: ALLEGRO_AGENT_OFFER_PROCEDURE, warnings: Array.isArray(result?.warnings) ? result.warnings : [], operation: { id: approvalOperationId, status: responseMeta?.status || 200, location: responseMeta?.location || '', completed: publicationConfirmed && (operationCheck.completed || publicationWait?.completed === true), checks: operationCheck.checks || 0 }, allegro: await allegroStatus(req), categorySuggestion };
      if (!publicationConfirmed) {
        const unconfirmedError = Object.assign(new Error(`Allegro zapisało ofertę ${offerId}, ale odczyt API nie potwierdził statusu ${expectedStatus || 'po operacji'}. Ponowienie zaktualizuje tę samą ofertę bez tworzenia duplikatu.`), { code: 'allegro_publication_unconfirmed', status: 409 });
        responseBody.error = unconfirmedError.message;
        responseBody.code = unconfirmedError.code;
        await zapisz('allegro_offer_last_error', { at: new Date().toISOString(), productId: saleProductId, productName: tekst(publicationProduct.nazwa || publicationProduct.name, 300), message: unconfirmedError.message, status: 409, code: unconfirmedError.code, offerId, expectedStatus, verifiedStatus });
        await allegroPublicationTracker.unconfirmed({ productId: saleProductId, prepared, draft, existing, idempotencyKey: publicationAttemptId, responseMeta, error: unconfirmedError });
        await allegroOperationReceipts.fail(approvalHandle, unconfirmedError);
        return odpowiedz(responseBody, 409);
      }
