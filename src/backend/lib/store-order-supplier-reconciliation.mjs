...
import { allegroOrderClosedForSupplierDemand, allegroOrdersForInventoryDeduction, allegroOrdersForSupplierDemand } from './domain/allegro-supplier-demand.mjs';
import { recordInventoryShortfall } from './domain/inventory-shortfall.mjs';
import { vonHalskyOrdersForSupplierDemand } from './domain/von-halsky-supplier-demand.mjs';
import { vonHalskyOrderShippingStage } from './domain/von-halsky-order-shipment.mjs';
...
export function createStoreOrderSupplierReconciliation({
  ...
  async function reconcileDrafts() {
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      const [ordersVersion, settingsVersion, allegroVersion, mappingsVersion, offersVersion, vonHalskyOrders] = await Promise.all([
      ...
      const vonHalskyDemand = vonHalskyOrdersForSupplierDemand(array(vonHalskyOrders), products);
      const orders = normalizeOrderInventoryModes([...shopOrders, ...allegroDemand.orders, ...vonHalskyDemand.orders], settings);
      const closedOrderReferences = [
        ...shopOrders.filter(shipmentFinal).map(orderNumber),
        ...array(object(allegroVersion.value).items)
          .filter(allegroOrderClosedForSupplierDemand)
          .map((order) => `Allegro ${text(order?.id || order?.nr || order?.orderId || order?.checkoutForm?.id, 180)}`),
        ...array(vonHalskyOrders)
          .filter((order) => {
            const stage = vonHalskyOrderShippingStage(order);
            return ['shipped', 'in_transit', 'delivered'].includes(stage.key)
              || ['COMPLETED', 'CANCELLED', 'REFUNDED', 'REFUSED'].includes(text(order?.status, 80).toUpperCase());
          })
      ...
      if (!result.changed) {
        return {
          ...
          diagnostics: { ...result.diagnostics, allegro: allegroDemand.diagnostics, vonHalsky: vonHalskyDemand.diagnostics },
        };
      }
