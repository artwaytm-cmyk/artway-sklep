  ...
  async function wyslijEmailSMTP({ to, subject, text, html, replyTo, attachments = [] }) {
    ...
    try {
      ...
      return {
        ...
        accepted: Array.isArray(info.accepted) ? info.accepted.map((value) => tekst(value, 240)) : [],
        rejected: Array.isArray(info.rejected) ? info.rejected.map((value) => tekst(value, 240)) : [],
        pending: Array.isArray(info.pending) ? info.pending.map((value) => tekst(value, 240)) : [],
        response: tekst(info.response || '', 500),
      };
    } catch (error) {
      const safe = bezpiecznyBladEmail(error);
      smtpHealth = { authenticated: false, checkedAt: new Date().toISOString(), ...safe };
