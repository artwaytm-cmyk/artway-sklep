...
export function createInventoryStockRoute({
  ...
  return async function inventoryStockRoute(req, url, action) {
    if (action === 'warehouse-product-location-assign' || action === 'warehouse-location-delete-preview' || action === 'warehouse-location-delete') {
      ...
      try {
        if (action === 'warehouse-product-location-assign') return respond(await locations.assign(body, actor));
        return respond(action.endsWith('-preview') ? await locations.preview(body) : await locations.remove(body, actor));
      }
      catch (error) { return respond({ ok: false, error: error?.message || 'Nie udało się zapisać lokalizacji.', code: error?.code || 'warehouse_location_error', ...(error?.details || {}) }, Number(error?.status || 422)); }
    }
