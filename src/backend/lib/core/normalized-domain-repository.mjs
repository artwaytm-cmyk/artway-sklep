...
async function writeSettingsDomain(client, namespace, key, value, config, version = null, prepared = null) {
...
function prepareNormalizedDomainWrite(value, config) {
  const split = splitNormalizedValue(value, config);
  const payload = split.records.map((row) => ({
  ...
  return {
    ...
    metadataJson: JSON.stringify(split.metadata || {}),
    payloadJson: JSON.stringify(payload),
  };
}

function postgresConnectionIsBroken(error) {
  const code = String(error?.code || '');
  const message = String(error?.message || error || '');
  return /^08/.test(code)
    || ['57P01', '57P02', '57P03', '25P03'].includes(code)
    || /connection (?:terminated|closed)|connection error|not queryable|terminating connection/i.test(message);
}

async function rollbackAfterFailure(client, error) {
  let destroy = postgresConnectionIsBroken(error);
  try {
...
async function replaceDomain(client, namespace, domain, value, config, {
  expectedVersion = null, initialVersion = null, updatedAt = null,
  skipIfEqual = false, prepared = null,
} = {}) {
  const normalized = prepared || prepareNormalizedDomainWrite(value, config);
  const { split, hash, metadataJson, payloadJson } = normalized;
  ...
  await client.query(`INSERT INTO artway_domain_snapshots(namespace,domain,metadata,content_hash,version,updated_at) VALUES($1,$2,$3::jsonb,$4,$5,$6)
    ON CONFLICT(namespace,domain) DO UPDATE SET metadata=EXCLUDED.metadata,content_hash=EXCLUDED.content_hash,version=EXCLUDED.version,updated_at=EXCLUDED.updated_at`, [namespace, domain, metadataJson, hash, nextVersion, timestamp]);
  const payload = split.records;
  if (dedicatedTableForDomain(domain)) {
    await replaceDedicatedDomainRecords(client, namespace, domain, split.records, timestamp, payloadJson);
    ...
    await client.query(`WITH incoming AS (
      ...
      WHERE artway_domain_records.ordinal IS DISTINCT FROM EXCLUDED.ordinal OR artway_domain_records.data IS DISTINCT FROM EXCLUDED.data`, [namespace, domain, payloadJson, timestamp]);
    await client.query(`DELETE FROM artway_domain_records r WHERE namespace=$1 AND domain=$2 AND NOT EXISTS (
      ...
      WHERE x.collection=r.collection AND x.record_id=r.record_id)`, [namespace, domain, payloadJson]);
...
export function createNormalizedDomainRepository({ pool, namespace, legacy }) {
  ...
  const writeSettingsDomainValue = async (key, value, version = null) => {
    ...
    const prepared = prepareNormalizedDomainWrite(value, config);
    const client = await pool.connect();
    let releaseError = null;
    try {
      await client.query('BEGIN');
      const result = await writeSettingsDomain(client, namespace, settingsDomainFromKey(key), value, config, version, prepared);
      if (result?.unchanged) {
        await client.query('ROLLBACK');
  ...
  const writeSettingsIfVersion = async (value, version = null) => {
    ...
    const preparedDomains = new Map();
    for (const [key, config] of Object.entries(SETTINGS_DOMAIN_CONFIGS)) {
      if (!Object.prototype.hasOwnProperty.call(data, key)) continue;
      preparedDomains.set(domainForSetting(key), prepareNormalizedDomainWrite(data[key], config));
      delete data[key];
    }
    stored.data = data;
    ...
    try {
      ...
      const settingsDomains = [...preparedDomains.keys()];
      ...
      for (const [key, config] of Object.entries(SETTINGS_DOMAIN_CONFIGS)) {
        ...
        const prepared = preparedDomains.get(domain);
        if (!prepared) continue;
        if (hashes.get(domain) !== prepared.hash) await replaceDomain(client, namespace, domain, null, config, { prepared });
      }
      if (current.rowCount) await client.query("UPDATE artway_kv_store SET value=$2::jsonb,version=version+1,updated_at=now() WHERE namespace=$1 AND key='settings'", [namespace, storedJson]);
      else await client.query("INSERT INTO artway_kv_store(namespace,key,value,version,updated_at) VALUES($1,'settings',$2::jsonb,1,now())", [namespace, storedJson]);
      await client.query('COMMIT'); return { modified: true };
    } catch (error) { releaseError = await rollbackAfterFailure(client, error); throw error; }
    finally { client.release(releaseError || undefined); }
  };
  ...
  const writeDirect = async (key, value, config, version = null) => {
    ...
    if (version && version.exists !== false && expectedVersion === null) return { modified: false };
    // Dzielenie, haszowanie i serializacja dużych rejestrów (np. ofert
    // Allegro) muszą zakończyć się przed BEGIN. Dzięki temu PostgreSQL nigdy
    // nie widzi bezczynnej transakcji podczas pracy procesora Node.
    const prepared = prepareNormalizedDomainWrite(value, config);
    const client = await pool.connect();
    let releaseError = null;
    try {
