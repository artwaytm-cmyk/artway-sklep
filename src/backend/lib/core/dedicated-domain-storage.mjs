...
export async function replaceDedicatedDomainRecords(client, namespace, domain, records, updatedAt, serializedPayload = '') {
  const table = dedicatedTableForDomain(domain);
  if (!table) return false;
  const source = records || [];
  const payloadJson = serializedPayload || JSON.stringify(source.map((row) => ({
    collection: row.collection,
  ...
  if (source.length) {
    ...
    [namespace, domain, payloadJson, updatedAt]);
