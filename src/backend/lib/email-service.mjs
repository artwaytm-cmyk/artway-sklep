...
import { storeShippingConfig } from './domain/store-shipping-config.mjs';
...
export function createEmailService({ read, write }) {
  ...
  const DOMYSLNA_WYSYLKA = storeShippingConfig({});
  const OPLATA_PACZKA_WEEKEND = DOMYSLNA_WYSYLKA.weekendPrice;
  ...
  function kosztyEmail(z) {
    ...
    const platnosc = kwotaSerwer(ma(z, 'oplataPlatnosci') ? z.oplataPlatnosci : (ma(k, 'platnosc') ? k.platnosc : (z?.platnoscId === 'pobranie' ? DOMYSLNA_WYSYLKA.cashOnDeliveryPrice : 0)));
    const metoda = tekst(z?.dostawaId || '', 40).trim().toLowerCase();
    const poRabacieZapisane = ma(k, 'poRabacie') ? kwotaSerwer(k.poRabacie) : 0;
    const kosztDomyslny = metoda === 'kurier' || metoda === 'kurier_inpost' ? DOMYSLNA_WYSYLKA.courier.price : DOMYSLNA_WYSYLKA.locker.price;
    const dostawa = ma(z, 'dostawaKoszt') || ma(k, 'dostawa')
      ? kwotaSerwer(ma(z, 'dostawaKoszt') ? z.dostawaKoszt : k.dostawa)
      : ((poRabacieZapisane || produktyZPozycji) >= DOMYSLNA_WYSYLKA.freeFrom ? 0 : kosztDomyslny);
