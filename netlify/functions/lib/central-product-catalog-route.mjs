export function createCentralProductCatalogRoute({ catalog, isAdmin, rateLimit, respond, revisionSnapshot, synchronize } = {}) {
  return async function centralProductCatalogRoute(request, url, action) {
    ...
    if (action === 'product-catalog-query') {
      ...
      const admin = isAdmin(request, url), metaBefore = await catalog.metadata(), revision = await revisionSnapshot();
      let synchronization = null;
      if (!metaBefore.count || metaBefore.outdated) synchronization = await synchronize({ force: true });
      else if (metaBefore.sourceRevision !== revision.sourceRevision) void synchronize().catch((error) => console.error('central_product_catalog_sync', error));
      const result = await catalog.query({
        admin, query: url.searchParams.get('q'), category: url.searchParams.get('category'), producer: url.searchParams.get('producer'), status: url.searchParams.get('status'), source: url.searchParams.get('source'), stock: url.searchParams.get('stock'), allegro: url.searchParams.get('allegro'), data: url.searchParams.get('data'), sale: url.searchParams.get('sale'), promotion: url.searchParams.get('promotion'), link: url.searchParams.get('link'),
      ...
      return respond({ ok: true, ...result, private: admin, stale: metaBefore.count > 0 && metaBefore.sourceRevision !== revision.sourceRevision, synchronization });
    }

    if (action === 'product-catalog-item') {
      if (request.method !== 'GET') return respond({ ok: false, error: 'Metoda niedozwolona' }, 405);
      const admin = isAdmin(request, url), product = await catalog.get(url.searchParams.get('id'), { admin });
